...
public class DataSetAssert {
    ...
    protected void appendColumnNames(Row row, StringBuilder result) {
    	appendColumnNames(row.getPrimaryKeyColumns(), result);
    	appendColumnNames(row.getColumns(), result);
        result.setLength(result.length() - 2);
    }


	private void appendColumnNames(List<Column> columns, StringBuilder result) {
    ...
    protected void appendRow(Row row, StringBuilder result) {
        appendColumnValues(row.getPrimaryKeyColumns(), result);
        appendColumnValues(row.getColumns(), result);
        result.setLength(result.length() - 2);
    }
    private void appendColumnValues(List<Column>columns, StringBuilder result) {
        for (Column column : columns) {
