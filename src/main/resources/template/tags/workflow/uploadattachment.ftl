<div id="aazone.workflow_attachments_list" style="DISPLAY: inline">
	...
	<table  style="border: 1px solid rgb(127, 170, 255); padding: 2px; display: block;  margin-top: 8px;" width="530" border="0">
		<thead>
			<tr>
				<td width="380">${uploadQueue}</td><td width="150">${uploadOperation}</td>
			</tr>
		</thead>
		<tbody >
			<#if attachments?exists>
				<#list attachments as being>
					<tr id="upload_tr_#{being.id}">
						...
						<td width="150">
						<#if deleteRight>
							<a href="#" onclick="deleteUpload('${ctx}/engine/upload!delete.htm',#{being.id})">${uploadDelete}</a>
						</#if>
						</td>
					</tr>
				</#list>
...
$(function(){
	if("${addRight}"=="true"){
		swfu= new SWFUpload({
			...
			button_width: 250,
			button_height: 18,
			button_text : '<span class="button">${uploadSFile}<span class="buttonSmall">(${fileMax}${fileSizeLimit}MB)</span></span>',
