...
public class OptionGroupDao extends HibernateDao<OptionGroup, Long> {
	...
	public List<OptionGroup> getAllOptionGroupBySystemId(Long systemId){
		String hql = "from OptionGroup og where og.companyId = ? and og.systemId=? order by og.createdTime desc";
		return this.find(hql.toString(),ContextUtils.getCompanyId(),systemId);
	}
