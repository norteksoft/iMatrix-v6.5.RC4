...
public class CornInfoDao extends HibernateDao<Timer, Long> {
	...
	public Timer getCornInfo(String typeEnum,String corn,String dateTime,String weekTime,String appointTime,String appointSet,Long jobInfoId,String everySecond,String everyHour){
		...
		if(TimingType.everyDate.toString().equals(typeEnum)){
			...
			if(StringUtils.isNotEmpty(everySecond)){
				hql=hql+" where t.timingType=? and t.jobId=? and t.everySecond=? ";
				corns=this.find(hql,TimingType.intervalTime,jobInfoId,everySecond);
			}else if(StringUtils.isNotEmpty(everyHour)){
				hql=hql+" where t.timingType=? and t.jobId=? and t.everyHour=? ";
				corns=this.find(hql,TimingType.intervalTime,jobInfoId,everyHour);
			}
			if(corns.size()>0)return corns.get(0);
		}
		return null;
