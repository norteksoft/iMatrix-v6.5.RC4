...
public class JobInfoAction extends CrudActionSupport<TimedTask> {
	...
	public String delete() throws Exception {
		...
		messages[0] = str.split("=-")[0];
		messages[1] = str.split("=-")[1];
...
//			addActionMessage("<font class='onSuccess'><nobr>成功删除"+str.split("=-")[0]+"条,失败"+str.split("=-")[1]+"条,不能删除已启用的记录.</nobr></font>");	
			addActionMessage("<font class='onSuccess'><nobr>"+Struts2Utils.getText("bs.job.info.successDelete.notDeleteEnabledData",messages)+"</nobr></font>");	
		}else{
		    addActionMessage("<font class='onSuccess'><nobr>"+Struts2Utils.getText("bs.job.info.successDelete",messages)+"</nobr></font>");
		}
		ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("bs.jobConfigure"), ApiFactory.getBussinessLogService().getI18nLogInfo("bs.deleteJob"),ContextUtils.getSystemId("bs"));
		return "job-info";
	}
	...
	public void prepareJobTimeTaskInput() throws Exception {
		prepareModel();
	}
	/**
	 * 新建
	 */
	@Action("job-info-timeTaskInput")
	public String jobTimeTaskInput() throws Exception {
		if(jobInfo.getRunAsUser()==null){
			jobInfo.setRunAsUser(ContextUtils.getLoginName());
			jobInfo.setRunAsUserName(ContextUtils.getUserName());
		}
		return "job-info-time-task-input";
	}
	...
	public String list() throws Exception {
		...
		ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("bs.jobConfigure"), ApiFactory.getBussinessLogService().getI18nLogInfo("bs.viewTimingList"),ContextUtils.getSystemId("bs"));
		return SUCCESS;
	}
	...
	public String save() throws Exception {
		...
		if(typeEnum!=null||everyMonth!=null||everyWeek!=null||everyDate!=null||appointTime!=null||appointSet!=null
			...
			ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("bs.jobConfigure"), ApiFactory.getBussinessLogService().getI18nLogInfo("bs.addTiming"),ContextUtils.getSystemId("bs"));
		}
		this.renderText(jobInfo.getId().toString());
		ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("bs.jobConfigure"), ApiFactory.getBussinessLogService().getI18nLogInfo("bs.saveTiming"),ContextUtils.getSystemId("bs"));
		return null;
	}
	...
	public String deleteCornInfo() throws Exception {
		...
		addActionMessage("<font class='onSuccess'><nobr>"+Struts2Utils.getText("basicSetting.deleteSuccess")+"</nobr></font>");//删除成功
		ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("bs.jobConfigure"), ApiFactory.getBussinessLogService().getI18nLogInfo("bs.deleteTimerTask"),ContextUtils.getSystemId("bs"));
		return "job-info";
	}
	...
	public String setState()throws Exception {
		...
		messages[0] = jobInfoManager.setJobInfos(ids,dataState)+"";
		addActionMessage("<font class='onSuccess'><nobr>"+Struts2Utils.getText("bs.job.info.successSet",messages)+"</nobr></font>");
		ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("bs.jobConfigure"), ApiFactory.getBussinessLogService().getI18nLogInfo("bs.controlTimer"),ContextUtils.getSystemId("bs"));
		return "job-info";
	}
