...
public class OptionGroupAction extends CrudActionSupport<OptionGroup> {
	...
	public String list() throws Exception {
		if(groups.getPageSize()>1){
			optionGroupManager.getAllOptionGroupForPage(groups,sysId);
			ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("bs.optionGroup"), ApiFactory.getBussinessLogService().getI18nLogInfo("bs.viewOptionGroupList"),ContextUtils.getSystemId("bs"));
	 ...
	 * @throws Exception
	 */
	@Action("option-group-checkGroupName")
	 ...
	 * @throws Exception
	 */
	@Action("option-group-checkGroupNo")
	...
	public String save() throws Exception {
		...
		if(optionGroup.getSystemId()==null){
			optionGroup.setSystemId(sysId);
		}
		option = optionGroupManager.saveOptionGroup(optionGroup, option);
		addActionMessage("<font class=\"onSuccess\"><nobr>"+Struts2Utils.getText("form.save.success")+"</nobr></font>");
		ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("bs.optionGroup"), ApiFactory.getBussinessLogService().getI18nLogInfo("bs.saveOptionGroup"),ContextUtils.getSystemId("bs"));
	...
	public String delete() throws Exception {
		...
		ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("bs.optionGroup"), ApiFactory.getBussinessLogService().getI18nLogInfo("bs.deleteOptionGroup"),ContextUtils.getSystemId("bs"));
		return null;
	}
	 ...
	 * @throws Exception
	 */
	@Action("option-group-deleteOption")
	...
	public String systemTree() throws Exception {
		...
		ZTreeNode root = new ZTreeNode("all_system","0",Struts2Utils.getText("bs.all.systems"), "true", "false", "", "", "folder", "");
		systemNodes.add(root);
//		businessSystems= businessSystemManager.getAllBusiness();
//		for(BusinessSystem system :businessSystems){
//			ZTreeNode node = new ZTreeNode(system.getId().toString(),"all_system",menuManager.getNameToi18n(system.getName()), "true", "false", "", "", "folder", "");
//			systemNodes.add(node);
//		}
		List<Menu> menus = menuManager.getAllEnabledStandardRootMenus();
		for(Menu menu :menus){
			ZTreeNode node = new ZTreeNode(menu.getSystemId().toString(),"all_system",menuManager.getNameToi18n(menu.getName()), "true", "false", "", "", "folder", "");
			systemNodes.add(node);
		}
