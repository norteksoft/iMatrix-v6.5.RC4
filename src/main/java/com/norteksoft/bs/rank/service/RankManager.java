...
public class RankManager {
	...
	public User getDirectLeader(Long userId, Long companyId) {
		...
		if(!dicts.isEmpty()){
			...
			if(!dicts.isEmpty()){
				leaderId = getLeaderIdFromDict(getLearderByDept(dicts,userId));
			}
		}
	...
	private List<Superior> getLearderByDept(List<Superior> dicts,Long userId){
		List<Superior> result = new ArrayList<Superior>();
		for(Superior superior:dicts){
			if(!userId.equals(superior.getUserId())){
				result.add(superior);
			}
		}
		return result;
