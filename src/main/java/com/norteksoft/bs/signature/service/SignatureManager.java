...
public class SignatureManager {
	...
	public void saveUploadPicture(Long signatureId) throws Exception {
		...
		ThreadParameters tp=new ThreadParameters(signature.getCompanyId());
		...
		if(oldPath!=null){
			File fileOld =new File(path+"/"+companyCode+oldPath);//删除老版本图片
			if(fileOld.exists())fileOld.delete();
			File old =new File(path+oldPath);
			if(old.exists())old.delete();
		}
		signature.setPictureSrc("/"+companyCode+"/"+time+fileType);
		save(signature);
	}
