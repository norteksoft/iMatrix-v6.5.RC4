...
public class Struts2Utils {
	 ...
	 * 
	 * @param name
	 * @param messages 替换占位符中的信息。带有占位符的key值不能有".",否则占位符不管用，国际化文件中配置如：deleteInfo=删除成功{0}个，删除失败{1}个
	 * @return
	 */
	public static String getText(String name,String[] messages){
		if(Struts2Utils.getRequest()==null){//定时等没有Struts环境中时，直接去数据库中取
			return ApiFactory.getSettingService().getInternationOptionValue(name);
		}else{
			if(textProvider != null && name != null){
				return textProvider.getText(name,messages);
			}
		}
		return name;
	}
	...
	public static String getText(String name,String language){
		...
		CustomLocaleProvider localeProvider = new CustomLocaleProvider();
		localeProvider.setLanguage(language);
		TextProvider textProvider = 
				new TextProviderFactory().createInstance(Struts2Utils.class, localeProvider);
		if(textProvider != null && name != null)
			return textProvider.getText(name);
		return name;
	}
	 ...
	 * @return
	 */
	public static String getText(String name,String language,String[] messages){
		if(EN_US.equals(language)){
			language = "en";
		}else if(ZH_CN.equals(language)){
			language = "zh";
	...
	public static void main(String[] args) {
		System.out.println(getText("reminder.message.info.task.bbb","zh_CN",new String[]{"1","2"}));
	}
}
