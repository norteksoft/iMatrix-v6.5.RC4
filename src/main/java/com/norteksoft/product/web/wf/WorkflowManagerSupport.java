...
public abstract class WorkflowManagerSupport<T extends FormFlowable> {
	...
	/**
	 * 减签。
	 * 该方法不适用于多分支机构，当分支之间存在同名用户时该方法会有问题；当一个用户有多条任务时该api有问题，会造成任务丢失；请使用<code>removeSigner(List<Long> taskIds);</code>
	 * @param taskId
	 * @param lists
	 */
	@Deprecated
	 ...
	 * @param taskId
	 * @param lists 用户id
	 *  @deprecated 当一个用户有多条任务时该api有问题，会造成任务丢失，请使用<code>removeSigner(List<Long> taskIds);</code>
	 */
	@Deprecated
	...
	public void removeSigners( List<Long> taskIds) {
		ApiFactory.getTaskService().removeSigners(taskIds);
	}
	...
	public String getCompleteTaskTipType(
		...
		case RETURN_URL:
			return t.getId().toString()+";"+completeTaskTipType.getContent()+";"+(completeTaskTipType.getTaskId()==null?"":completeTaskTipType.getTaskId());
	 ...
	 * @return 所有办理人节点
	 */
	public List<ZTreeNode> generateRemoveSingerTree(Long taskId){
		return WorkflowUtil.generateRemoveSingerTree(taskId);
	}
