...
public class ExcelExporter {
	...
	//此修改将导出的数据分批写入excel中不要等所有数据都查出来处理好了再统一插入excel中，为了防止内存溢出
	public static String export(Page<?> page,String listCode,String excelName,ExcelExportEnum excelEdition) throws Exception {
		if(excelEdition==ExcelExportEnum.EXCEL2007){
			MmsUtil mmsUtil = (MmsUtil) ContextUtils.getBean("mmsUtil");
			XSSFWorkbook wb = (XSSFWorkbook) mmsUtil.createReport(page,listCode,excelName,ExcelExportEnum.EXCEL2007);
			String filedName = setExcelName(excelName,ExcelExportEnum.EXCEL2007);
			return produceExcelToServers(wb,filedName);
		}else{
			return export(page,listCode,excelName); 
		}
	}
	//此修改将导出的数据分批写入excel中不要等所有数据都查出来处理好了再统一插入excel中，为了防止内存溢出
	public static String export(Page<?> page,String listCode,String excelName) throws Exception {
		MmsUtil mmsUtil = (MmsUtil) ContextUtils.getBean("mmsUtil");
		Workbook wb = mmsUtil.createReport(page,listCode,excelName,ExcelExportEnum.EXCEL2003);
		String filedName = setExcelName(excelName,ExcelExportEnum.EXCEL2003);
		return produceExcelToServers(wb,filedName);
	}
