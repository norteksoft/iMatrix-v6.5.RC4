...
public class DataImporterServiceImpl implements DataImporterService {
	...
	public String importData(File file, String fileName,DataImporterCallBack callBack) throws Exception{
		...
		if(StringUtils.isEmpty(result)){
			result=Struts2Utils.getText("menuManager.importSuccess");
		}
		return result;
	}
	...
	private String importTextData(File file, String fileName, DataImporterCallBack callBack) throws Exception{
		...
		if(fileName.lastIndexOf("_")==-1)
			//文件名不符合要求，正确格式为：导入定义编号_ 文件名.文件格式！
			return Struts2Utils.getText("dataImporterService.fileNameMistake");
			...
			if(importDefinition.getImportColumns() !=null && importDefinition.getImportColumns().size()>0){
				if(ImportType.TXT_DIVIDE.equals(importDefinition.getImportType())){
					if(StringUtils.isEmpty(importDefinition.getDivide())){
						result = Struts2Utils.getText("dataImporterService.matchNotFilled")+"\n";//字段信息中的 分隔符没有填写！\n
					}else{
	...
	private String importTxtData(File file, ImportDefinition importDefinition, DataImporterCallBack callBack) throws Exception{
		...
		int i=0;
		int headRow=0;
		if(importDefinition.getHeadRow()!=null){
			headRow=importDefinition.getHeadRow()-1;
		}
		...
		isr.close();
		fis.close();
		fileContent=resetTxtDataFileContent(fileContent,importDefinition);
		String result = "";
	...
	private String haveErrorNotImport(List<String> fileContent,ImportDefinition importDefinition, DataImporterCallBack callBack) {
		...
		int rowNum=getRowNum(importDefinition,fileContent.size());//文件行数
	...
	private String importTxtDivideData(File file,ImportDefinition importDefinition, DataImporterCallBack callBack) throws Exception{
		...
		fis.close();
		String result = "";
		fileContent=resetTxtDivideDataFileContent(fileContent,importDefinition);
		if(ImportWay.SUCCESS.equals(importDefinition.getImportWay())){//所有数据正确后导入
				...
				if(importDefinition != null){
					if(importDefinition.getImportColumns() !=null && importDefinition.getImportColumns().size()>0){
						...
						if("success".equals(sheetResult)){
							result+=Struts2Utils.getText("formManager.sheetName")+code+" "+ Struts2Utils.getText("formManager.listFileImportSuccess");
						}else{
							result+=sheetResult;
						}
					}else{
						result += Struts2Utils.getText("formManager.codeIs")+importDefinition.getCode()+" " + Struts2Utils.getText("formManager.listImportNoColumns");
					}
				}else{
					result += Struts2Utils.getText("formManager.listImportNoCode")+code+" " + Struts2Utils.getText("formManager.listImportDefinetion");
				}
			}
		}else{
			result=Struts2Utils.getText("formManager.listImportNoContent");
		}
		return result;
	}
	/**
	...
	private String resolvingExcelData(Sheet sheet, DataImporterCallBack callBack,ImportDefinition importDefinition) {
		...
		int[] row=getImportRowNumber(firstRowNum,lastRowNum,importDefinition);
		...
		if(list.size()>0){
			...
			result=Struts2Utils.getText("formManager.sheetName")+importDefinition.getCode()+ " " + Struts2Utils.getText("formManager.listImportNoContent");
		}
		return result;
	}
	...
	public static int[] getImportRowNumber(int startRowNum,int endRowNum,ImportDefinition importDefinition){
		int startRow=0;
		int endRow=0;
		if(importDefinition.getHeadRow()!=null){
			if(importDefinition.getStartRow()!=null){
				if(importDefinition.getStartRow()>=0){
					startRow=importDefinition.getHeadRow()+importDefinition.getStartRow()-1;
				}else{
					startRow=endRowNum+importDefinition.getStartRow()+1;
					if(startRow<importDefinition.getHeadRow())startRow=importDefinition.getHeadRow();
				}
			}else{
				startRow=importDefinition.getHeadRow();
			}
			if(importDefinition.getEndRow()!=null){
				if(importDefinition.getEndRow()>=0){
					endRow=importDefinition.getHeadRow()+importDefinition.getEndRow()-1;
					if(endRow>endRowNum)endRow=endRowNum;
				}else{
					endRow=endRowNum+importDefinition.getEndRow()+1;
				}
			}else{
				endRow=endRowNum;
			}
			...
			if(importDefinition.getStartRow()!=null){
				if(importDefinition.getStartRow()>=0){
					startRow=importDefinition.getStartRow();
				}else{
					startRow=endRowNum+importDefinition.getStartRow()+1;
			...
			if(importDefinition.getEndRow()!=null){
				if(importDefinition.getEndRow()>=0){
					endRow=importDefinition.getEndRow();
					if(endRow>endRowNum)endRow=endRowNum;
				}else{
					endRow=endRowNum+importDefinition.getEndRow()+1;
				}
		...
		int[] row={startRow,endRow};
		return row;
	}
	
	public static List<String[]> resetTxtDivideDataFileContent(List<String[]> fileContent,ImportDefinition importDefinition){
		List<String[]> content=new ArrayList<String[]>(); 
		int size=fileContent==null?0:fileContent.size();
		if(size>0){
			int startRow=0;
			int endRow=0;
			if(importDefinition.getStartRow()!=null){
				if(importDefinition.getStartRow()>=0){
					startRow=importDefinition.getStartRow()-1;
				}else{
					startRow=size+importDefinition.getStartRow();
			...
			if(importDefinition.getEndRow()!=null){
				if(importDefinition.getEndRow()>=0){
					...
					endRow=size+importDefinition.getEndRow()+1;
				}
			}else{
				endRow=size;
			}
			if(size==endRow && startRow==0){
				content=fileContent;
			}else{
				for(int i=startRow;i<endRow;i++){
					content.add(fileContent.get(i));
				}
			}
		}
		return content;
	}
	public static List<String> resetTxtDataFileContent(List<String> fileContent,ImportDefinition importDefinition){
		List<String> content=new ArrayList<String>(); 
		int size=fileContent==null?0:fileContent.size();
		if(size>0){
			int startRow=0;
	...
	public static int getRowNum(ImportDefinition importDefinition,int fileSize){
		int rowNum=1;
		if(importDefinition.getStartRow()!=null){
			if(importDefinition.getStartRow()>=0){
				rowNum=importDefinition.getStartRow();
			}else{
				rowNum=fileSize+importDefinition.getStartRow();
			}
		}
		return rowNum;
	}
