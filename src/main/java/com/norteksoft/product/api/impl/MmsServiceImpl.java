...
import com.norteksoft.product.api.entity.ListColumn;
...
public class MmsServiceImpl implements MmsService {
	...
	public String getAuthorizeUrl(String code, String systemCode) {
		return menuManager.getAuthorizeUrl(code,systemCode);
	}

	public String getAuthorizeUrl(String code) {
		return getAuthorizeUrl(code,ContextUtils.getSystemCode());
	}
	@Deprecated
	public List<ListView> getListViewPageByMenu(Long menuId,
			String dataTableName) {
		return BeanUtil.turnToModelListViewList(listViewManager.getListViewPageByMenu(menuId,dataTableName));
	}

	public List<ListView> getListViewsByDataTable(String dataTableName) {
		return BeanUtil.turnToModelListViewList(listViewManager.getListViewByTabelName(dataTableName));
	}

	public List<ListColumn> getListColumnsByViewCode(String listViewCode) {
		List<com.norteksoft.mms.form.entity.ListColumn> columns = listColumnManager.getColumnsByViewCode(listViewCode);
		List<ListColumn> result = new ArrayList<ListColumn>();
		for(com.norteksoft.mms.form.entity.ListColumn col:columns){
			ListColumn lcol = new ListColumn();
			lcol.setHeaderName(col.getHeaderName());
			lcol.setVisible(col.getVisible());
			TableColumn tcol = col.getTableColumn();
			if(tcol!=null){
				lcol.setColumnName(tcol.getName());
				lcol.setDbName(tcol.getDbColumnName());
				lcol.setDataType(tcol.getDataType());
			}
			result.add(lcol);
		}
		return result;
