...
//	private BasicTypeManager basicTypeManager;
	...
	public void startInstance(String workflowDefinitionCode, FormFlowable entity){
		...
		//保存意见控件的内容
		Long taskId = entity.getWorkflowInfo().getFirstTaskId();
		saveOpinionControl(taskId);
...
//		entity.setExtendField(extendField);
	...
	public void deleteInstance(String workflowId){
		...
		if(workflowInstanceManager.isInstanceInHistory(workflowId)){//是历史表中实例
			//FIXME 由于定时中归档可能出现并发,引起一个实例id对应多条实例
			List<com.norteksoft.wf.engine.entity.HistoryWorkflowInstance> wfis = historyWorkflowInstanceManager.getHistoryWorkflowInstances(workflowId);
			for(com.norteksoft.wf.engine.entity.HistoryWorkflowInstance wfi:wfis){
	...
	public void saveOpinion(com.norteksoft.product.api.entity.Opinion opinionp){
		...
		String workflowId = opinionp.getWorkflowId();
		if(opinionp.getId()==null){
			WorkflowTask task = this.getTask(opinionp.getTaskId());
			if(task!=null)workflowId = task.getProcessInstanceId();
		}
		if(workflowId!=null){
			instanceInHistory = workflowInstanceManager.isInstanceInHistory(workflowId);
			if(instanceInHistory){
				saveHistoryOpinion(opinionp);
			}else{
				saveCurrentOpinion(opinionp);
			}
		}
	}
	 ...
	 * @param opinion
	 */
	public void saveCurrentOpinion(com.norteksoft.product.api.entity.Opinion opinionp){
		boolean instanceInHistory = false;
		...
		opinion.setControlId(opinionp.getControlId());
		opinion.setOpinionSign(opinionp.getOpinionSign());
		opinion.setOpinionDate(opinionp.getOpinionDate());
		opinion.setConclusion(opinionp.getConclusion());
	 ...
	 * @param opinion
	 */
	public void saveHistoryOpinion(com.norteksoft.product.api.entity.Opinion opinionp){
		HistoryOpinion opinion=null;
		if(opinionp.getId()==null){
			opinion=new HistoryOpinion();
			opinion.setCreatedTime(new Date());
		}else{
			opinion = historyWorkflowInstanceManager.getOpinionsById(opinionp.getId());
		}
		opinion.setCustomField(opinionp.getCustomField());
		opinion.setOpinion(opinionp.getOpinion());
		opinion.setControlId(opinionp.getControlId());
		...
		historyWorkflowInstanceManager.saveOpinion(opinion);
	}
...
//		docment.setFileBody(file.getContent());
	...
	public List<com.norteksoft.product.api.entity.Document> getDocuments(FormFlowable formFlowable) {
		...
		return new ArrayList<com.norteksoft.product.api.entity.Document>();
	}
...
//----正文权限 end-- --	
	...
	public String fillEntityByDefinition(FormFlowable entity,String wfDefinationCode){
		WorkflowDefinition definition=getEnabledHighestVersionDefinition(wfDefinationCode);
		if(definition!=null){
			return autoFilledEntityBeforeByDefinitionId(entity, definition.getId(),null);
		}
		return "";
	}
	
	public String fillEntityByDefinition(FormFlowable entity,String wfDefinationCode, Integer version){
		return autoFilledEntityBeforeByDefinitionId(entity, getWorkflowDefinitionByCodeAndVersion(wfDefinationCode, version).getId(),null);
	}
	...
	public String fillEntityByTask(FormFlowable entity,Long taskId){
		Assert.notNull(entity, "FormFlowable实体不能为null");
		WorkflowTask task = ApiFactory.getTaskService().getTask(taskId);
		if(task==null){
			log.debug("任务不能为null");
		...
		Map<String,Object> valueMap = new HashMap<String,Object>();
 		...
 		for(AutomaticallyFilledField aff :filledField){
 			if(aff.getName().contains("~~")){//表示是意见控件的自动填写
				getAutomaticallyFilledOpinionControl(aff,result,task.getId());
			}else{
	...
	public String fillEntityByTask(FormFlowable entity,Long taskId,Map data){
		WorkflowTask task = ApiFactory.getTaskService().getTask(taskId);
		if(task==null){
			log.debug("任务不能为null");
			throw new RuntimeException("任务不能为null");
	 ...
	 * @param taskId 任务id 如果该id为空，将填充第一个环节的值
	 */
	public String autoFilledEntityBeforeByDefinitionId(FormFlowable entity,Long definitionId,Map data){
		log.debug(LOGMESSAGE_METHOD_PARAMETER + "FormFlowable entity" + entity);
		WorkflowDefinition definition = workflowDefinitionManager.getWfDefinition(definitionId);
		if(definition==null){
			log.debug("WorkflowDefinition实体不能为null");
		...
		taskName = DefinitionXmlParse.getFirstTaskName(definition.getProcessId());
		Object value;
		StringBuilder result = new StringBuilder();//用于存放意见控件办理前填写的字段信息
		List<AutomaticallyFilledField> filledField = DefinitionXmlParse.getBeforeFilledFields(definition.getProcessId(),taskName);
		log.debug("filledField:" + filledField);
		if(entity!=null){
			Map<String,Object> valueMap = new HashMap<String,Object>();
			try {
				for(AutomaticallyFilledField aff :filledField){
					if(aff.getName().contains("~~")){//表示是意见控件的自动填写
						getAutomaticallyFilledOpinionControl(aff,result,null);
					}else{
						value = getAutoFilledFieldValue(formManager.getControls(form.getId()),aff,entity,null);
						log.debug("value:" + value);
						valueMap.put(aff.getName(), value);
	...
	private String getResult(StringBuilder result){
		if(result.indexOf("controlId")>0){//表示不是空
	...
	private void getAutomaticallyFilledOpinionControl(AutomaticallyFilledField aff,StringBuilder result,Long taskId){
		String fieldName = aff.getName();
		Object value=null;
		String controlId = fieldName.substring(0,fieldName.indexOf("~~"));
		String name=null;
		String fillType=AutomaticallyFilledField.AUTO_FILLED_FILL_TYPE_COVERAGE;
		if(fieldName.contains("~~opinion")){//意见控件的意见框
			name="opinion";
			value = taskService.getValue(aff, null);
			fillType=aff.getFillType();
		}else if(fieldName.contains("~~sign")){
			name="sign";
			if(CommonStrings.CURRENT_TRANSACTOR_NAME.equals(aff.getValue())){
				value =ContextUtils.getUserName();
			}else{
				value =aff.getValue();
			}
			...
			for(int i=0;i<strs.length;i++){
				if(strs[i].contains(TaskService.CONCLUSION_VAL_SEPERATE)){
					conclusion.append(strs[i].substring(0,strs[i].indexOf(TaskService.CONCLUSION_VAL_SEPERATE))).append(",");
				}
			}
			if(conclusion.length()>0&&conclusion.lastIndexOf(",")==conclusion.length()-1){
				value = conclusion.substring(0,conclusion.lastIndexOf(","));
			}
		}
		StringBuilder affResult = new StringBuilder();
		affResult.append("{\"controlId\":\"").append(controlId).append("\",")
		.append("\"name\":\"").append(name).append("\",")
		.append("\"fillType\":\"").append(fillType).append("\",")
		.append("\"separate\":\"").append(aff.getSeparate()==null?"":aff.getSeparate()).append("\",")
		.append("\"taskId\":\"").append(taskId!=null?taskId:"").append("\",")
		.append("\"value\":\"").append(value!=null?value.toString():"").append("\"}");
		result.append(affResult).append(",");
	}
	...
	public CompleteTaskTipType selectActivity(Long taskId, String transitionName){
		...
		CompleteTaskTipType type = taskService.completeTacheChoice(taskId, transitionName);
		saveOpinionControl(taskId);
		return type;
	}
	...
	public CompleteTaskTipType submitInstance(String definitionCode,
		...
		log.debug("workflowId" + workflowId);
		CompleteTaskTipType type = submitWorkflowById(workflowId,entity);
		return type;
	}
	...
	public CompleteTaskTipType completeWorkflowTask(Long taskId,
		...
		CompleteTaskTipType type = completeWorkflowTask(taskId, result, null);
		return type;
	}
	public CompleteTaskTipType completeWorkflowTask(WorkflowTask task,
			TaskProcessingResult result) {
		CompleteTaskTipType type = completeWorkflowTask(task,result,null);
		return type;
	}
	...
	public String fillEntityByDefinition(Map data, String wfDefinationCode,Long... systemId) {
		Long sysId=ContextUtils.getSystemId();
		if(systemId.length>0){
			sysId=systemId[0];
		}
		return autoFilledEntityBeforeByDefinitionId(null, getEnabledHighestVersionDefinitionBySystem(wfDefinationCode,sysId).getId(),data);
	}

	public String fillEntityByDefinition(Map data, String wfDefinationCode,
			Integer version,Long... systemId) {
		Long sysId=ContextUtils.getSystemId();
		if(systemId.length>0){
			sysId=systemId[0];
		}
		return autoFilledEntityBeforeByDefinitionId(null, getWorkflowDefinitionByCodeAndVersion(wfDefinationCode, version,sysId).getId(),data);
	}

	public String fillEntityByTask(Map data, Long taskId) {
		return fillEntityByTask(null, taskId,data);
	}
	...
	public Map startCustomInstance(Long definitionId) {
		...
		WorkflowDefinition definition = this.getWorkflowDefinitionById(definitionId);
		Assert.notNull(definition,"definition不可为空");
		Map result = startMyCustomInstance(definition.getProcessId());
		//保存意见控件的值
		String instanceId = (String)result.get("instanceId");
		com.norteksoft.wf.engine.entity.WorkflowInstance workflow = workflowInstanceManager.getWorkflowInstance(instanceId);
		saveOpinionControl(workflow.getFirstTaskId());
		return result;
	}
	...
	public void removeSigners(List<Long> taskIds) {
		//生成减签流转历史
		taskService.generateRemoveHistory(taskIds);
		taskService.deleteSignerHandlerByTaskId(taskIds);
	}
	
	/**
	 * 根据实例Id和意见控件id获得意见
	 */
	public com.norteksoft.product.api.entity.Opinion getOpinion(String instanceId,String controlId,Long companyId){
		Opinion op = workflowInstanceManager.getOpinion(instanceId, controlId, companyId);
		if(op != null){
			return BeanUtil.turnToModelOpinion(op);
		}else{
			return BeanUtil.turnHistoryToModelOpinion(workflowInstanceManager.getHistoryOpinion(instanceId, controlId, companyId));
		}
	}
	 ...
	 */
	public String getOpinionConclusion(String instanceId, String controlId){
		Long companyId = ContextUtils.getCompanyId();
		String conclusion = workflowInstanceManager.getOpinionConclusion(instanceId, controlId, companyId);
		if(StringUtils.isNotEmpty(conclusion)){
			return conclusion;
		}else{
			return workflowInstanceManager.getHistoryOpinionConclusion(instanceId, controlId, companyId);
		}
	}

	public void saveOpinionControl(Long taskId) {
		taskService.saveOpinionControls(taskId);
	}

	public List<WorkflowInstance> getSubProcessInstanceByTaskName(String parentWorkflowId, String tacheName) {
		return workflowInstanceManager.getSubProcessInstanceByTaskName(parentWorkflowId,tacheName);
	}
	...
	public List<WorkflowTask> getCompleteTasks(Long companyId, String closeSign) {
		return BeanUtil.turnToModelTaskList(taskService.getCompleteTasks(companyId,closeSign));
	}
	...
	public List<WorkflowTask> getDidNotDealWithTasks(Long companyId,String pushSign) {
		return BeanUtil.turnToModelTaskList(taskService.getDidNotDealWithTasks(companyId, pushSign));
	}

	public void enableDefinition(Long definitionId){
		try{
			workflowDefinitionManager.enableDefinition(definitionId);
		}catch (Exception e) {
	...
	public void enableDefinition(String workflowDefinitionCode,
		...
		com.norteksoft.product.api.entity.WorkflowDefinition def = getWorkflowDefinitionByCodeAndVersion(workflowDefinitionCode, workflowDefinitionVersion);
		if(def!=null)enableDefinition(def.getId());
		
	}

	public void enableDefinition(String workflowDefinitionCode) {
		com.norteksoft.product.api.entity.WorkflowDefinition def = getEnabledHighestVersionWorkflowDefinition(workflowDefinitionCode);
		if(def!=null)enableDefinition(def.getId());
	}
	...
	public void deleteDefinition(Long definitionId) {
		...
		List<com.norteksoft.wf.engine.entity.WorkflowInstance> instances = workflowInstanceManager.getInstancesByDefId(definitionId);
		for(com.norteksoft.wf.engine.entity.WorkflowInstance instance : instances){
			workflowInstanceManager.deleteWorkflowInstance(instance,false);
		}
		...
		List<HistoryWorkflowInstance> historyInstances = historyWorkflowInstanceManager.getInstancesByDefId(definitionId);
		for(HistoryWorkflowInstance instance : historyInstances){
			workflowInstanceManager.deleteWorkflowInstanceHistory(instance,false);
		}
		workflowDefinitionManager.deleteWfDefinition(definitionId);
	}

	public void deleteDefinition(String workflowDefinitionCode,
			Integer workflowDefinitionVersion) {
		com.norteksoft.product.api.entity.WorkflowDefinition def = getWorkflowDefinitionByCodeAndVersion(workflowDefinitionCode, workflowDefinitionVersion);
		if(def!=null)deleteDefinition(def.getId());
	...
	public Long cloneDefinition(Long definitionId,String definitionName,String definitionCode) {
		return workflowDefinitionManager.cloneDefinition(definitionId, definitionName, definitionCode);
	}

	public List<String> getTaskNames(Long workflowDefinitionId) {
		return taskService.getTaskNames(workflowDefinitionId);
	}

	public List<String> getTaskCodes(Long workflowDefinitionId) {
		return taskService.getTaskCodes(workflowDefinitionId);
	}
}
