...
public class DelegateMainManager {
	...
	public String startDelegateMain(Long id){
		...
		switch(delegateMain.getState()){
			case STARTED:
				return errorMessage(Struts2Utils.getText("wf.engine.delegate.delegateIsEnabled"));
			case EFFICIENT:
				return errorMessage(Struts2Utils.getText("wf.engine.delegate.delegateEffect"));
			case END:
				return errorMessage(Struts2Utils.getText("wf.engine.delegate.commissionOver"));
			default:
	...
	private String startDelegateMain(TrustRecord delegateMain){
		...
		cal.set(Calendar.MINUTE, 0);
		cal.set(Calendar.SECOND, 0);
		if(delegateMain.getEndTime().compareTo(cal.getTime())<0) return errorMessage(Struts2Utils.getText("wf.engine.delegate.theCurrentTime"));
		if(delegateMain.getStyle()==3){
			...
			if(result!=null){
				return errorMessage(message.append(Struts2Utils.getText("wf.engine.delegate.from")).append(SIMPLEDATEFORMAT.format(result.getBeginTime()))
					.append(Struts2Utils.getText("wf.engine.delegate.to")).append(SIMPLEDATEFORMAT.format(result.getEndTime())).append(Struts2Utils.getText("wf.engine.delegate.theTimeMemory")).toString()); 
			}
			//验证流程委托
			...
			if(results.size()>0){
				...
				if(obj!=null){
					...
					return errorMessage(message.append(Struts2Utils.getText("wf.engine.delegate.from")).append(SIMPLEDATEFORMAT.format(result.getBeginTime()))
							.append(Struts2Utils.getText("wf.engine.delegate.to")).append(SIMPLEDATEFORMAT.format(result.getEndTime()))
							.append(Struts2Utils.getText("wf.engine.delegate.timeMemory")).append(results.get("activityName")).append(Struts2Utils.getText("wf.engine.delegate.fullPower")).toString());
				}
			}
		...
		return successMessage(Struts2Utils.getText("wf.engine.delegate.delegateEnabled"));
	...
	public String endDelegateMain(Long id) {
		...
		switch(delegateMain.getState()){
			case NEW_CREATING:
				return errorMessage(Struts2Utils.getText("wf.engine.delegate.delegateNotEnabled"));
			case CANCEL:
				return errorMessage(Struts2Utils.getText("wf.engine.delegate.delegateSuspended"));
			case END:
				return errorMessage(Struts2Utils.getText("wf.engine.delegate.commissionOver"));
			default:
	...
	private String endDelegateMain(TrustRecord delegateMain) {
		...
		return successMessage(Struts2Utils.getText("wf.theDelegateSuccessfullySuspended"));
	}
