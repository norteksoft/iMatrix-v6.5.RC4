...
public class WorkflowDefinitionManager {
     ...
     */
    @Transactional(readOnly=false)
    public Long createWfDefinition(Long companyId, String xmlFile,String typeCode,String systemCode,String saveUrl){
    	...
    	Long wfdId =  saveWorkflowDefinition(null, companyId, xmlFile,type.getId(),system.getId());
    	//保存时回调事件处理
    	saveDefinitionCallback(saveUrl,wfdId);
    	return wfdId;
    }
    /**
     * 保存时回调事件处理
     */
    private void saveDefinitionCallback(String saveUrl,Long wfdId ){
    	if(StringUtils.isNotEmpty(saveUrl)){
    		if(saveUrl.contains("~~")){//问号
    		...
    		String joinSign = StringUtils.contains(saveUrl, "?") ? "&" : "?";
    		saveUrl = saveUrl+joinSign+"definitionId="+wfdId;
    		PropUtils.httpClent(saveUrl);
    	}
    }
    /**
     ...
     */
    @Transactional(readOnly=false)
    public Long updateWfDefinition(Long id, Long companyId, String xmlFile,String typeCode,String systemCode,String saveUrl){
    	ThreadParameters params=new ThreadParameters(companyId);
    	ParameterUtils.setParameters(params);
    	WorkflowType type=workflowTypeManager.getWorkflowType(typeCode);
    	BusinessSystem system=businessSystemManager.getSystemBySystemCode(systemCode);
    	...
    	Long wfdId = saveWorkflowDefinition(id, companyId, xmlFile,type.getId(),system.getId());
    	//保存时回调事件处理
    	saveDefinitionCallback(saveUrl,wfdId);
    	return wfdId;
    }
    ...
    public Long saveWorkflowDefinition(Long id, Long companyId, String xmlFile,Long type,Long systemId){
    	...
    	if(id == null){
    		id = createWorkflowDefinition(companyId, xmlFile, type);
    	}else{
    		id = updateWorkflowDefinition(id, companyId, xmlFile, type);
    	}
     ...
     */
    @Transactional(readOnly=false)
    private Long createWorkflowDefinition(Long companyId, String xmlFile,Long type){
    	log.debug("*** createWorkflowDefinition 方法开始");
    	
    	Map<String, String> props = DefinitionXmlParse.getProcessBaseInfo(xmlFile);
    	//保存流程定义信息
    	WorkflowDefinition wfd = createWorkflowDefinition(companyId, props, type);
		workflowDefinitionDao.save(wfd);
		//保存流程定义文件
    ...
    @Transactional(readOnly=false)
    private WorkflowDefinition createWorkflowDefinition(Long companyId, Map<String, String> props,Long type){
    	...
    	wfd.setVersion(workflowDefinitionDao.generateWorkflowDefinitionVersion(
    			props.get(ProcessProperties.WF_CODE.toString()),companyId));
    	return wfd;
    }
     ...
     */
    @Transactional(readOnly=false)
    private Long updateWorkflowDefinition(Long id, Long companyId, String xmlFile,Long type){
		...
		if(wfd.getEnable()==DataState.DRAFT){
        	...
        	wfd = createWorkflowDefinition(companyId, props, type);
...
//    	张三/北科汇智[zhangsan~~nortek]
    ...
    public String deployProcess(Long wfdId) throws UnsupportedEncodingException{
    	...
    	if(wfd.getEnable()==DataState.ENABLE){
    		...
    		message = Struts2Utils.getText("view.state.enable") + " -> " + Struts2Utils.getText("view.state.disable");
    	}else{
    		message = enableDefinition(wfd);
    	}
     ...
     * @param wfd
     */
    public String enableDefinition(WorkflowDefinition wfd){
    	String message = "";
    	 if(wfd.getEnable()==DataState.DISABLE){
     		log.debug("*** 禁用 -> 启用");
     		//disableOtherProcess(wfd);
     		wfd.setEnable(DataState.ENABLE);
     		message =  Struts2Utils.getText("view.state.disable") + " -> " + Struts2Utils.getText("view.state.enable");
     	}else if(wfd.getEnable()==DataState.DRAFT){
     		if(ProcessType.PREDEFINED_PROCESS.equals(wfd.getProcessType())){
     			log.debug("*** 草稿 -> 启用");
     			//如果是草稿状态，部署流程并设置为启用
     			WorkflowDefinitionFile file = workflowDefinitionFileDao.getWfDefinitionFileByWfdId(wfd.getId());
     			log.debug("file:"+file);
     			try {
...
//				MemCachedUtils.add("document="+wfd.getProcessId(), Dom4jUtils.getDocument(file.getDocument()));
     		...
     		message = Struts2Utils.getText("view.state.draft") + " -> " + Struts2Utils.getText("view.state.enable");;
     	}
    	 return message;
    }
    ...
    public void enableDefinition(Long definitionId){
    	WorkflowDefinition wfd = workflowDefinitionDao.get(definitionId);
    	enableDefinition(wfd);
    }
    ...
    public WorkflowDefinition getLatestVersion(String name, Long companyId){
    	return workflowDefinitionDao.getLatestVersion(name, companyId);
    }
    ...
    public WorkflowDefinition getEnabledHighestVersionWorkflowDefinition(String code){
    	...
    	return workflowDefinitionDao.getEnabledHighestVersionWorkflowDefinition(code, getCompanyId());
    }
    /**
     * 获得最新版本的流程定义
     * @param name
    ...
    public WorkflowDefinition getEnabledHighestVersionWorkflowDefinition(String code,Long companyId){
    	return workflowDefinitionDao.getEnabledHighestVersionWorkflowDefinition(code, companyId);
    }
	...
	public Integer generateWorkflowDefinitionVersion(String name) {
		return workflowDefinitionDao.generateWorkflowDefinitionVersion(name, getCompanyId());
	}
...
//		}
	...
	public void deleteWfDefinition(Long definitionId){
		workflowDefinitionFileDao.deleteDefinitionFileByWfdId(definitionId);
		workflowDefinitionDao.delete(definitionId);
	}
	/**
	 * 拷贝流程定义并修改流程名称和编码
	 * @param definitionId
	...
	public Long cloneDefinition(Long definitionId,String definitionName,String definitionCode){
		WorkflowDefinition definition = getWfDefinition(definitionId);
		Long defId=null;
		if(definition!=null){
			WorkflowDefinition def = definition.clone();
			def.setId(null);
			def.setName(definitionName);
			def.setCode(definitionCode);
			def.setEnable(DataState.DRAFT);
			def.setCreatedTime(new Date());
			def.setCreator(ContextUtils.getLoginName());
			def.setCreatorName(ContextUtils.getUserName());
			def.setCreatorId(ContextUtils.getUserId());
			def.setSubCompanyId(ContextUtils.getSubCompanyId());
			def.setSubCompanyName(ContextUtils.getSubCompanyName());
			def.setDepartmentId(ContextUtils.getDepartmentId());
			workflowDefinitionDao.save(def);
			defId = def.getId();
			try{
				WorkflowDefinitionFile defFile=workflowDefinitionFileDao.getWfDefinitionFileByWfdId(definition.getId());
				if(defFile!=null){
					WorkflowDefinitionFile dFile = new WorkflowDefinitionFile();
					dFile.setWfDefinitionId(def.getId());
					dFile.setDocument(defFile.getDocument());
					dFile.setCompanyId(defFile.getCompanyId());
					dFile.setCreatorId(defFile.getCreatorId());
					workflowDefinitionFileDao.save(dFile);
					
					Document document=Dom4jUtils.getDocument(dFile.getDocument());
					Element root = document.getRootElement();
					...
					while(it.hasNext()){//只会循环一次
						Element element = (Element)it.next();
						Attribute attr=element.attribute("name");
						attr.setValue(definitionName);
//						element.addAttribute("name", definitionName);
					}
					
					propElement = Dom4jUtils.getSubElementByName(basePropElement, "process-code");
					propElement.setText(definitionCode);
					
					List<Department> branches = ApiFactory.getAcsService().getBranchs();
					if(branches.size()>0&&def.getSubCompanyName()!=null){
						propElement = Dom4jUtils.getSubElementByName(basePropElement, "branch-name");
						if(propElement!=null){
							propElement.setText(def.getSubCompanyName());
						}else{
							Element element = basePropElement.addElement("branch-name");
							element.addText(def.getSubCompanyName());
						}
						
						propElement = Dom4jUtils.getSubElementByName(basePropElement, "branch-code");
						if(propElement!=null){
							propElement.setText(ContextUtils.getSubCompanyCode());
						}else{
							Element element = basePropElement.addElement("branch-code");
							element.addText(ContextUtils.getSubCompanyCode());
						}
					}
					
					dFile.setDocument(document.asXML());
					workflowDefinitionFileDao.save(dFile);
				}
			}catch (Exception e) {
				log.debug(PropUtils.getExceptionInfo(e));
