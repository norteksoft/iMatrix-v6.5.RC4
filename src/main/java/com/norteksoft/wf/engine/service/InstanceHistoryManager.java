...
public class InstanceHistoryManager {
	 ...
	 * @return 	String[][]: [环节名称，办理人，操作，办理意见];自动环节没有办理人
	 */
	public List<WfHistoryModelFlex> getHistoryByInstanceIdFlex(Long companyId, String instanceId){
		boolean isInstanceComplete = workflowInstanceManager.isInstanceInHistory(instanceId,companyId);
		if(isInstanceComplete){//实例在历史实例表中
			return getHistoryHistoryByInstanceIdFlex(companyId,instanceId);
		}else{//实例在当前实例表中
			return getCurrentHistoryByInstanceIdFlex(companyId,instanceId);
		}
	}
	 ...
	 * @return 	String[][]: [环节名称，办理人，操作，办理意见];自动环节没有办理人
	 */
	public List<WfHistoryModelFlex> getCurrentHistoryByInstanceIdFlex(Long companyId, String instanceId){
		List<Object[]> specialHistorieAndOpinions = instanceHistoryDao.getHistoryBySpecial(companyId, instanceId, true);
		List<Object[]> historieAndOpinions = instanceHistoryDao.getHistoryBySpecial(companyId, instanceId, false);
		...
		List<WfHistoryModelFlex> wfHistoryModelFlexs = new ArrayList<WfHistoryModelFlex>();
		for(int i = 0; i < specialHistories.size(); i++){
			WfHistoryModelFlex wfHistoryModelFlex = new WfHistoryModelFlex();
			wfHistoryModelFlex.setTaskName(specialHistories.get(i).getTaskName());
			wfHistoryModelFlex.setTransactor(specialHistories.get(i).getTransactor());
			wfHistoryModelFlex.setTransactionResult(getHistoryI18nTransactionResult(specialHistories.get(i).getTransactionResult()));
			
			wfHistoryModelFlex.setTransactorOpinion(specialHistories.get(i).getTransactorOpinion());
			wfHistoryModelFlex.setSpecialTask(specialHistories.get(i).getSpecialTask()+"");
			wfHistoryModelFlexs.add(wfHistoryModelFlex);
		}
		for(int i = 0; i < histories.size(); i++){
			WfHistoryModelFlex wfHistoryModelFlex = new WfHistoryModelFlex();
			wfHistoryModelFlex.setTaskName(histories.get(i).getTaskName());
			wfHistoryModelFlex.setTransactor(histories.get(i).getTransactor());
			wfHistoryModelFlex.setTransactionResult(getHistoryI18nTransactionResult(histories.get(i).getTransactionResult()));
			wfHistoryModelFlex.setTransactorOpinion(histories.get(i).getTransactorOpinion());
			wfHistoryModelFlex.setSpecialTask(histories.get(i).getSpecialTask()+"");
			wfHistoryModelFlexs.add(wfHistoryModelFlex);
		}
		return wfHistoryModelFlexs;
	}
	
	private String getHistoryI18nTransactionResult(String result){
		if(result.contains("[")){
		...
		return workflowInstanceManager.getI18nTransactionResult(result);
	}
	 ...
	 * @return 	String[][]: [环节名称，办理人，操作，办理意见];自动环节没有办理人
	 */
	public List<WfHistoryModelFlex> getHistoryHistoryByInstanceIdFlex(Long companyId, String instanceId){
		List<Object[]> specialHistorieAndOpinions = historyInstanceHistoryDao.getHistoryBySpecial(companyId, instanceId, true);
		List<Object[]> historieAndOpinions = historyInstanceHistoryDao.getHistoryBySpecial(companyId, instanceId, false);
	...
	private List<InstanceHistory> getHistoryFromHistoryAndOpinion(List<Object[]> histories){
		...
		for(int i = 0; i < histories.size(); i++){
			...
			if(opinion != null && lastTaskId != null && lastTaskId.equals(opinion.getTaskId())){
				if(StringUtils.isEmpty(opinion.getControlId())){//该行代码是为了解决，用意见控件和意见标签填写意见时，不在"流转历史/列表视图"中的"办理意见"中显示
					if(ih.getTransactorOpinion() != null){
						if(opinion.getOpinion() != null){
							...
							ih.setTransactorOpinion(ih.getTransactorOpinion());
						}
					}else{
				...
				if(opinion != null&&StringUtils.isEmpty(opinion.getControlId())) ih.setTransactorOpinion(opinion.getOpinion());
	 ...
	 * @return List 当前环节
	 */
	public List<CurrentTasksModelFlex> getCurrentTasksFlex(Long companyId, String instanceId){
		...
		List<WorkflowTask> tasks = workflowTaskService.getActivityTasks(instanceId, companyId);
		processTasks(tasks, transactors, transactorCandidates,specials,companyId);
		List<CurrentTasksModelFlex> result = new ArrayList<CurrentTasksModelFlex>();
		
		CurrentTasksModelFlex currentTasksModelFlex = null;
		String resultTransactors = "";
		for(Map.Entry<String, StringBuilder> taskTransactorPair : transactors.entrySet()){
			currentTasksModelFlex = new CurrentTasksModelFlex();
			resultTransactors = taskTransactorPair.getValue().toString();
			if(!resultTransactors.endsWith(",")){
				resultTransactors = transactorCandidates.get(taskTransactorPair.getKey()).toString();
			}
			currentTasksModelFlex.setKey(taskTransactorPair.getKey());
			if(resultTransactors.length()>0){
				currentTasksModelFlex.setResultTransactors(resultTransactors.substring(0, resultTransactors.length()-1));
			}else{
				currentTasksModelFlex.setResultTransactors("");
			}
			currentTasksModelFlex.setSpecials(specials.get(taskTransactorPair.getKey()).toString());
			result.add(currentTasksModelFlex);
		}
