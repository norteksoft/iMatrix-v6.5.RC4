...
public class WorkflowDefinitionDao extends HibernateDao<WorkflowDefinition, Long>{
	 ...
	 * @return 最新版本的流程定义
	 */
	public WorkflowDefinition getLatestVersion(String code, Long companyId){
		 List<WorkflowDefinition> wds = find("from WorkflowDefinition wfd where wfd.companyId = ? and wfd.code=? order by wfd.version desc",companyId, code);
		 if(wds.isEmpty()){
			 return null;
		 }else{
			 return wds.get(0);
	...
	public WorkflowDefinition getEnabledHighestVersionWorkflowDefinition(String code, Long companyId){
		 List<WorkflowDefinition> wds = find("from WorkflowDefinition wfd where wfd.companyId = ? and wfd.code=? and wfd.enable=0 order by wfd.version desc", 
				 companyId, code);
		 if(wds.isEmpty()){
			 return null;
		 }else{
	 ...
	 * @return 新的版本号
	 */
	public int generateWorkflowDefinitionVersion(String code, Long companyId){
		WorkflowDefinition wd = getLatestVersion(code,companyId);
		if(wd==null){
			return 1;
		}else{
