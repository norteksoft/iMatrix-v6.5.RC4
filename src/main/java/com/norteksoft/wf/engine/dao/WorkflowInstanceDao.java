...
public class WorkflowInstanceDao extends HibernateDao<WorkflowInstance, Long>{
	...
	public Object[] getParentInstanceInfoByJbpmInstanceId(String jbpmInstanceId, Long companyId){
		return findUnique("select wfi.processState,wfi.parentProcessId from WorkflowInstance wfi where wfi.companyId=? and wfi.processInstanceId=? and wfi.parentProcessId is not null", companyId, jbpmInstanceId);
	}
	public Object[] getInstanceInfoByJbpmInstanceId(String jbpmInstanceId, Long companyId){
		return findUnique("select wfi.processState,wfi.parentProcessId from WorkflowInstance wfi where wfi.companyId=? and wfi.processInstanceId=?", companyId, jbpmInstanceId);
	}
	//删除正文
	 ...
	 * @return
	 */
	public void getEndAndCancelProcess(Page<WorkflowInstance> wiPages){
		 findPage(wiPages, "from WorkflowInstance wi where wi.companyId = ? and (wi.processState=? or wi.processState=?) ", ContextUtils.getCompanyId(),ProcessState.END,ProcessState.MANUAL_END);
	}
	public  List<WorkflowInstance> getInstancesByDefId(Long workflowDefinitionId) {
		String hql = "from WorkflowInstance wi where wi.workflowDefinitionId = ?";
