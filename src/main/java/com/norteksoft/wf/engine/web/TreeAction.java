...
public class TreeAction extends CrudActionSupport {
	...
	public String myProcess() throws Exception{
		...
		if("INITIALIZED".equals(currentId)){
			...
			tree.append(JsTreeUtils.generateJsTreeNodeDefault("ING", "open", 
					Struts2Utils.getText("workflow.doing")+"("+getInstanceNumByEnable(false)+")", subNodes.toString())).append(",");
			//已完成
			...
			tree.append(JsTreeUtils.generateJsTreeNodeDefault("END", "", 
					Struts2Utils.getText("workflow.complete")+"("+getInstanceNumByEnable(true)+")", subNodes.toString())).append(",");
	...
	public String delegateMonitor() throws Exception{
		...
		String name = Struts2Utils.getText("workflow.doing")+ "(" + 
		...
		name = Struts2Utils.getText("workflow.complete")+ "(" + 
	...
	public String transferTaskTree() throws Exception{
		...
		ZTreeNode root = new ZTreeNode("TRANSFER_ING","0",name, "false", "false", "", "", "folder", "");
		treeNodes.add(root);
...
//				Struts2Utils.getText("workflow.doing") + "(" + 
...
//				Struts2Utils.getText("workflow.complete") + "(" + 
//				taskService.getTrusteeTasksNum(getCompanyId(), getCurrentUser(), ContextUtils.getUserId(),true) + ")" ));
//		
//	tree.append(" ]");
	return null;
	...
	public String delegateAccept() throws Exception{
		List<ZTreeNode> treeNodes = new ArrayList<ZTreeNode>();
		String name = Struts2Utils.getText("workflow.doing")+ "(" + 
		taskService.getAcceptTasksNum(false)+ ")";
		ZTreeNode root = new ZTreeNode("ACC_ING","0",name, "false", "false", "", "", "folder", "");
		treeNodes.add(root);
		
		name = Struts2Utils.getText("workflow.complete")+ "(" + 
		taskService.getAcceptTasksNum(true)+ ")";
	...
	public String wfTypes() throws Exception{
		...
		if( "INITIALIZED_PROCESS".equals(currentId)){
			ZTreeNode root = new ZTreeNode("ENABLE_ALL_1","0",Struts2Utils.getText("wf.menu.currentVersion"), "true", "false", "", "", "folder", "");
			treeNodes.add(root);
			getSecondNodesInWftypeTree(wfTypes,menuList,"ENABLE",treeNodes,"ENABLE_ALL_1");
			ZTreeNode UnenableRoot = new ZTreeNode("UNABLE_ALL_1","0",Struts2Utils.getText("wf.menu.historicalVersion"), "false", "false", "", "", "folder", "");
			treeNodes.add(UnenableRoot);
			getSecondNodesInWftypeTree(wfTypes,menuList,"UNABLE",treeNodes,"UNABLE_ALL_1");
		}else if( "INITIALIZED_MONITOR".equals(currentId)){
			ZTreeNode root = new ZTreeNode("WFT_monitor_0","0",Struts2Utils.getText("wf.menu.currentInstance"), "true", "false", "", "", "folder", "");
			treeNodes.add(root);
			getSecondNodesInMonitorTree(wfTypes,treeNodes,"WFT_monitor_0");
			root = new ZTreeNode("WFT_history_monitor_0","0",Struts2Utils.getText("wf.menu.archiveInstance"), "false", "false", "", "", "folder", "");
			treeNodes.add(root);
			getSecondNodesInMonitorHistoryTree(wfTypes,treeNodes,"WFT_history_monitor_0");
		}else if("INITIALIZED_DICT".equals(currentId)){
			ZTreeNode root = new ZTreeNode("WFT_myCreate_0","0",Struts2Utils.getText("wf.menu.allData"), "false", "false", "", "", "folder", "");
			treeNodes.add(root);
			root = new ZTreeNode("WFT_type_0","0",Struts2Utils.getText("wf.menu.typeManagement"), "false", "false", "", "", "folder", "");
			treeNodes.add(root);
		}else if("INITIALIZED_TEMPLATE".equals(currentId)){
			ZTreeNode root = new ZTreeNode("WFT_0","0",Struts2Utils.getText("workflow.allTemplate"), "false", "false", "", "", "folder", "");
			treeNodes.add(root);
			for(WorkflowType wft : wfTypes){
			...
			ZTreeNode root = new ZTreeNode("WFDT_0","0",Struts2Utils.getText("workflow.allTemplate"), "false", "false", "", "", "folder", "");
			treeNodes.add(root);
			for(WorkflowType wft : wfTypes){
	 ...
	 public void getSecondNodesInMonitorTree(List<WorkflowType> wfTypes,List<ZTreeNode> treeNodes,String parentId){
			ZTreeNode root = new ZTreeNode("WFT_monitor_def",parentId,Struts2Utils.getText("wf.menu.allProcess"), "true", "false", "", "", "folder", "");
			treeNodes.add(root);
	 ...
	 public void getSecondNodesInMonitorHistoryTree(List<WorkflowType> wfTypes,List<ZTreeNode> treeNodes,String parentId){
			ZTreeNode root = new ZTreeNode("WFT_history_monitor_def",parentId,Struts2Utils.getText("wf.menu.allProcess"), "true", "false", "", "", "folder", "");
			treeNodes.add(root);
	...
	public void getSecondNodesInWftypeTree(List<WorkflowType> wfTypes,List<Menu> menuList,String belongType,List<ZTreeNode> treeNodes,String parentId){
		//流程类型
		ZTreeNode root = new ZTreeNode(belongType+"_WFT_0",parentId,Struts2Utils.getText("wf.menu.flowType"), "true", "false", "", "", "folder", "");
		treeNodes.add(root);
		for(WorkflowType wft : wfTypes){
		...
		//所有系统
		root = new ZTreeNode(belongType+"_BSYS_0",parentId,Struts2Utils.getText("wf.menu.allSystems"), "false", "false", "", "", "folder", "");
		treeNodes.add(root);
		for(Menu menu : menuList){
			root = new ZTreeNode(belongType+"_BSYS_"+menu.getSystemId(),belongType+"_BSYS_0",menuManager.getNameToi18n(menu.getName()), "false", "false", "", "", "folder", "");
			treeNodes.add(root);
		}
	}
	...
	public String formTypes(List<WorkflowType> wfTypes,String formType){
		StringBuilder tree = new StringBuilder();
		tree.append(JsTreeUtils.generateJsTreeNodeDefault("WFT_"+formType+"_0", null, Struts2Utils.getText("workflow.allForm"))).append(",");
		for(WorkflowType wft : wfTypes){
	...
	public String load() throws Exception{
		...
		if("INITIALIZED".equals(currentId)){
			...
			subNodes.append(JsTreeUtils.generateJsTreeNodeDefault("NODEPARTMENTUS," + getCompanyId(), 
					"closed", Struts2Utils.getText("user.noDepartment"), ""));
			JsTreeUtils.removeLastComma(subNodes);
			//公司节点
