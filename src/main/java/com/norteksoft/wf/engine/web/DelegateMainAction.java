...
public class DelegateMainAction extends CrudActionSupport<TrustRecord>{
	...
	@Override
	@Action("delegate-main-delete")
	public String delete() throws Exception {
		int[] result = delegateMainManager.deleteDelegateMains(deleteIds);
		StringBuilder message = new StringBuilder();
		if(result[0]!=0){
			message.append(result[0]).append(Struts2Utils.getText("wf.engine.delegate.deleted"));
		}
		if(result[0]!=0&&result[1]!=0)message.append(",");
		if(result[1]!=0){
			message.append(result[1]).append(Struts2Utils.getText("wf.engine.delegate.isEnabledIn"));
		}
		ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("wf.entrust"), ApiFactory.getBussinessLogService().getI18nLogInfo("wf.deleteEntrust"), ContextUtils.getSystemId("wf"));
		this.addSuccessMessage(message.toString());
		return list();
	}

	@Override
	@Action("delegate-main-input")
	public String input() throws Exception {
		ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("wf.entrust"), 
				ApiFactory.getBussinessLogService().getI18nLogInfo("wf.entrustFormpage"), 
		...
		if(id==null||delegateMain.getState()==TrustRecordState.NEW_CREATING||needStart){
			...
			return "delegate-main-input";
		}else{
			this.addErrorMessage(Struts2Utils.getText("wf.engine.delegate.enabledAfter"));
			return list();
		}
	}
	...
	public void prepareView() throws Exception {
		if(id==null){
			...
			if(!"0".equals(delegateMain.getProcessId())){
	...
	//查看页面
	@Action("delegate-main-viewReceive")
	public String viewReceive() throws Exception {
		...
		ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("wf.entrust"), 
				ApiFactory.getBussinessLogService().getI18nLogInfo("wf.viewReceive"), 
				ContextUtils.getSystemId("wf"));
		return "viewReceive";
	}
	...
	public String list() throws Exception {
		if(page.getPageSize()>1){
			...
			ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("wf.entrust"), 
					ApiFactory.getBussinessLogService().getI18nLogInfo("wf.trustList"), 
					ContextUtils.getSystemId("wf"));
	...
	public String receive() throws Exception{
		if(receivePage.getPageSize()>1){
			...
			ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("wf.entrust"), 
					ApiFactory.getBussinessLogService().getI18nLogInfo("wf.receivelist"), 
					ContextUtils.getSystemId("wf"));
		...
		if(validateDelegateDate(delegateMain)==0){
			if(!ContextUtils.getUserId().equals(delegateMain.getTrusteeId())){
				...
				case 1: 
					...
					break;
				case 2: 
					delegateMain.setName("delegatemain.carte.blanche");//全权委托
					break;
				case 3:
					delegateMain.setName("delegatemain.authority.delegation");//权限委托
					...
					if(rolesIds!=null){
						...
						this.addErrorMessage(Struts2Utils.getText("wf.engine.delegate.selectARole"));
						ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("wf.entrust"), 
								ApiFactory.getBussinessLogService().getI18nLogInfo("wf.saveEntrust"), ContextUtils.getSystemId("wf"));
						//不能执行保存方法，直接返回
						return input();
					}
				...
				this.addErrorMessage(Struts2Utils.getText("wf.engine.delegate.noEntrusted"));
			}
		}else{
			if(validateDelegateDate(delegateMain)==2){
			    this.addErrorMessage(Struts2Utils.getText("wf.engine.delegate.TheDeadlineFor"));
			}else if(validateDelegateDate(delegateMain)==1){
				this.addErrorMessage(Struts2Utils.getText("wf.engine.delegate.TheTimeRequired"));
			}
		}
		ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("wf.entrust"), 
				ApiFactory.getBussinessLogService().getI18nLogInfo("wf.saveEntrust"), ContextUtils.getSystemId("wf"));
	...
	@Action("delegate-main-start")
	public String start() throws Exception {
		if(ids.size()==1){
			this.addActionMessage(delegateMainManager.startDelegateMain(ids.get(0)));
		}else if(ids.size()==0){
			this.addErrorMessage(Struts2Utils.getText("user.pleaseSelect"));
		}else{
			this.addErrorMessage(Struts2Utils.getText("user.pleaseSelectOne"));
		}
		ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("wf.entrust"), 
				ApiFactory.getBussinessLogService().getI18nLogInfo("wf.enableEntrust"), ContextUtils.getSystemId("wf"));
		return list();
	}
	
	public void prepareEnd() throws Exception{
	}
	@Action("delegate-main-end")
	public String end() throws Exception {
		...
		ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("wf.entrust"),
				ApiFactory.getBussinessLogService().getI18nLogInfo("wf.cancelTrust"), ContextUtils.getSystemId("wf"));
		return list();
	}
...
//	}
	...
	//我的委托
	@Action("delegate-main-myDelegate")
	public String myDelegate()throws Exception{
		...
		if(tasks.getPageSize()>1){
			...
			ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("wf.entrust"), 
					ApiFactory.getBussinessLogService().getI18nLogInfo("wf.TaskTrustlist"), 
					ContextUtils.getSystemId("wf"));
			this.renderText(PageUtils.pageToJson(tasks));
			return null;
		}
		return "delegate-main-delegate";
	}
	 ...
	 * @throws Exception
	 */
	@Action("delegate-main-myHistoryDelegate")
	...
	//我的受托
	@Action("delegate-main-superviseAsTrustee")
	public String superviseAsTrustee()throws Exception{
		if(inHistory){
			if(historyTasks.getPageSize()>1){
				...
				ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("wf.entrust"), 
						ApiFactory.getBussinessLogService().getI18nLogInfo("wf.commissionedList"), 
						ContextUtils.getSystemId("wf"));
				this.renderText(PageUtils.pageToJson(historyTasks));
				return null;
			}
			return "delegate-main-trusteeviewHistory";
		}else{
			if(tasks.getPageSize()>1){
			...
			return "delegate-main-trusteeview";
		}
	}
	//我的接收
	@Action("delegate-main-myAccept")
	public String myAccept()throws Exception{
		if(inHistory){
			if(historyTasks.getPageSize()>1){
				taskService.getHistoryTaskAsAccept(historyTasks, isDone);
				ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("wf.receive"), 
						ApiFactory.getBussinessLogService().getI18nLogInfo("wf.taskReceiveMonitorlist"), 
						ContextUtils.getSystemId("wf"));
				this.renderText(PageUtils.pageToJson(historyTasks));
				return null;
			}
			return "delegate-main-acceptHistory";
		}else{
			if(tasks.getPageSize()>1){
				taskService.getTaskAsAccept(tasks, isDone);
				ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("wf.receive"), 
						ApiFactory.getBussinessLogService().getI18nLogInfo("wf.taskReceiveMonitorlist"), 
						ContextUtils.getSystemId("wf"));
				this.renderText(PageUtils.pageToJson(tasks));
	 ...
	 * @throws Exception
	 */
	@Action("delegate-main-getRolesByUser")
	public String getRolesByUser() throws Exception{
		...
		htm.append("<ul>");
		for(Role role : roles){
			boolean isAdmin = ApiFactory.getAcsService().isAdmin(role.getCode());
			if(!isAdmin&&!"workflowManager".equals(role.getCode())&&!"defManager".equals(role.getCode())){//委托管理中不能委托三员、分支机构管理员、工作流管理员、流程定义管理员权限
