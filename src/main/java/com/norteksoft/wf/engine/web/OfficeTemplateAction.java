...
public class OfficeTemplateAction extends CrudActionSupport<DocumentTemplate> {
	...
	@Override
	@Action("office-template-delete")
	public String delete() throws Exception {
		documentTemplateFileManager.deleteTemplateByIds(templateIds);
		ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("wf.textTemplate"), 
				ApiFactory.getBussinessLogService().getI18nLogInfo("wf.deleteTextTemplate"), 
	...
	@Override
	@Action("office-template-input")
	public String input() throws Exception {
		typeList = workflowTypeManager.getAllWorkflowType();
		typeId = documentTemplate.getTypeId();
		ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("wf.textTemplate"), 
				ApiFactory.getBussinessLogService().getI18nLogInfo("wf.textTemplatepage"), 
				ContextUtils.getSystemId("wf"));
		return "office-template-input";
	}
	...
	public String list() throws Exception {
		if(page.getPageSize()>1){
			...
			ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("wf.textTemplate"), 
					ApiFactory.getBussinessLogService().getI18nLogInfo("wf.textTemplatelist"), 
					ContextUtils.getSystemId("wf"));
	...
	public String save() throws Exception {
		...
		ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("wf.textTemplate"), 
				ApiFactory.getBussinessLogService().getI18nLogInfo("wf.saveTextTemplate"), 
				ContextUtils.getSystemId("wf"));
		this.addActionMessage(getText("form.save.success"));
		return input();
	}
...
//		documentTemplateFileManager.saveTemplate(documentTemplate);
		...
		ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("wf.textTemplate"), 
				ApiFactory.getBussinessLogService().getI18nLogInfo("wf.uplaodTextTemplate"), 
				ContextUtils.getSystemId("wf"));
		return null;
	}
