...
public class DefinitionXmlParse {
	...
	public static final String REMIND_TIME_WAY = "time-way";
	...
	public static final String REMIND_TRANSFER_USER_CONDITION = "transfer-user-condition";//移交人条件设置
	public static final String REMIND_CONTAIN_NOTICE_USER = "contain-notice-user";//是否包含通知人员
	public static final String REMIND_CONTAIN_TRANSFER_USER = "contain-transfer-user";//是否包含移交人员
	public static final String REMIND_TIME_WAY_DAY="day";
	public static final String REMIND_TIME_WAY_HOUR="hour";
	...
	public static String SAVE_NAME = "save-name";
	public static String APPOINT_NAME = "appoint-name";
	public static String COPY_NAME = "copy-name";
	public static String DRAW_NAME = "draw-name";
	public static String GET_BACK_NAME = "get-back-name";
	public static String ABANDON_NAME = "abandon-name";
	public static String SHOW_BUTTON_ADD_COUNTER="show-Button-add-counter";
	public static String SHOW_BUTTON_DEL_COUNTER="show-Button-del-counter";
	public static String SHOW_BUTTON_KIKEN="show-button-kiken";
	public static String SHOW_BUTTON_APPOINT="show-button-appoint";
	public static String SHOW_BUTTON_COPY="show-button-copy";
	public static String SHOW_BUTTON_SAVE="show-button-save";
	public static String SHOW_BUTTON_DRAW="show-button-draw";
	public static String SHOW_BUTTON_GETBACK="show-button-getBack";
	public static String SHOW_BUTTON_ABANDON="show-button-abandon";
	public static String DEFAULT_VALUE="_A_A";//当在监听中将该值已放入缓存，但其值是null时，设置默认值为该值。
	...
	public static List<String> getDecisionConditions(String processId ,String decisionName){
    	...
    	if(StringUtils.isNotEmpty(decisionName)){
    		...
    		if(result==null||result.size()<=0)result = getMyDecisionConditions(processId,decisionName);
    	}else{
    	...
    	if(StringUtils.isNotEmpty(receivers)){
    		...
    		for(String loginName:loginNames){
    			try {
    				...
    				String type = "待办任务流向通知";
    				if(user!=null){
    					String language = ApiFactory.getPortalService().getUserLanguageById(user.getId());
    					type = Struts2Utils.getText("message.theToDoTaskToNotice", language);//待办任务流向通知
    					ApiFactory.getPortalService().addMessage("task", ContextUtils.getUserName(), ContextUtils.getUserId(), user.getId(),type,getNeedInformSubject(processId,transitionName), "/task/workflow-notification.htm?notificationType=transition&processId="+processId+"&transitionName="+transitionName,true,"task--"+workflow.getProcessInstanceId());
    				}else{
    					ApiFactory.getPortalService().addMessage("task", ContextUtils.getUserName(), ContextUtils.getLoginName(), user.getLoginName(),type,getNeedInformSubject(processId,transitionName), "/task/workflow-notification.htm?notificationType=transition&processId="+processId+"&transitionName="+transitionName,true,"task--"+workflow.getProcessInstanceId());
    				}
				} catch (Exception e) {
					e.printStackTrace();
				}
    	...
    	if(StringUtils.isNotEmpty(receivers)){
    		...
    		for(String loginName:loginNames){
    			try {
    				...
    				if(user!=null){
    					String language = ApiFactory.getPortalService().getUserLanguageById(user.getId());
    					type = Struts2Utils.getText("message.notificationAtTheEndOfTheProcess", language);//流程结束时通知
    					ApiFactory.getPortalService().addMessage("task", ContextUtils.getUserName(), ContextUtils.getUserId(), user.getId(),type,getProcessInformSubject(processId), "/task/workflow-notification.htm?notificationType=process&processId="+processId,true,"task--"+workflow.getProcessInstanceId());
    				}else{
    					ApiFactory.getPortalService().addMessage("task", ContextUtils.getUserName(), ContextUtils.getLoginName(), user.getLoginName(),type,getProcessInformSubject(processId), "/task/workflow-notification.htm?notificationType=process&processId="+processId,true,"task--"+workflow.getProcessInstanceId());
    				}
				} catch (Exception e) {
					e.printStackTrace();
				}
    ...
    */
	...
	public static List<AutomaticallyFilledField> getAfterFilledFields(String processId,
		...
		if(StringUtils.isNotEmpty(taskName)){
			...
			if(result==null||result.size()<=0) result = getMyAfterFilledFields(processId,taskName);
		}else{
	...
	public static List<AutomaticallyFilledField> getBeforeFilledFields(String processId,
		...
		if(StringUtils.isNotEmpty(taskName)){
			...
			if(result==null||result.size()<=0) result = getMyBeforeFilledFields(processId,taskName);
		}else{
			result = new ArrayList<AutomaticallyFilledField>();
		}
		return result;
	...
	public static List<AutomaticallyFilledField> getFlowingFilledFields(String processId,
		...
		if(StringUtils.isNotEmpty(transitionName)){
			...
			if(result==null||result.size()<=0) result = getMyFlowingFilledFields(processId,transitionName);
		}else{
			result = new ArrayList<AutomaticallyFilledField>();
		}
		return result;
	...
	public static List<String> getTaskNames(String processId){
		...
		if(result==null||result.size()<=0)result = getMyTaskNames(processId,null);
		return result;
	}
	@SuppressWarnings("unchecked")
	public static List<String> getMyTaskNames(String processId,Long workflowDefinitionId){
		Document document;
		if(StringUtils.isNotEmpty(processId)){
			document = DocumentParameterUtils.getDocument(processId);
		}else{
			...
			WorkflowDefinitionFile defFile = workflowDefinitionManager.getWfDefinitionFileByWfdId(workflowDefinitionId, ContextUtils.getCompanyId());
			document = Dom4jUtils.getDocument(defFile.getDocument());
		}
		String namespaceUrl = document.getRootElement().getNamespace().getURI();
		HashMap<String,String> xmlMap = new HashMap<String,String>();   
		xmlMap.put("wf",namespaceUrl);   
		...
		for(Element node: childs){
			//子流程环节
			String name = node.attributeValue("name");
			if(isSubProcessTask(processId,name,workflowDefinitionId)){
				taskNames.add(name);
			}
		}
		return taskNames;
	...
	@SuppressWarnings("unchecked")
	public static List<String> getTaskCodes(Long workflowDefinitionId){
		Document document;
		WorkflowDefinitionManager workflowDefinitionManager = (WorkflowDefinitionManager)ContextUtils.getBean("workflowDefinitionManager");
		WorkflowDefinitionFile defFile = workflowDefinitionManager.getWfDefinitionFileByWfdId(workflowDefinitionId, ContextUtils.getCompanyId());
		document = Dom4jUtils.getDocument(defFile.getDocument());
		...
		HashMap<String,String> xmlMap = new HashMap<String,String>();   
		xmlMap.put("wf",namespaceUrl);   
		XPath x = document.createXPath("//wf:process/wf:task/wf:extend/wf:basic-properties/wf:task-code");   
		x.setNamespaceURIs(xmlMap);
		List<String> taskCodes = new ArrayList<String>();
		List<Element> childs = x.selectNodes(document);
		if(childs==null)return taskCodes;
		for(Element node: childs){
			taskCodes.add(node.getText());
		}
		return taskCodes;
	}
	...
	public static List<String> getUnFieldTaskNames(String processId){
		...
		if(result==null||result.size()<=0)result = getMyUnFieldTaskNames(processId);
		return result;
	}
	@SuppressWarnings("unchecked")
	private static List<String> getMyUnFieldTaskNames(String processId){
		...
		for(Element node: childs){
			...
			"']/extend/transactor-settings/user-condition");
			if(isSubProcessTask(processId,name,null)){
	...
	public static List<String> getFieldTaskNames(String processId){
		...
		if(result==null||result.size()<=0)result = getMyFieldTaskNames(processId);
		return result;
	}
	@SuppressWarnings("unchecked")
	private static List<String> getMyFieldTaskNames(String processId){
		...
		for(Element node: childs){
			...
			String userCondition = Dom4jUtils.getSingleElementValueByPath(processId, 
					"/process/custom[@name='" + name+ 
			"']/extend/transactor-settings/user-condition");
			if(isSubProcessTask(processId,name,null)&&StringUtils.isNotEmpty(userCondition)&&userCondition.startsWith("${field[")){
				taskNames.add(name);
			}
		}
		return taskNames;
	...
	public static List<String[]> getBeforeTaskSubmitUrl(String processId,String taskName){
		...
		if(StringUtils.isNotEmpty(taskName)){
			...
			if(result==null||result.size()<=0)result = getMyBeforeTaskSubmitUrl(processId,taskName);
		}else{
			result = new ArrayList<String[]>();
		}
		return result;
	...
	private static List<String[]> getMyBeforeTaskSubmitUrl(String processId,String taskName){
		...
		if(nodes==null){
			...
			for(Element node:nodes){
				Element dfNode = node.element("default-url");
				String defaultUrl = dfNode==null?"":dfNode.getText();//默认值：意见框OPINION；选择办理人 SELECT_TRANSACTOR
				String url = node.element("url").getText();
				if(StringUtils.isNotEmpty(defaultUrl)){
					url = defaultUrl;
				}
				resultList.add(new String[]{node.element("condition").getText(),url});
			}
		}
		return resultList;
	}
	...
	public static Map<String,String>  getReminderSetting(String processId,String taskName){
		...
		if(StringUtils.isNotEmpty(taskName)){
			...
			if(result==null||result.size()<=0)result = getMyReminderSetting(processId,taskName);
		}else{
			result = new HashMap<String,String>();
		}
		return result;
	...
	private static Map<String,String>  getMyReminderSetting(String processId,String taskName){
		...
		if(node!=null){
			...
			reminderSetting.put(REMIND_TIME_WAY, element.elementText(REMIND_TIME_WAY));
			...
			reminderSetting.put(REMIND_TRANSFER_USER_CONDITION,element.elementText(REMIND_TRANSFER_USER_CONDITION));
			reminderSetting.put(REMIND_CONTAIN_NOTICE_USER,element.elementText(REMIND_CONTAIN_NOTICE_USER));
			reminderSetting.put(REMIND_CONTAIN_TRANSFER_USER,element.elementText(REMIND_CONTAIN_TRANSFER_USER));
		}
	...
	public static Map<String,String>  getReminderSetting(String processId){
		...
		if(result==null||result.size()<=0)result = getMyReminderSetting(processId);
		return result;
	}
	...
	public static Map<String,String>  getParameterSetting(String processId){
		...
		if(result==null||result.size()<=0)result = getMyParameterSetting(processId);
		return result;
	}
	...
	public static Map<String, String> getChoiceTaches(String processId, String taskName) {
		...
		if(StringUtils.isNotEmpty(taskName)){
			...
			if(result==null||result.size()<=0)result = getMyChoiceTaches(processId, taskName);
		}else{
			result = new HashMap<String,String>();
		}
		return result;
...
//	}
    ...
    public static boolean isSubProcessTask(String processId,String taskName,Long workflowDefinitionId){
    	Boolean result = false;
    	if(StringUtils.isNotEmpty(taskName)){
    		if(StringUtils.isNotEmpty(processId)){
    			result = (Boolean)WorkflowMemcachedUtil.get(getHashCode(processId+"=isSubProcessTask="+taskName));
    			if(result == null)result = isMySubProcessTask(processId,taskName,null);
    		}else{
    			result = isMySubProcessTask(processId,taskName,workflowDefinitionId);
    		}
    	}
    	return result;
    }
    private static boolean isMySubProcessTask(String processId,String taskName,Long workflowDefinitionId){
    	Document document;
    	if(StringUtils.isNotEmpty(processId)){
    		document = DocumentParameterUtils.getDocument(processId);
    	}else{
	...
	public static Map<String,String>  getButtonNameByProcessMode(String processId,String taskName,TaskProcessingMode processModel){
    	...
    	if(StringUtils.isNotEmpty(taskName)){
    		...
    		if(result==null||result.size()<=0) result = getMyButtonNameByProcessMode(processId,taskName,processModel);
    	}else{
    		result = new HashMap<String,String>();
    	}
    	return result;
	...
	private static Map<String,String>  getMyButtonNameByProcessMode(String processId,String taskName,TaskProcessingMode processModel){
		...
		if(node!=null){
			...
			if(TaskProcessingMode.TYPE_EDIT.equals(processModel)){
				...
				buttonNameSetting.put(SAVE_NAME, element.attributeValue(SAVE_NAME));
				buttonNameSetting.put(APPOINT_NAME, element.attributeValue(APPOINT_NAME));
				buttonNameSetting.put(COPY_NAME, element.attributeValue(COPY_NAME));
				buttonNameSetting.put(DRAW_NAME, element.attributeValue(DRAW_NAME));
				buttonNameSetting.put(GET_BACK_NAME, element.attributeValue(GET_BACK_NAME));
				buttonNameSetting.put(ABANDON_NAME, element.attributeValue(ABANDON_NAME));
				buttonNameSetting.put(SHOW_BUTTON_SAVE, element.attributeValue(SHOW_BUTTON_SAVE));
				buttonNameSetting.put(SHOW_BUTTON_APPOINT, element.attributeValue(SHOW_BUTTON_APPOINT));
				buttonNameSetting.put(SHOW_BUTTON_COPY, element.attributeValue(SHOW_BUTTON_COPY));
				buttonNameSetting.put(SHOW_BUTTON_DRAW, element.attributeValue(SHOW_BUTTON_DRAW));
				buttonNameSetting.put(SHOW_BUTTON_GETBACK, element.attributeValue(SHOW_BUTTON_GETBACK));
				buttonNameSetting.put(SHOW_BUTTON_ABANDON, element.attributeValue(SHOW_BUTTON_ABANDON));
				...
				buttonNameSetting.put(COPY_NAME, element.attributeValue(COPY_NAME));
				buttonNameSetting.put(GET_BACK_NAME, element.attributeValue(GET_BACK_NAME));
				buttonNameSetting.put(SHOW_BUTTON_ADD_COUNTER, element.attributeValue(SHOW_BUTTON_ADD_COUNTER));
				buttonNameSetting.put(SHOW_BUTTON_DEL_COUNTER, element.attributeValue(SHOW_BUTTON_DEL_COUNTER));
				buttonNameSetting.put(SHOW_BUTTON_SAVE, element.attributeValue(SHOW_BUTTON_SAVE));
				buttonNameSetting.put(SHOW_BUTTON_APPOINT, element.attributeValue(SHOW_BUTTON_APPOINT));
				buttonNameSetting.put(SHOW_BUTTON_COPY, element.attributeValue(SHOW_BUTTON_COPY));
				buttonNameSetting.put(SHOW_BUTTON_GETBACK, element.attributeValue(SHOW_BUTTON_GETBACK));
				...
				buttonNameSetting.put(COPY_NAME, element.attributeValue(COPY_NAME));
				buttonNameSetting.put(GET_BACK_NAME, element.attributeValue(GET_BACK_NAME));
				buttonNameSetting.put(SHOW_BUTTON_KIKEN, element.attributeValue(SHOW_BUTTON_KIKEN));
				buttonNameSetting.put(SHOW_BUTTON_SAVE, element.attributeValue(SHOW_BUTTON_SAVE));
				buttonNameSetting.put(SHOW_BUTTON_APPOINT, element.attributeValue(SHOW_BUTTON_APPOINT));
				buttonNameSetting.put(SHOW_BUTTON_COPY, element.attributeValue(SHOW_BUTTON_COPY));
				buttonNameSetting.put(SHOW_BUTTON_GETBACK, element.attributeValue(SHOW_BUTTON_GETBACK));
	...
	public static Map<String,String[]> getActivityPermissionCondition(String processId,String taskName){
		...
		if(StringUtils.isNotEmpty(taskName)){
			...
			if(result==null||result.size()<=0)result=getMyActivityPermissionCondition(processId,taskName);
		}else{
			result=new HashMap<String, String[]>();
		}
		return result;
	...
	public static Map<String,String> getMonitorDeleteInstanceSet(String processId) {
		...
		if(result==null||result.size()<=0)result=getMyMonitorDeleteInstanceSet(processId);
		return result;
	}
	...
	public static Map<String,String> getMonitorCancelInstancSet(String processId) {
		...
		if(result==null||result.size()<=0)result=getMyMonitorCancelInstancSet(processId);
		return result;
	}
	...
	public static Map<String,String> getMonitorTaskJumpSet(String processId) {
		...
		if(result==null||result.size()<=0)result=getMyMonitorTaskJumpSet(processId);
		return result;
	}
	...
	public static Map<String,String> getPauseInstancSet(String processId) {
		...
		if(result==null||result.size()<=0)result=getMyPauseInstancSet(processId);
		return result;
	}
	...
	public static Map<String,String> getContinueInstancSet(String processId) {
		...
		if(result==null||result.size()<=0)result=getMyContinueInstancSet(processId);
		return result;
	}
	...
	public static Map<String,String> getChangeTransactorSet(String processId) {
		...
		if(result==null||result.size()<=0)result=getMyChangeTransactorSet(processId);
		return result;
	}
	...
	public static Map<String,String> getAddTransactorSet(String processId) {
		...
		if(result==null||result.size()<=0)result=getMyAddTransactorSet(processId);
		return result;
	}
	...
	public static Map<String,String> getDeleteTransactorSet(String processId) {
		...
		if(result==null||result.size()<=0)result=getMyDeleteTransactorSet(processId);
		return result;
	}
	...
	public static Map<String,String> getAssignTransactorSet(String processId) {
		...
		if(result==null||result.size()<=0)result=getMyAssignTransactorSet(processId);
		return result;
	}
	...
	private static Map<String,String> getMyExtendFields(String processId, String taskName){
    	...
    	XPath x = document.createXPath("/wf:process/wf:task[@name='"+taskName+"']/wf:extend/wf:extend-field");   
 		x.setNamespaceURIs(xmlMap);  
 		Node node = x.selectSingleNode(document);
    	String extendFields = node==null?"":node.getText().trim();
    	if(StringUtils.isNotEmpty(extendFields)){
    		String[] fields=extendFields.split(",");
    		for(String field:fields){
    			String[] tempField=field.split(":");
    			extendFieldMap.put(tempField[0], packagingExtendFieldValue(tempField));
    		}
    	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static Map<String,String> getFilterTachePopupPageSet(String processId,Long defId) {
		Map<String,String>  result = null;
		if(StringUtils.isNotEmpty(processId)){
			result = (Map<String,String>)WorkflowMemcachedUtil.get(getHashCode(processId+"=filterTachePopupPageSet"));
			if(result==null||result.size()<=0)result=getMyFilterTachePopupPageSet(getDocument(processId,null));
		}else{
			result = getMyFilterTachePopupPageSet(getDocument(processId,defId));
		}
		return result;
	}
	private static Map<String,String> getMyFilterTachePopupPageSet(Document document) {
		String namespaceUrl = document.getRootElement().getNamespace().getURI();
		HashMap<String,String> xmlMap = new HashMap<String,String>();   
		xmlMap.put("wf",namespaceUrl);   
		XPath x = document.createXPath("/wf:process/wf:extend/wf:expiation-setting/wf:set-type");
		...
		Node node = x.selectSingleNode(document);
		String setType = node==null?"":node.getText();
		x = document.createXPath("/wf:process/wf:extend/wf:expiation-setting/wf:filter-tache-popup-page");   
		x.setNamespaceURIs(xmlMap);  
		node = x.selectSingleNode(document);
		String filterTachePopupPage = node==null?"":node.getText();
		Map<String,String> map=new HashMap<String, String>();
		map.put(SET_TYPE, setType);
		map.put(FILTER_TACHE_POPUP_PAGE, filterTachePopupPage);
		return map;
	}
	
	@SuppressWarnings("unchecked")
	public static Map<String,String> getProcessExtendFields(String processId,Long defId){
		Map<String,String>  result = null;
		if(StringUtils.isNotEmpty(processId)){
			result = (Map<String,String>)WorkflowMemcachedUtil.get(getHashCode(processId+"=processExtendFields"));
			if(result == null)result = getMyProcessExtendFields(getDocument(processId,null));
		}else{
			result = getMyProcessExtendFields(getDocument(processId,defId));
		}
		return result;
	}
	private static Document getDocument(String processId,Long defId){
		Document document;
		if(StringUtils.isNotEmpty(processId)){
			document = DocumentParameterUtils.getDocument(processId);
		}else{
			WorkflowDefinitionManager workflowDefinitionManager = (WorkflowDefinitionManager)ContextUtils.getBean("workflowDefinitionManager");
			WorkflowDefinitionFile defFile = workflowDefinitionManager.getWfDefinitionFileByWfdId(defId, ContextUtils.getCompanyId());
			document = Dom4jUtils.getDocument(defFile.getDocument());
		}
		return document;
	}
	private static Map<String,String> getMyProcessExtendFields(Document document){
		Map<String,String> extendFieldMap=new HashMap<String, String>();
		String namespaceUrl = document.getRootElement().getNamespace().getURI();
		HashMap<String,String> xmlMap = new HashMap<String,String>();   
		xmlMap.put("wf",namespaceUrl);   
		XPath x = document.createXPath("/wf:process/wf:extend/wf:process-extend-field");   
		x.setNamespaceURIs(xmlMap);  
		Node node = x.selectSingleNode(document);
		String extendFields = node==null?"":node.getText().trim();
		if(StringUtils.isNotEmpty(extendFields)){
	...
	private static String packagingExtendFieldValue(String[] tempField){
		String fieldValue="";
     ...
     * 解析任务预设的显示标题
     * @param xmlFile
     * @param tagName 表示是任务环节还是抄送环节，任务环节时该值为“task”，抄送环节时该值为“state”
     * @return
     */
    public static String getTaskUrl(String processId, String taskName,String tagName){
    	String result = null; 
    	if(StringUtils.isNotEmpty(taskName)){
    		result = (String)WorkflowMemcachedUtil.get(getHashCode(processId+"=taskUrl="+taskName));
    		if(result == null){
				result = getMyTaskUrl(processId,taskName,tagName);
			}else{
				if(DEFAULT_VALUE.equals(result)){
					result = null;
				}
    ...
    private static String getMyTaskUrl(String processId, String taskName,String tagName){
    	Document document = DocumentParameterUtils.getDocument(processId);
    	String namespaceUrl = document.getRootElement().getNamespace().getURI();
    	HashMap<String,String> xmlMap = new HashMap<String,String>();   
    	xmlMap.put("wf",namespaceUrl);   
    	XPath x = document.createXPath("/wf:process/wf:"+tagName+"[@name='" + taskName+ 
    	"']/wf:extend/wf:basic-properties/wf:task-url");   
    	x.setNamespaceURIs(xmlMap);  
    	Node node = x.selectSingleNode(document); 
    	if(node==null)return null;
    	return node.getText();
	...
	private static void getDefinitionParseTacheInfo(List<Element> nodes,String processId,String tacheType ){
		for(Element element : nodes){
				...
				if("task".equals(tacheType)){
					...
					value = getMyTaskUrl(processId, name,"task");
					MemCachedUtils.add(getHashCode(processId+"=taskUrl="+name), value==null?DEFAULT_VALUE:value);
					...
					value = getMyTaskUrl(processId, name,"state");
					MemCachedUtils.add(getHashCode(processId+"=taskUrl="+name), value==null?DEFAULT_VALUE:value);
					...
					MemCachedUtils.add(getHashCode(processId+"=isSubProcessTask="+name), isMySubProcessTask( processId, name,null));
	...
	private static void getDefinitionParseProcess(String processId){
		...
		MemCachedUtils.add(getHashCode(processId+"=taskNames"), getMyTaskNames( processId,null));
		...
		MemCachedUtils.add(getHashCode(processId+"=processExtendFields"), getProcessExtendFields(processId,null));
		MemCachedUtils.add(getHashCode(processId+"=filterTachePopupPageSet"), getFilterTachePopupPageSet(processId,null));
	}
	...
	public static final String FILTER_TACHE_POPUP_PAGE="filterTachePopupPage";
