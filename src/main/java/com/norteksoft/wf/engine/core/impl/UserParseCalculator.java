...
//			userId:老版本记录的是登录名，新版本有多分支时获得的是用户id
	...
	private  String getUserId(String valueExpress){
		...
		}else if(valueExpress.trim().endsWith(CommonStrings.CURRENT_TRANSACTOR_NAME)){
			if(currentTransactorId==null)return null;
			return currentTransactorId+"";
		}else if(valueExpress.trim().endsWith(CommonStrings.CURRENT_TRANSACTOR_DIRECT_SUPERIOR_NAME)){
			User user=rankManager.getDirectLeader(currentTransactorId,currentTransactor);
			return user==null?null:user.getId()+"";
		}else{
	...
	private Set<com.norteksoft.acs.entity.organization.Workgroup> getWorkGroup(String valueExpress){
		...
		}else if(valueExpress.trim().equals(CommonStrings.CURRENT_TRANSACTOR_WORKGROUP)){
			List<Workgroup> workGroups = UserUtil.getWorkGroupsByUserId(currentTransactorId);
			workGroupSet.addAll(workGroups);
		}else if(valueExpress.trim().equals(CommonStrings.CURRENT_TRANSACTOR_DIRECT_SUPERIOR_WORKGROUP)){
			User user=rankManager.getDirectLeader(currentTransactorId,currentTransactor);
			if(user!=null){
				List<Workgroup> workGroups = UserUtil.getWorkGroupsByUserId( user.getId());
				workGroupSet.addAll(workGroups);
	...
	private Set<Department> getDepartment(String valueExpress){
		...
		}else if(valueExpress.trim().equals(CommonStrings.CURRENT_TRANSACTOR_DEPARTMENT)){
			//当前办理人部门
			departmentSet.addAll(UserUtil.getDepartmentsByUserId( currentTransactorId));
		}else if(valueExpress.trim().equals(CommonStrings.CURRENT_TRANSACTOR_SUPERIOR_DEPARTMENT)){
			//当前办理人上级部门
			departmentSet.addAll(UserUtil.getDepartmentsByUserId( currentTransactorId));
		}else if(valueExpress.trim().equals(CommonStrings.CURRENT_TRANSACTOR_UPSTAGE_DEPARTMENT)){
			//当前办理人顶级部门CURRENT_TRANSACTOR_UPSTAGE_DEPARTMENT_ID
			departmentSet.addAll(ApiFactory.getAcsService().getTopDepartmentsByUser(currentTransactorId));
		}else if(valueExpress.trim().equals(CommonStrings.CURRENT_TRANSACTOR_DIRECT_SUPERIOR_DEPARTMENT)){
			//当前办理人直属上级的部门
			User user=rankManager.getDirectLeader(currentTransactorId,currentTransactor);
			if(user!=null){
				departmentSet.addAll(UserUtil.getDepartmentsByUserId(user.getId()));
