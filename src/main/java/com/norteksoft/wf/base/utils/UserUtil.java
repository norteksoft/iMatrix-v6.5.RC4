...
public class UserUtil {
	...
	public static Set<Long> getUsersByRoleName( String roleName,Long systemId,Long companyId){
		Set<Long> userIds = new HashSet<Long>();
		Set<User> users=ApiFactory.getAcsService().getUsersByRoleName(systemId,roleName);
		for(User user: users){
			if(user!=null){
				userIds.add(user.getId());
				System.out.println();
	...
	public static Set<Long> getUsersByRoleCode( String roleCode,Long systemId,Long companyId){
		Set<Long> userIds = new HashSet<Long>();
		Set<User> users=ApiFactory.getAcsService().getUsersByRoleCodeExceptTrustedRole(systemId,roleCode);
		for(User user: users){
			if(user!=null){
				userIds.add(user.getId());
			}
