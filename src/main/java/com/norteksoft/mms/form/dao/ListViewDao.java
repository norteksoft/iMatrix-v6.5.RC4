...
public class ListViewDao extends HibernateDao<ListView, Long> {
	...
	public void getListViewPageByMenu(Page<ListView> page, Long id,Long dataTableId) {
		Long companyId= ContextUtils.getCompanyId();
		String hql = " from ListView lv where lv.companyId="+companyId +
		" and lv.menuId=? and lv.deleted=? and lv.dataTable.id=?";
		this.searchPageByHql(page, hql, id,false,dataTableId);
	}
	public List<ListView> getListViewPageByMenu(Long id,Long dataTableId) {
		Long companyId= ContextUtils.getCompanyId();
		if(dataTableId==null){
			String hql = " from ListView lv where lv.companyId="+companyId +
			" and lv.menuId=? and lv.deleted=?";
			return this.find(hql,id,false);
		}else{
			String hql = " from ListView lv where lv.companyId="+companyId +
			" and lv.menuId=? and lv.deleted=? and lv.dataTable.id=?";
			return this.find(hql,id,false,dataTableId);
		}
	}
	...
	public List<ListView> getListViewByTabelName(String dataTableName){
		String hql = " from ListView v where v.dataTable.name=? and v.companyId=? and v.dataTable.deleted=? and v.deleted=?";
		return this.find(hql, dataTableName,ContextUtils.getCompanyId(),false,false);
	}
