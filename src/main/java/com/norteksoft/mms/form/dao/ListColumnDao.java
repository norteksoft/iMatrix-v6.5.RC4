...
public class ListColumnDao extends HibernateDao<ListColumn, Long> {
	...
	public List<ListColumn> getUnCompanyTempColumns(Long viewId){
		String hql = " from ListColumn lc where lc.listView.id=? and lc.tableColumn is null order by lc.displayOrder";
		return this.findNoCompanyCondition(hql, viewId);
	}
	...
	public List<ListColumn>  getDisplayColumns(Long viewId){
		String hql = " from ListColumn lc where lc.companyId=? and lc.listView.id=? and lc.visible=? order by lc.displayOrder";
		return this.find(hql, ContextUtils.getCompanyId(),viewId,true);
	}
