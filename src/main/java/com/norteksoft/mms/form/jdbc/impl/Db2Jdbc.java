...
public class Db2Jdbc extends JdbcDaoSupport implements JdbcSupport {
	private Log log=LogFactory.getLog(Db2Jdbc.class);
	...
	public void createTable(String tableName,List<TableColumn> columns){
		...
		str.append("id BIGINT NOT NULL PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY ( START WITH 1 INCREMENT BY 1 MINVALUE 1 MAXVALUE 9223372036854775807 NO CYCLE CACHE 20 NO ORDER ),instance_id varchar(100 OCTETS)" );
		if(columns.size()>0){
			str.append(",");
		}else{
			str.append(" )");
	...
	public void createDefaultTable(String tableName,List<FormControl> columns){
		StringBuilder str = new StringBuilder();
		str.append("CREATE TABLE ").append(tableName).append(" (");
		str.append("id BIGINT NOT NULL PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY ( START WITH 1 INCREMENT BY 1 MINVALUE 1 MAXVALUE 9223372036854775807 NO CYCLE CACHE 20 NO ORDER ),instance_id varchar(100 OCTETS),company_id BIGINT,creator VARCHAR(255 OCTETS),creator_name VARCHAR(255 OCTETS),first_task_id BIGINT,process_state INTEGER,current_activity_name VARCHAR(255 OCTETS),workflow_definition_name VARCHAR(255 OCTETS),workflow_definition_id VARCHAR(255 OCTETS),workflow_definition_code  VARCHAR(255 OCTETS),workflow_definition_version INTEGER,form_id INTEGER,create_date TIMESTAMP,creator_department VARCHAR(255 OCTETS),department_id BIGINT,creator_id BIGINT,sub_company_id BIGINT,custom_display_index INTEGER " );
		if(columns.size()>0){
			str.append(",");
		}else{
			str.append(" )");
	...
	private String getSqlType(TableColumn tableColumn){
		...
		if(type.equals(DataType.TEXT)){
			if(length != null){
				dataType = " VARCHAR("+ length +" OCTETS)";
			}else{
				dataType = " VARCHAR(255 OCTETS)";
			}
			...
			dataType = " DOUBLE";
		}else if(type.equals(DataType.TIME)||type.equals(DataType.DATE)){
			dataType = " TIMESTAMP";
	...
	public void createFK(String majorTableName,String childTableName ){
		...
		sql.append("alter table ").append(childTableName).append(" add ").append(TABLE_FK_PREFIX_STRING).append(majorTableName).append(" BIGINT ");
		this.getJdbcTemplate().execute(sql.toString());
		this.getJdbcTemplate().execute("alter table "+childTableName+" add constraint FK_"+majorTableName+" foreign key("+TABLE_FK_PREFIX_STRING+majorTableName+") references "+majorTableName+"(id)");
	}
	...
	public Long insertTable(Map<String,String[]> parameterMap,FormView form,List<FormControl> fields){
		...
		boolean shouldAddCommera=false;
		if(instanceIds!=null&&instanceIds.length>0&&StringUtils.isNotEmpty(instanceIds[0])){
			sql.append(INSTANCE_ID).append(",");
			sql_values.append("?");
			obj.add(instanceIds[0]);
			shouldAddCommera = true;
		}
		sql.append(COMPANY_ID).append(",");
		if(shouldAddCommera)sql_values.append(",?");
		else sql_values.append("?");
