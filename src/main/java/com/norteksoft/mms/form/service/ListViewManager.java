...
public class ListViewManager {
	...
	public String  delete(String viewIds){
		...
		if(viewIds!=null){
			...
			String errorStr=(ids.length-canNum)!=0?(ids.length-canNum)+Struts2Utils.getText("pageManager.deleteFormFail"):"";
	...
	public void  deleteEnable(Long id){
		...
		List<ModulePage> pages=modulePageDao.getModulePagesByListViewId(id);
	...
	public void getListViewPageByMenu(Page<ListView> page,Long menuId,String dataTableName){
		DataTable dataTable = dataTableManager.getDataTableByTableName(dataTableName);
		if(dataTable!=null){
			viewDao.getListViewPageByMenu(page,menuId,dataTable.getId());
		}else{
			viewDao.getListViewPageByMenu(page,menuId);
		}
	}
	...
	public List<ListView> getListViewPageByMenu(Long menuId,String dataTableName){
		DataTable dataTable = dataTableManager.getDataTableByTableName(dataTableName);
		return viewDao.getListViewPageByMenu(menuId,dataTable.getId());
	}
	/**
	 * 通过系统Id得到列表视图
	 * @param 
	...
	public List<ListView> getListViewByTabelName(String dataTableName){
		return viewDao.getListViewByTabelName(dataTableName);
	}
