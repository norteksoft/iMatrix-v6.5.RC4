...
public class FormHtmlParser {
	...
	public void setFormHtml(String formHtml){
	 ...
	 public String validatHtml(String formHtml){
		 ...
		 final String doubleFieldName = Struts2Utils.getText("formManager.checkRepeatField");
		 final String doubleControlIdName = Struts2Utils.getText("formManager.checkRepeatControl");
		 FormFields formFields=source.getFormFields();
		 List<FormControl> controls=getControls(formHtml,false);
	 ...
	 /**
	  * 得到所有的字段
	  * @param formHtml 表单的html片段
	  * @param opinionVisible 是否获得表单中的意见控件
	  */
	 public  List<FormControl> getControls(String formHtml,boolean opinionVisible){
		 this.setFormHtml(formHtml);
		 FormFields formFields=source.getFormFields();
		 ...
		 for (FormField formField : formFields) {
			...
			if(ControlType.OPINION.toString().equals(pluginType)){
				if(opinionVisible){
					//添加意见
					control = getOpinionControl(element,"opinion");
					list.add(control);
					//添加意见签名
					control = getOpinionControl(element,"sign");
					list.add(control);
					//添加意见日期
					control = getOpinionControl(element,"date");
					list.add(control);
					//添加结论
					String conclusionSourceValue=element.getAttributeValue("conclusionSourceValue");
					if(StringUtils.isNotEmpty(conclusionSourceValue)){
						control=new FormControl();
						control = getOpinionControl(element,"conclusion");
						list.add(control);
					}
				}else{
					continue;
...
//			if(StringUtils.isNotEmpty(StringUtils.trim(element.getAttributeValue("columnid"))))control.setTableColumnId(Long.valueOf(StringUtils.trim(element.getAttributeValue("columnid")))); 
	  ...
	  * 
	  * @param element
	  * @param isConclusion 是否是结论
	  * @return
	  */
	 private FormControl getOpinionControl(Element element,String type){
		 String typeInofo = "意见";
		 FormControl control=new FormControl();
		 String controlId=element.getAttributeValue("id");
		 String title=element.getAttributeValue("title");
		 String conclusionSourceValue=element.getAttributeValue("conclusionSourceValue");
		 String conclusionType=element.getAttributeValue("conclusionType");//结论控件类型：单选、复选
		 String myControlId = StringUtils.trim(controlId)+"~~"+type;
		 if(type.equals("conclusion")){
			 typeInofo = "结论";
			 myControlId = StringUtils.trim(controlId)+"~~"+conclusionType+"~~"+type;
		 }else if(type.equals("sign")){//签名
//			 _iMatrix_opinionSign_controId
			 typeInofo = "签名";
		 }else if(type.equals("date")){//签署日期
//			 _iMatrix_opinionDate_controId
			 typeInofo = "日期";
		 }
		 if(StringUtils.isEmpty(conclusionType)){//没有设置，默认为单选
			 conclusionType = "radio";
		 }
		 control.setTitle(StringUtils.trim(title)+"-"+typeInofo);
		control.setName(controlId+"~~"+type);
		control.setControlId(myControlId);
		control.setMacroType(conclusionType);
		control.setControlType(ControlType.valueOf("OPINION"));
		//添加结论来源
		if(StringUtils.isNotEmpty(conclusionSourceValue)){
			control.setDataSrc(conclusionSourceValue);
		}
		return control;
	 }
	...
	//往 FORM里插入值 
	 ...
	 public String setFieldValue(Map<String, Object> map){
		 ...
		 while(it.hasNext()){
			...
			if("OPINION".equals(pluginType))continue;
			if(ControlType.TEXT.toString().equals(pluginType)&&type!=null&&"RADIO".equals(type.toUpperCase())){
				...
				String optionGroupId = element.getAttributeValue("optionGroupId");
				...
				if(StringUtils.isNotEmpty(selectvalues)){
					...
					for(String str:values){
						...
						String controlAttribute="complate=\"true\" value=\""+vals[1]+"\""+(val.contains(vals[1]+",")?" checked=\"checked\"":"")+" id=\""+name+i+"\" updateSign=\"true\"/>";
				...
				}else if(StringUtils.isNotEmpty(optionGroupId)){
						 OptionGroupManager optionGroupManager= (OptionGroupManager)ContextUtils.getBean("optionGroupManager");
						 List<Option> ops=optionGroupManager.getOptionsByGroupCode(optionGroupId);
						 for(Option op:ops){
							 String controlAttribute="complate=\"true\" value=\""+op.getValue()+"\""+(val.contains(op.getValue()+",")?" checked=\"checked\"":"")+" id=\""+name+i+"\" />";
							 newControl+=element.toString().replace("/>", controlAttribute).replace("title=\""+title+"\"", "title=\""+op.getName()+"\"")+op.getName();
							 i++;
						 }
						 outputDocument.replace(element.getBegin(), element.getEnd(), newControl);
					 }
				...
				if(StringUtils.isNotEmpty(selectvalues)){
					...
					for(String str:values){
						...
						String controlAttribute="complate=\"true\" value=\""+vals[1]+"\""+(StringUtils.isNotEmpty(val)&&val.equals(vals[1])?" checked=\"checked\"":"")+" id=\""+name+i+"\" updateSign=\"true\"/>";
						newControl+=element.toString().replace("/>", controlAttribute).replace("title=\""+title+"\"", "title=\""+vals[0]+"\"")+vals[0];
						i++;
					}
					outputDocument.replace(element.getBegin(), element.getEnd(), newControl);
					 ...
					 List<Option> ops=optionGroupManager.getOptionsByGroupCode(optionGroupId);
					 for(Option op:ops){
						 String controlAttribute="complate=\"true\" value=\""+op.getValue()+"\""+(StringUtils.isNotEmpty(val)&&val.equals(op.getValue())?" checked=\"checked\"":"")+" id=\""+name+i+"\" />";
						 newControl+=element.toString().replace("/>", controlAttribute).replace("title=\""+title+"\"", "title=\""+op.getName()+"\"")+op.getName();
						 i++;
					 }
					 outputDocument.replace(element.getBegin(), element.getEnd(), newControl);
				...
				serMacroFieldValue(element,(map.get(valueKey)==null?"":map.get(valueKey).toString()));
				 ...
				 outputDocument.replace(element.getBegin(), element.getEnd(), newControl);
			 }else if(map.size()>0&&"PULLDOWNMENU".equals(pluginType)){//下拉菜单
				 Object valObj = map.get(valueKey);
				 String val = null;
				 if(valObj!=null){
					 val = valObj.toString();
				 }
				 setPullDownMenuValue(element,val,formFields,key);
			}else{
				if(map.get(valueKey)!=null){
	 ...
	 private void setPullDownMenuValue(Element element,String myvalue,FormFields formFields,String key){
		 OptionGroupManager optionGroupManager= (OptionGroupManager)ContextUtils.getBean("optionGroupManager");
		 String optionGroupId=element.getAttributeValue("optionGroupId");
		if(StringUtils.isNotEmpty(optionGroupId)){
			List<Option> ops=optionGroupManager.getOptionsByGroupCode(optionGroupId);
			replacePullDownMenuHtml(ops,element,myvalue);
		}else{
			if(myvalue!=null)formFields.get(key).setValue(myvalue);
		}
	 }
	...
	//往 FORM里插入值 
	 @SuppressWarnings("unchecked")
	 public String setStandardFieldValue(FormView form,Object entity){
		 FormFields formFields=source.getFormFields();
		 Iterator it = formFields.iterator();
		 FormField formField;
		 Element element;
		 ...
		 while(it.hasNext()){
			...
			dataType = dataType==null ? "" : dataType;
			try {
				if("_opinion_control".equals(valueKey))continue;
				...
				if(value!=null){
					...
					}else if(ControlType.PULLDOWNMENU.toString().equals(pluginType)){//下拉菜单
						 String val = value.toString();
						 setPullDownMenuValue(element,val,formFields,key);
					}else if( "MACRO".equals(pluginType)){//如果是宏控件
						serMacroFieldValue(element,(value==null?"":value.toString()));
					 }else{
						if(!dataType.equals(DataType.COLLECTION.toString())){//不是集合类型
							formFields.get(key).setValue(value.toString());
						}
					}
				}else if(ControlType.PULLDOWNMENU.toString().equals(pluginType)){//下拉菜单值为空
					setPullDownMenuValue(element,"",formFields,key);
				}
			} catch (Exception e) {
	  ...
	  * @param value
	  */
	 private void serMacroFieldValue(Element element,String value){
		 ...
		 if(element.toString().indexOf("<input")>-1){
			 ...
			 String styeContent=element.getAttributeValue("style");
			 ...
			 if(element.getAttributeValue("macroReadonly")==null||"".equals(element.getAttributeValue("macroReadonly"))||"false".equals(element.getAttributeValue("macroReadonly"))||"0".equals(element.getAttributeValue("macroReadonly"))){
				 readonly="";
			 }
			 ...
			 +" macroReadonly=\""+element.getAttributeValue("macroReadonly")+"\""
			 ...
			 setMacroSelectOptions(element,parseMacroSelectControl(datafld),outputDocument,value);
		 }
	 }
	...
	@SuppressWarnings("unchecked")
	public String getFormHtml(FormView form,String formHtml,Map<String,List> queryMap,boolean fieldRight,boolean signatureVisible,String attachInfo,boolean viewable,WorkflowTask task,Long dataId){
		List<Element> inputs=source.getAllElements("input");
		 StringBuilder jsHtml=new StringBuilder("");
			...
			//解析表单中的控件
			parseControls(jsHtml,signatureVisible,attachInfo,viewable,task,null,dataId,false);
			
			formHtml=preViewHtmlMacro(form,task);
			//如果有富文本加入以下js代码
			...
			String timeStr = "";
			try {
				timeStr = sdfTime.format(current);
			}catch (Exception e) {
				e.printStackTrace();
			}
			jsHtml.append("<script type='text/javascript'>__signatureVisible="+signatureVisible+";___initControls("+signatureVisible+","+viewable+",'"+timeStr+"');").append("</script>");
	...
	private void parseControls(StringBuilder jsHtml,boolean signatureVisible,String attachInfo,boolean viewable,WorkflowTask task,Object entity,Long dataId,boolean isStandardForm){
		//如果有下拉菜单控件加入以下js代码
		...
		if(selects!=null&&selects.size()>0&&(isNewWhenUnStardardForm(dataId,isStandardForm)||isNewWhenStardardForm(entity,isStandardForm))){//表单新增时增加选项
			addOption(selects);
		}
		//如果有签章控件
		if(signatureVisible){
	 ...
	 * @return 
	 */
	private boolean isNewWhenStardardForm(Object entity,boolean isStandardForm){
		try {
			if(entity!=null){
				Object value = PropertyUtils.getProperty(entity, "id");
				if(isStandardForm&&value==null)return true;
			}
	 ...
	 * @return 
	 */
	private boolean isNewWhenUnStardardForm(Long dataId,boolean isStandardForm){
		return !isStandardForm&&dataId==null;
	}
	...
	public String getFormHtml(FormView form,String formHtml){
		...
		//如果有自定义列表控件加入以下js代码
		 ...
		 //如果下拉控件选择了选项组作为选项值则解析选项组并添加入select
		...
		try {
		 ...
		 jsHtml.append("<script type='text/javascript'>___initControls(false,false,'"+timeStr+"');").append("</script>");
		 formHtml=preViewHtmlMacro(form,null)+jsHtml;
		 return formHtml; 
	 }
	
	private void addOption(List<Element> selects) {
		OptionGroupManager optionGroupManager= (OptionGroupManager)ContextUtils.getBean("optionGroupManager");
		for (Iterator iterator = selects.iterator(); iterator.hasNext();) {
			Element select = (Element) iterator.next();
			String optionGroupId=select.getAttributeValue("optionGroupId");
			if(StringUtils.isNotEmpty(optionGroupId)){
				List<Option> ops=optionGroupManager.getOptionsByGroupCode(optionGroupId);
				replacePullDownMenuHtml(ops,select,null);
			}
		}
	}
	
	private void replacePullDownMenuHtml(List<Option> ops,Element select,String myvalue){
		StringBuilder sb=new StringBuilder();
		sb.append("<option value=''>请选择</option>");
		for(Option op:ops){
			if(op.getValue().equals(myvalue)){
				sb.append("<option   value='"+op.getValue()+"' selected='selected'>"+op.getName()+"</option>");
			}else{
				sb.append("<option   value='"+op.getValue()+"'>"+op.getName()+"</option>");
			}
		}
		outputDocument.replace(select.getStartTag().getEnd(), select.getEndTag().getBegin(), sb.toString());
	}
	...
	private String getRichTextareaScript(Element element){
	  ...
	  * @return
	  */
	 public String preViewHtmlMacro(FormView form,WorkflowTask task){
		 FormFields formFields=source.getFormFields();
		 ...
		 while(it.hasNext()){
			 ...
			 if(ControlType.RADIO.toString().equals(pluginType)){
				 ...
				 if(StringUtils.isEmpty(updateSign)){
					 ...
					 String name = element.getAttributeValue("name");
					 String optionGroupId = element.getAttributeValue("optionGroupId");
					 String complate = element.getAttributeValue("complate");
					 String newControl="";
					 int i=1;
					 if(StringUtils.isNotEmpty(selectvalues)){
						 String[] values=selectvalues.split(",");
						 ...
						 outputDocument.replace(element.getBegin(), element.getEnd(), newControl);
					 }else if(StringUtils.isNotEmpty(optionGroupId)&&(complate==null||!complate.equals("true"))){
						 OptionGroupManager optionGroupManager= (OptionGroupManager)ContextUtils.getBean("optionGroupManager");
						 List<Option> ops=optionGroupManager.getOptionsByGroupCode(optionGroupId);
						 for(Option op:ops){
							 String controlAttribute=" value=\""+op.getValue()+"\" id=\""+name+i+"\" />";
							 newControl+=element.toString().replace("/>", controlAttribute).replace("title=\""+title+"\"", "title=\""+op.getName()+"\"")+op.getName();
							 i++;
						 }
						 outputDocument.replace(element.getBegin(), element.getEnd(), newControl);
			 ...
			 if(ControlType.TEXTAREA.toString().equals(pluginType==null?"":pluginType.toUpperCase())){
				 ...
				 if("1".equals(rich)){
					 ...
					 String onkeyup=element.getAttributeValue("onkeyup");
					 if(StringUtils.isNotEmpty(onkeyup)){
						 if(onkeyup.contains("calTextareaLen(value,")){
							 outputDocument.replace(element.getBegin(), element.getEnd(), element.toString().replace(onkeyup, "")); 
						 }
					 }
				 }
			 }
			 if(ControlType.OPINION.toString().equals(pluginType)){
				 parseOpinionControl(element,task);
			 }
		 }
		 //（自定义的标签会有乱码）
		 outputDocument.replace(formFields);          
	  ...
	  * @param task
	  */
	 private void parseOpinionControl(Element element,WorkflowTask task){
		 String newControl=element.toString();
		 String controlId=element.getAttributeValue("id");
		 String controlIdOpinion=controlId+"~~opinion";
		 String title=element.getAttributeValue("title");
		 String conclusionSourceValue=element.getAttributeValue("conclusionSourceValue");
		 String titleVisible=element.getAttributeValue("titleVisible");
		 String conclusionPosition=element.getAttributeValue("conclusionPosition");//结论位置
		 String conclusionType=element.getAttributeValue("conclusionType");//结论控件
		 String signRename=element.getAttributeValue("signRename");//签名重命名
		 String dateRename=element.getAttributeValue("dateRename");//日期重命名
		 if(StringUtils.isEmpty(signRename)){
			 signRename = "签名";
		 }
		 if(StringUtils.isEmpty(dateRename)){
			 dateRename = "日期";
		 }
		 if(StringUtils.isEmpty(conclusionPosition)){
			 conclusionPosition = "top";
		 }
		 if(StringUtils.isEmpty(conclusionType)){
			 conclusionType = "radio";
		 }
		 newControl=newControl.replace("<textarea ", "<textarea id=\""+controlIdOpinion+"\" name=\"_iMatrix_opinion_"+controlId+"\" onblur=\"_iMatrix_promptlyOpinionSignAndDate('"+controlId+"',this,'"+ContextUtils.getUserName()+"');\" ");
		 Opinion opi = null;
		 if(task!=null){
			 opi = ApiFactory.getOpinionService().getOpinion(task.getProcessInstanceId(), controlId, task.getCompanyId());
			 if(opi!=null){
				 String controlAttribute=(StringUtils.isEmpty(opi.getOpinion())?"":opi.getOpinion())+"</textarea>";
				 newControl=newControl.replace("</textarea>", controlAttribute);
			 }
		 }
		 
		 String table="<table style='border:0px;'>";
		 int colspan=4;
		 if("false".equals(titleVisible)){//不显示控件名称
			 colspan=3;
		 }
		 //获得结论tr
		 if("top".equals(conclusionPosition)){//结论在意见框的上方
			String conclusionTr = getConclusionTr(conclusionSourceValue,opi,controlId,task,colspan,conclusionPosition,conclusionType);
			if(StringUtils.isNotEmpty(conclusionTr))table+=conclusionTr;
		 }
		 table+="<tr>";
		 if(!"false".equals(titleVisible)){//显示控件名称
			 table+="<td rowspan='3' style='border:0px;'>"+title+"：</td>";
		 }
		 
		 table+="<td colspan='"+(colspan)+"' style='border:0px;'>"+newControl+"</td></tr>";
		 table+="<tr>";
		 if(!"false".equals(titleVisible)){//显示控件名称
			 table+="<td  style='border:0px;'>&nbsp;&nbsp;&nbsp;&nbsp;</td>";
		 }
		 table+="<td  style='border:0px;'>&nbsp;&nbsp;&nbsp;&nbsp;</td>";
		 table+="<td align='right'  style='border:0px;'>"+signRename+"：<input id='_iMatrix_opinionSign_"+controlId+"' name='_iMatrix_opinionSign_"+controlId+"' value='"+getOpinionPartMessage(opi,"opinionSign")+"' readonly='readonly' style='background:none;border:none;width:80px;'/></td>";
		 table+="<td align='right'  style='border:0px;'>"+dateRename+"：<input id='_iMatrix_opinionDate_"+controlId+"' name='_iMatrix_opinionDate_"+controlId+"' value='"+getOpinionPartMessage(opi,"opinionDate")+"' readonly='readonly' style='background:none;border:none;'/></td></tr>";
		 //获得结论tr
		 if("bottom".equals(conclusionPosition)){//结论在意见框的下方
			String conclusionTr = getConclusionTr(conclusionSourceValue,opi,controlId,task,colspan,conclusionPosition,conclusionType);
			if(StringUtils.isNotEmpty(conclusionTr))table+=conclusionTr;
		 }
		 table+="</table>";
		 outputDocument.replace(element.getBegin(), element.getEnd(), table);
	 }
	 /**
	  * 获得结论tr
	  * @param conclusionSourceValue
	  * @param opi
	  * @param controlId
	  * @param task
	  * @param colspan
	  * @return
	  */
	 private String getConclusionTr(String conclusionSourceValue,Opinion opi,String controlId,WorkflowTask task,int colspan,String position,String conclusionType){
		 String conclusionTr = null;
		 if(StringUtils.isNotEmpty(conclusionSourceValue)){
			 List<com.norteksoft.product.api.entity.Option> options=ApiFactory.getSettingService().getOptionsByGroupCode(conclusionSourceValue);
			 String tempOption="";
			 for(com.norteksoft.product.api.entity.Option o:options){
				 if(StringUtils.isEmpty(o.getName())&&StringUtils.isEmpty(o.getValue()))break;
				 if("checkbox".equals(conclusionType)){
				 ...
				 tempOption+=" type='"+conclusionType+"' ";
			 ...
			 if(StringUtils.isNotEmpty(tempOption)){
				 conclusionTr = "<tr>";
				 if("top".equals(position)&&colspan==4){//当在上方且显示名称时，必须加该单元格否则样式有问题，暂不知道什么原因
					 conclusionTr+="<td  style='border:0px;'>&nbsp;&nbsp;&nbsp;&nbsp;</td>";
				 }
				 conclusionTr+="<td colspan='"+colspan+"' style='border:0px;'>"+tempOption+"</td></tr>";
				 return conclusionTr;
			 }
		 }
		 return null;
...
//							html.append("<th style='width:").append(lvSizes[j]).append("px;white-space: nowrap;'>").append(lvTitles[j]).append("</th>");
							html.append("<th style='white-space: nowrap;'>").append(lvTitles[j]).append("</th>");
						}
					}
					html.append("<th style='width:30px;white-space: nowrap;'>&nbsp;&nbsp;操&nbsp;作&nbsp;&nbsp;</th>");
					html.append("</tr></thead><tbody>")
					.append(listControlAddRow(queryResult,"tb_"+elementId,lvSum,lvField,lvCal,fieldRight,lvSize))
	...
	private String packagingDateJs(String function){
	...
	//列表控件的"新增"
	@SuppressWarnings("unchecked")
	private String listControlAddRow(List queryResult,String tbId,String lvSum,String lvField,String lvCal,boolean fieldRight,String lvSize){
		String[] lvFields=lvField.split(",");
		...
		for(int i=0;i<queryResult.size();i++){
			...
			for(int j=0;j<lvFields.length;j++){
				if(!"".equals(lvFields[j])){
					...
					String cellWidth=packagingCustomListCellWidth(lvSizes[j]);
					
					if("DATE".equals(dataType) || "TIME".equals(dataType)){
						if("DATE".equals(dataType)){
							html.append("<input style='width:").append(cellWidth).append("px;' name='listControl_").append(elementId).append("_").append(lvFields[j]).append("' ");
							if(fieldRight){
								dateScript+="___executeDatepicker(\'"+currentId+"\');";
							}
							...
							html.append("<input style='width:").append(cellWidth).append("px;' name='listControl_").append(elementId).append("_").append(lvFields[j]).append("' ");
							if(fieldRight){
								dateScript+="___executeDatetimepicker(\'"+currentId+"\');";
							}
							html.append(" id='").append(currentId).append("' readonly");;
							if(StringUtils.isNotEmpty(value))value=value.replace(":00.0", "");
						}
					}else{
						html.append("<input style='width:").append(cellWidth).append("px;' name='listControl_").append(elementId).append("_").append(lvFields[j])
					...
					html.append(" rowid='").append(rowId).append("'></input>").append("</td>") ;
			...
			if(fieldRight){
				...
				.append("','").append(lvCal).append("','").append(fieldRight).append("','").append(lvSize)
		...
		if(isNeedSum(lvSum)){
			html.append(listControlAddSumRow(tbId,lvSum,totalMap,lvSize));
		}
		if(StringUtils.isNotEmpty(dateScript)){
			html.append("<tr style='display: none;'>");
			html.append("<td colspan='"+(columnNum+1)+"'>");
			html.append(packagingDateJs(dateScript));
			html.append("</td>");
			html.append("</tr>");
		}
		return html.toString();
	...
	private String packagingCustomListCellWidth(String width){
		String cellWidth="80";
		if(StringUtils.isNotEmpty(width)){
			if(Integer.valueOf(width)>80){
				cellWidth= width;
			}
		}
		return cellWidth;
	...
	//列表控件增加"合计"行
	private String listControlAddSumRow(String tbId,String lvSum,Map<String,String> totalMap,String lvSize){
		StringBuilder html=new StringBuilder("");
		...
		for(int i=0;i<lvSums.length;i++){
			if("1".equals(lvSums[i])){
				String cellWidth=packagingCustomListCellWidth(lvSizes[i]);
				html.append("<td><input style='width:").append(cellWidth).append("px;' readonly " );
				String elementId=tbId.substring(tbId.indexOf("_")+1,tbId.length());
				html.append("id='listControlSum_").append(tbId).append("_td_").append(i+1).append("' name='listControlSum_").append(elementId)
				.append("_").append(i+1).append("' value='").append(StringUtils.isNotEmpty(totalMap.get(i+""))?totalMap.get(i+""):"0").append("'");
...
//							 }
	 ...
	 */
	 @SuppressWarnings("unchecked")
	 public List<Collection> getStandardFieldValue(Object entity){
		 String type,dataType,key,valueKey;
		 ...
		 List<Element> lists= source.getAllElements("input");
			for(Element element:lists){
				if( ControlType.STANDARD_LIST_CONTROL.toString() .equals(element.getAttributeValue("pluginType"))){
					...
					type = type==null ? "" : type;
					dataType = dataType==null ? "" : dataType;
					getEntityCollection(valueKey,entity,result);
				}
			}
		 return result;
	 }
	 @SuppressWarnings("unchecked")
	 public List<Collection> getEntityCollection(String valueKey,Object entity,List<Collection> result){
		 try {
	 ...
	 public String getStandardFormHtml(FormView form,String formHtml,Object entity,Collection collection,boolean signatureVisible,String attachInfo,boolean viewable,WorkflowTask task){
		 ...
		 if(entity!=null&&(collection==null||collection.size()<=0)){
			 result=getStandardFieldValue(entity);
		 }
		this.setFormHtml(formHtml);
		List<Element> inputs=source.getAllElements("input");
		StringBuilder jsHtml=new StringBuilder("");
		...
		if(collection!=null&&collection.size()>0){
			...
			parseStandardListControl(collection,entity,0,jsHtml);
			...
			if(isHasAnyTag(ControlType.STANDARD_LIST_CONTROL.toString(),inputs)){
				...
				parseStandardListControl(new ArrayList(),entity,0,jsHtml);
			}
		}else{
			int i=0;
			for(Collection queryResult:result ){
				if(queryResult.size()<=0){
					if(isHasAnyTag(ControlType.STANDARD_LIST_CONTROL.toString(),inputs)){
						jsHtml.append("<script type='text/javascript'>initStandardListControl();</script>");
						parseStandardListControl(queryResult,entity,i,jsHtml);
					}
				}else{
		...
		//解析表单中的控件
		parseControls(jsHtml,signatureVisible,attachInfo,viewable,task,entity,null,true);
		 ...
		 if(imageUploads!=null){
			 ...
			 for(Element element:result){
				 ...
				 if(StringUtils.isEmpty(orgUploadUrl)){
					 uploadZones.append("<table class='form-table-border-left' style='border:0px;width:350px;' id='attachUpload_"+i+"'")
...
//					 }else
					 ...
					 uploadZones.append("<tbody id='attachUpload_"+i+"_body'>");
					 ...
					 if(ControlType.ATTACH_UPLOAD.toString().equals(pluginType)){
						 ...
						 for(FormAttachment att:attachs){
							 uploadZones.append("<tr id='"+att.getId()+"' class='form-upload-attach-tr'>")
							 .append("<td  class='form-upload-attach-td' style='border:none;'>").append("<a href='#' onclick='____downloadFile(\""+att.getId()+"\",\""+pluginType+"\");'>").append(att.getFileName());
							 if(!viewable){
								 uploadZones.append("</a><a href='#' onclick='____deleteFile(\""+att.getId()+"\",\"attachUpload_"+i+"\",\""+pluginType+"\");'><span class='ui-icon ui-icon-close' style='float:right;'></span></a>");
							 }else{
								 uploadZones.append("</a>");
							 }
							 uploadZones.append("</td>");
							 uploadZones.append("</tr>");
						 }
						 ...
						 for(FormAttachment att:attachs){
							 ...
							 .append("<td><img src='").append(SystemUrls.getSystemPageUrl(ContextUtils.getSystemCode())).append("/").append(att.getFilePath()).append("' width='").append(imageWidth).append("' height='").append(imageHeight).append("'/></td>");
							 if(viewable){
	...
	//标准列表控件的初始化
	 @SuppressWarnings("unchecked")
	 private void parseStandardListControl(Collection queryResult,Object entity,int index,StringBuilder jsHtml){
		 List<Element> lists= source.getAllElements("input");
		 int n=0;
	 	for(Element element:lists){
	 		if( "STANDARD_LIST_CONTROL".equals(element.getAttributeValue("pluginType")) ){//如果是宏控件
	 			if(n==index||queryResult.size()==0){
	 				StringBuilder html=new StringBuilder("");
	 				String listViewCode=element.getAttributeValue("listViewCode");
	 				String elementId=element.getAttributeValue("id");
	 				html.append("<table id='tb_")
	 				.append(elementId).append("' newId='1'></table>");
	 				outputDocument.replace(element.getStartTag().getEnd()-1, element.getStartTag().getEnd(), ">"+html.toString() );
	 				
	 				jsHtml.append(getJqGridScript("tb_"+elementId,listViewCode,queryResult,entity));
	 			}
 				n++;
	 		}
	 	}
	 ...
	 private String getJqGridScript(String tableId,String listViewCode,Collection queryResult,Object entity){
		 ...
		 String data=getJsonData(queryResult,columns,tableId);
		...
		script.append("data:").append(data).append(",");
		 ...
		 .append("if(mydata[i].id=='"+tableId+"_new_0'){")
		 .append("_controlGridEditColumns('"+tableId+"');")//动态控制某些列不能编辑
		 .append("jQuery('#").append(tableId).append("').jqGrid('editRow','"+tableId+"_new_0',true,function(){var id='"+tableId+"_new_0';"+getEditFunScript(columns,tableId)+"},function(){},'',{},function(){hasEdit=false;$editRowSave('"+tableId+"_new_0','").append(tableId).append("');lastsel=0;},function(){},function(){hasEdit=false;$editRowRestore('"+tableId+"_new_0','").append(tableId).append("');lastsel=0;jQuery('#").append(tableId).append("').jqGrid('delRowData','"+tableId+"_new_0'); _add_row('").append(tableId).append("'); }")
		 .append(");}")
		 .append("}");
		 ...
		 script.append(getEditFunForJs(columns,tableId));
		 
		 script.append("</script>");
		 return script.toString();
	 }
	 
	 public String getEditFunForJs(List<ListColumn> columns,String tableId){//jqGrid.custom.js引用的编辑行的脚本
		 StringBuilder script = new StringBuilder();
		 script.append("function editFun"+tableId+"(id,tableId){");
		 script.append(getEditFunScript(columns,tableId));
		 script.append("}");
		 return script.toString();
	 }
	 
	 public  String getEditFunScript(List<ListColumn> columns,String tableId){
		 StringBuilder script = new StringBuilder();
			 ...
			 if(col.getTableColumn()!=null){
				 ...
				 if(StringUtils.isNotEmpty(col.getControlValue())&& !col.getControlValue().equals("CUSTOM")){//如果编辑时控件是”自定义“的，则不要加事件
					 ...
					 if(col.getControlValue().equals("MULTISELECT")){
						 if(col.getTableColumn()!=null){
							 script.append("jQuery(\"").append(tag).append("[id='\"+").append("id").append("+\"_").append(col.getTableColumn().getName()).append("']\")").append(".multiselect({checkAllText:'全选',uncheckAllText:'清除',noneSelectedText:'请选择',selectedList:4});");
						 }
					 }
					 ...
					 if(col.getTableColumn().getDataType()==DataType.DATE){
						 ...
						 .append("onSelect:function(dateText, inst){$dateOnSelect({rowid:").append("id").append(",currentInputId:").append("id+'_").append(col.getTableColumn().getName()).append("',dateText:dateText,gridId:'"+tableId+"'});},")
						.append("onChangeMonthYear:function(){$dateOnChangeMonthYear({rowid:").append("id").append(",currentInputId:").append("id+'_").append(col.getTableColumn().getName()).append("',gridId:'"+tableId+"'});},")
						.append("onClose:function(){$dateOnClose({rowid:").append("id").append(",currentInputId:").append("id+'_").append(col.getTableColumn().getName()).append("'});}")
						 ...
						 .append("onSelect:function(dateText, inst){$dateOnSelect({rowid:").append("id").append(",currentInputId:").append("id+'_").append(col.getTableColumn().getName()).append("',dateText:dateText,gridId:'"+tableId+"'});},")
						.append("onChangeMonthYear:function(){$dateOnChangeMonthYear({rowid:").append("id").append(",currentInputId:").append("id+'_").append(col.getTableColumn().getName()).append("',gridId:'"+tableId+"'});},")
						.append("onClose:function(){$dateOnClose({rowid:").append("id").append(",currentInputId:").append("id+'_").append(col.getTableColumn().getName()).append("',gridId:'"+tableId+"'});}")
						 .append("});");
						 ...
						 for(String event:events){
							 if("ONCLICK".equals(event)){
								 script.append("jQuery(\"").append(tag).append("[id='\"+").append("id").append("+\"_").append(col.getTableColumn().getName()).append("']\")").append(".attr('readonly','readonly');");
								 script.append("jQuery(\"").append(tag).append("[id='\"+").append("id").append("+\"_").append(col.getTableColumn().getName()).append("']\")").append(".click(function(){").append(columName).append("Click({rowid:").append("id").append(",currentInputId:").append("id+'_").append(col.getTableColumn().getName()).append("',gridId:'"+tableId+"'});});");
							 }else if("ONCHANGE".equals(event)){
								 script.append("jQuery(\"").append(tag).append("[id='\"+").append("id").append("+\"_").append(col.getTableColumn().getName()).append("']\")").append(".change(function(){").append(columName).append("Change({rowid:").append("id").append(",currentInputId:").append("id+'_").append(col.getTableColumn().getName()).append("',gridId:'"+tableId+"'});});");
							 }else if("ONDBLCLICK".equals(event)){
								 script.append("jQuery(\"").append(tag).append("[id='\"+").append("id").append("+\"_").append(col.getTableColumn().getName()).append("']\")").append(".dblclick(function(){").append(columName).append("Dblclick({rowid:").append("id").append(",currentInputId:").append("id+'_").append(col.getTableColumn().getName()).append("',gridId:'"+tableId+"'});});");
							 }else if("BLUR".equals(event)){
								 script.append("jQuery(\"").append(tag).append("[id='\"+").append("id").append("+\"_").append(col.getTableColumn().getName()).append("']\")").append(".blur(function(){").append(columName).append("Blur({rowid:").append("id").append(",currentInputId:").append("id+'_").append(col.getTableColumn().getName()).append("',gridId:'"+tableId+"'});});");
							 }else if("KEYUP".equals(event)){
								 script.append("jQuery(\"").append(tag).append("[id='\"+").append("id").append("+\"_").append(col.getTableColumn().getName()).append("']\")").append(".keyup(function(){").append(columName).append("Keyup({rowid:").append("id").append(",currentInputId:").append("id+'_").append(col.getTableColumn().getName()).append("',gridId:'"+tableId+"'});});");
							 }else if("KEYDOWN".equals(event)){
								 script.append("jQuery(\"").append(tag).append("[id='\"+").append("id").append("+\"_").append(col.getTableColumn().getName()).append("']\")").append(".keydown(function(){").append(columName).append("Keydown({rowid:").append("id").append(",currentInputId:").append("id+'_").append(col.getTableColumn().getName()).append("',gridId:'"+tableId+"'});});");
							 }else if("FOCUS".equals(event)){
								 script.append("jQuery(\"").append(tag).append("[id='\"+").append("id").append("+\"_").append(col.getTableColumn().getName()).append("']\")").append(".focus(function(){").append(columName).append("Focus({rowid:").append("id").append(",currentInputId:").append("id+'_").append(col.getTableColumn().getName()).append("',gridId:'"+tableId+"'});});");
							 }else if("MOUSEDOWN".equals(event)){
								 script.append("jQuery(\"").append(tag).append("[id='\"+").append("id").append("+\"_").append(col.getTableColumn().getName()).append("']\")").append(".mousedown(function(){").append(columName).append("Mousedown({rowid:").append("id").append(",currentInputId:").append("id+'_").append(col.getTableColumn().getName()).append("',gridId:'"+tableId+"'});});");
							 }else if("MOUSEUP".equals(event)){
								 script.append("jQuery(\"").append(tag).append("[id='\"+").append("id").append("+\"_").append(col.getTableColumn().getName()).append("']\")").append(".mouseup(function(){").append(columName).append("Mouseup({rowid:").append("id").append(",currentInputId:").append("id+'_").append(col.getTableColumn().getName()).append("',gridId:'"+tableId+"'});});");
							 }
						 }
					 }else if(col.getControlValue().contains("SELECT_TREE")){
						 //SELECT_TREE,applyLoginName,COMPANY,true
						 String values = col.getControlValue();
						 String[] vals = values.split(",");
						 String hiddenField = vals[1];
						 String treeType = vals[2];
						 String isMutiply = vals[3];
						 script.append("jQuery(\"").append(tag).append("[id='\"+").append("id").append("+\"_").append(col.getTableColumn().getName()).append("']\")").append(".attr('readonly','readonly');");
						 script.append("jQuery(\"").append(tag).append("[id='\"+").append("id").append("+\"_").append(col.getTableColumn().getName()).append("']\")").append(".click(function(){formGridTree({rowid:").append("id").append(",currentInputId:").append("id+'_").append(col.getTableColumn().getName()).append("',hiddenField:'").append(hiddenField).append("',treeType:'").append(treeType).append("',isMutiply:'").append(isMutiply).append("',useType:true").append("});});");
					 }
				 }
			 }
		 }
	 ...
	 private String editClick(String tableId){
		 StringBuilder script=new StringBuilder();
		 script.append("function editClick(id,tableId){")
		 .append("saveRowForOtherTable();")
		 .append("if(!hasEdit){")
		 .append("if(id){")
		 .append("_controlGridEditColumns(tableId);")//动态控制某些列不能编辑
		 .append("customGridEditRow(id,tableId);")
		 .append("lastsel=id;")
		 .append("$editClickCallback(id,tableId);")
		 .append("}")
		 .append("}")
		 .append("}");
		 return script.toString();
	 ...
	 public String getValueSet(ListColumn col,Object entity,String dataTableName ){
		 ...
		 if(column!=null &&column.getDataType()!=DataType.BOOLEAN && col.getControlValue()!=null && !col.getControlValue().equals("MULTISELECT")){
			 opitions.append("'':'")
			 .append(textProvider.getText("list.cloumn.default.value"))
			 .append("'").append(",");
		 }
		 if(StringUtils.isNotEmpty(valueSet)){
		 			...
		 			if(valueSet.contains(":")){
		 				...
		 				for(String val:vals){
		 					...
		 					if(strArr.length>1){
		 						...
		 						if(strArr[1].contains("'")||strArr[1].contains("\"")){
		 							String opVal = strArr[1].substring(strArr[1].indexOf("'")+1,strArr[1].lastIndexOf("'"));
		 							opitions.append("'").append(MmsUtil.getNameToi18n(opVal)).append("'");
		 						}else{
		 							opitions.append("'").append(MmsUtil.getNameToi18n(strArr[1])).append("'");
		 						}
		 						if(i<vals.length){
		 							opitions.append(",");
		 						}
		 				...
		 				if(group!=null){
		 					...
		 					for(com.norteksoft.product.api.entity.Option op:ops){
		 						...
		 						.append("':'")
		 						.append(getInternation(op.getName()))
		 						.append("'");
		 						if(i<ops.size()){
		 							opitions.append(",");
		 						}
	  ...
	  */
	 @SuppressWarnings("unchecked")
	 public String getJsonData(Collection queryResult,List<ListColumn> columns,String tableId){
		 StringBuilder jsonData=new StringBuilder();
		 jsonData.append("[");
		 if(queryResult==null||queryResult.size()<=0){
			 jsonData.append("{\"id\":\""+tableId+"_new_0\"}"); 
		 }else{
				 ...
				 if(tc!=null&&tc.getDataType()!=DataType.COLLECTION){
					 ...
					 if(val!=null){
						 if(tc.getDataType()==DataType.DATE){
							 ...
							 NumberFormat nf=new DecimalFormat(getDecimals());
							 val=nf.format(val);
	 ...
	 private String getGridCompleteScript(String tableId,List<ListColumn> columns,boolean isSubSystem,String webCtx){
		 ...
		 .append("var cl = ids[i];")
		 .append("ae = \"<a href='#' class='small-button-bg' acttype='add' onclick=\\\"myAddRow('1','\"+cl+\"','").append(tableId).append("');\\\"").append("><span title='增加' class='ui-icon ui-icon-plusthick'></span></a>\"; ")
		 .append("if(deleteUrl!=''){")
		 .append("de = \"<a href='#' class='small-button-bg' acttype='del' onclick=\\\"deleteFormTableData('").append(tableId).append("','\"+cl+\"','\"+deleteUrl+\"','").append(isSubSystem).append("','").append(webCtx).append("');\\\"").append("><span title='删除' class='ui-icon  ui-icon-minusthick'></span></a>\"; ")
		 .append("}else{")
		 .append("de = \"<a href='#' class='small-button-bg' acttype='del' onclick=\\\"$deleteFormTableData('").append(tableId).append("','\"+cl+\"'").append(");\\\"").append("><span title='删除' class='ui-icon  ui-icon-minusthick'></span></a>\"; ")
		 .append("}")
		 .append("edit = \"<a href='#' class='small-button-bg' acttype='edit' onclick=\\\"editClick('\"+cl+\"','").append(tableId).append("');\\\"").append("><span title='修改' class='ui-icon  ui-icon-pencil'></span></a>\"; ");
	 ...
	 private String getOnSelectRowScript(String tableId){
		 ...
		 .append("if(hasEdit){if(id && id!=lastsel){saveRowForOtherTable();lastsel=0;}}")
		 .append("}");
		 return sb.toString();
	 }
	 /**
	 ...
	 private String getDecimals(){
		 String format="#.###";
		 String decimals = PropUtils.getProp("iMatrixGrid.default.decimals");
		 if(StringUtils.isNotEmpty(decimals) && decimals.matches("^-?\\d+$")){
			 format="#";
	...
	public String updateColumnAmount(FormView formView,Integer columnAmount) {
		...
		result.addAll(inputLists);
		result.addAll(textareaLists);
		result.addAll(selectLists);
		result.addAll(imgLists);
		html.append("<table  class=\"form-table-border-left\"  border=\"1\">");
		html.append("<tbody>");
		html.append(createColumnTable(result,columnAmount/2));//columnAmount/2:2/2表示1个字段，4/2表示2个字段，6/2表示3个字段s
		html.append("</tbody>");
		html.append("</table>");
		html.append("<br />");
		return html.toString();
	...
	private String createColumnTable(List<Element> elements,Integer columnAmount){
		...
		for(Element element:elements){
			...
			html.append("<td>").append(element.toString()).append("</td>");
