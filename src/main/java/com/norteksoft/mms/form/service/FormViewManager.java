...
public class FormViewManager {
	...
	public void deleteFormView(Long id){
		...
		// 软删除表单页面
		List<ModulePage> formPages=modulePageDao.getModulePagesByFormViewId(id);
			...
			if(dataTable!=null){
				...
				for(ListView listView:listViews){
					if(formViewList.size()==1){
						...
						//软删除列表页面
						List<ModulePage> listPages=modulePageDao.getModulePagesByListViewId(listView.getId());
	...
	public void rebackFormView(Long id){
		FormView view = formViewDao.get(id);
		// 还原表单页面
		List<ModulePage> pages=modulePageDao.getModulePagesByFormViewId(id);
	...
	public List<FormControl> getControlsByCodeAndVersion(Long companyId,String code,Integer version) {
		...
		return getFormHtmlParser().getControls(formView.getHtml(),true);
	}
	...
	public List<FormControl> getControls(FormView formView) {
		...
		List<FormControl> list = getFormHtmlParser().getControls(formView.getHtml(),false);
		return list;
	}
	/**
	 * 获得该表单的控件列表（按控件在html代码中出现先后排序）
...
//				sql.append("from ").append(childForm.getDataTable().getName()).append(" t")
		...
		return formHtmlParser.getFormHtml(form,formHtml,queryMap,fieldRight,signatureVisible,attachInfo,viewable,task,dataId);
	}
	...
	public Page<Object> getDataExcutionSql(Page<Object> page,String html,String controlId,Map<String, String[]> parameterMap,List<String[]> properties){
		...
		if(StringUtils.isNotEmpty(conditionSql.trim())){
			generalDao.findPageBySql(getExecutionSql(dataTable,conditionSql,properties),page, (Object[])conditions[1]);
		}else{
...
//		page = jdbcDao.excutionSql(page,result,conditionSql);
	...
	private Object[] getConditionSql(DataTable dataTable,Map<String, String[]> parameterMap){
		...
		String key;
		String[] value = null;
//		Object[] objs=new Object[tableColumns.size()+1];
...
//								objs[i]=false;
		...
		if(StringUtils.isNotEmpty(conditionSql.toString())&&!conditionSql.toString().equals(" ")){
			conditionSql.append(" and company_id=?");
		}else{
			conditionSql.append(" company_id=?");
		}
		Object[] objs=new Object[objList.size()+1];
		for(int i=0;i<objList.size();i++){
			objs[i]=objList.get(i);
		}
		objs[objList.size()]=ContextUtils.getCompanyId();
	...
	public String changeFormState(List<Long> formIds, Long menuId){
		...
		String[] messages = new String[]{draftToEn+"",enToDis+"",disToEn+""};
		Struts2Utils.getText("formmanager.changeFormState", messages);
		return Struts2Utils.getText("formmanager.changeFormState", messages);
	}
...
//			}
	...
	public String getHtml(FormView form,String html,Long dataId,boolean fieldRight,Object entity,Collection collection, boolean signatureVisible,String attachInfo,boolean viewable,WorkflowTask task){
		...
		if(!hasFill){
			if(form.getStandard()){
				html = formHtmlParser.setStandardFieldValue(form,entity);
			}else{
				if(dataId!=null){
					//主表单赋值
			 ...
			 for (ListView listView : listViews) {
				 ...
				 for (ListColumn listColumn : columns) {
					 if(listColumn.getTableColumn()!=null&&listColumn.getTableColumn().getAlias()!=null){
						 if(!listColumn.getTableColumn().getAlias().equals(getInternation(listColumn.getHeaderName()))){
	...
	public WorkflowDefinition getWfd(Long wfd_Id) {
		return workflowDefinitionDao.get(wfd_Id);
	}
	/**
	 * 根据选项组编码查询选项
	 * (flex有调用)
	 */
	public List<Option> getOpinionConclusion(String optionGroupCode,Long companyId){
		ThreadParameters parameters = new ThreadParameters(companyId);
		ParameterUtils.setParameters(parameters);
		return ApiFactory.getSettingService().getOptionsByGroupCode(optionGroupCode);
	}
