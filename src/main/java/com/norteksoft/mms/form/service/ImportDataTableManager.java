...
public class ImportDataTableManager extends DefaultDataImporterCallBack{
	...
	public String saveSingleRow(String[] rowValue,ImportDefinition importDefinition) {
		...
		}else if("MMS_GENERATE_SETTING".equals(importDefinition.getCode())){
			saveGenerateSetting(rowValue,importDefinition);
		}
		return "";
	}

	private void saveGenerateSetting(String[] rowValue,ImportDefinition importDefinition) {
		...
		if(rowValue.length>=2&&("false".equals(rowValue[1])||"0".equals(rowValue[1]))){
			generateSetting.setEntitative(false);
		}else{
			generateSetting.setEntitative(true);
		}
		if(rowValue.length>=3&&("true".equals(rowValue[2])||"1".equals(rowValue[2]))){
			generateSetting.setFlowable(true);
		}else{
			generateSetting.setFlowable(false);
		}
		if(rowValue.length>=4){
			generateSetting.setWorkflowCode(rowValue[3]);
		}else{
			generateSetting.setWorkflowCode(null);
		}
		if(rowValue.length>=5){
			if("FOUR_COLUMN".equals(rowValue[4])){
				generateSetting.setTemplateEnum(TemplateEnum.FOUR_COLUMN);
			}else if("SIX_COLUMN".equals(rowValue[4])){
				generateSetting.setTemplateEnum(TemplateEnum.SIX_COLUMN);
			}else{
				generateSetting.setTemplateEnum(TemplateEnum.TWO_COLUMN);
			}
		}else{
			generateSetting.setTemplateEnum(TemplateEnum.TWO_COLUMN);
		}
