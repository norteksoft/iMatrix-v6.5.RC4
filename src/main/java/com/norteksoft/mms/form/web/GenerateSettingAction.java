...
public class GenerateSettingAction extends CrudActionSupport<GenerateSetting> {
	...
	public String list() throws Exception {
		...
		ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("mms.formManager"), 
				ApiFactory.getBussinessLogService().getI18nLogInfo("mms.generateCodePage"), 
	...
	public String save() throws Exception {
		...
		ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("mms.formManager"), 
				ApiFactory.getBussinessLogService().getI18nLogInfo("mms.saveCodeConfigure"), 
				ContextUtils.getSystemId("mms"));
	...
	public String generateCodeMenuTree() throws Exception{
		...
		for(Menu menu :menus){
			if(menu.getChildren()==null||menu.getChildren().isEmpty()){
				tree.append(JsTreeUtils.generateJsTreeNodeNew(menu.getId().toString(), "close",  getInternation(menu.getName()), "")).append(",");
			}else{
				tree.append(JsTreeUtils.generateJsTreeNodeNew(menu.getId().toString(), "close",  getInternation(menu.getName()), childMenu(menu.getChildren()),"")).append(",");
			}
		}
	...
	private String childMenu(List<Menu> menus){
		...
		for(Menu menu :menus){
			if(menu.getChildren()==null||menu.getChildren().isEmpty()){
				tree.append(JsTreeUtils.generateJsTreeNodeNew(menu.getId().toString(), "",  getInternation(menu.getName()), "")).append(",");
			}else{
				tree.append(JsTreeUtils.generateJsTreeNodeNew(menu.getId().toString(), "close",  getInternation(menu.getName()), childMenu(menu.getChildren()),"")).append(",");
			}
		}
