...
public class CommonManager extends WorkflowManagerSupport<FormFlowable> implements FormFlowableDeleteInterface,RetrieveTaskInterface,EndInstanceInterface  {
	 ...
	 */
	@Transactional
	public Page<Object> list(Page<Object> page, ListView listView){
	 ...
	 */
	@Transactional
	public Object getDateById(FormView formView, Long id){
	...
	public Long save(Map<String,String[]> parameter){
		...
		FormView form =  modulePage.getFormView();
		
		return saveDate(parameter, form);
	}
	@Transactional
	public Long saveTaskDate(Map<String,String[]> parameter, FormView form,Long taskId){
		Long id =saveDate(parameter,form);
		ApiFactory.getTaskService().saveOpinionControl(taskId);
		return id;
	}
	 ...
	 */
	@Transactional
	public String delete(ListView listView, List<Long> ids){
		if(ids != null && !ids.isEmpty()){
			for(Long id : ids){
				Object obj=commonDao.getDateById(listView.getDataTable().getName(),id);
				String instanceId = (String)((Map)obj).get("instance_id");
				com.norteksoft.product.api.entity.WorkflowInstance wi= null;
				if(instanceId!=null)wi = ApiFactory.getInstanceService().getInstance((String)((Map)obj).get("instance_id"));
				if(wi==null){
					commonDao.delete(listView.getDataTable().getName(), id);
				}else{
					ApiFactory.getInstanceService().deleteInstance(instanceId);
				}
			}
		}
	...
	public Long startWorkflow(Map<String, String[]> parameter){
		...
		if(def!=null){
			...
			taskId = task.getId();
			parameter.put("taskId",  new String[]{taskId.toString()});
		}
		if(dataId!=null){
			String[]  customListControlVals= parameter.get("customListControlVals");
			if(customListControlVals!=null){
	...
	public CompleteTaskTipType submitTask(Map<String,String[]> parameter){
		...
		if(parameter.get("transactor")!=null&&parameter.get("transactor").length>0){
			...
			CompleteTaskTipType type = ApiFactory.getTaskService().completeInteractiveWorkflowTask(taskId, "", tr);
			return type;
		}
		WorkflowTask task = ApiFactory.getTaskService().getTask(taskId);
		TaskProcessingResult transact = TaskProcessingResult.valueOf(parameter.get("transact")[0]);
		CompleteTaskTipType type = ApiFactory.getTaskService().completeWorkflowTask(task, transact);
		return type;
	}
	...
	public List<String> opinionRightByTask(WorkflowDefinition definition){
		...
		TaskPermission permission = ApiFactory.getPermissionService().getActivityPermission(definition.getCode(),definition.getVersion(),ContextUtils.getSystemId("mms"));
		if(permission.getOpinionEditable()){
			result.add("edit");
		}
		if(permission.getOpinionRequired()){
...
//	}
     ...
     */
	  ...
	  * 发起前自动填写字段
	  */
	 public String fillEntityByDefinition(Map data,String wfDefinationCode, Integer version,Long... systemId){
		 return ApiFactory.getFormService().fillEntityByDefinition(data, wfDefinationCode, version,systemId);
	 }
	 /**
	  * 办理前自动填写字段
	  */
	 public String fillEntityByTask(Map data,Long taskId){
		 return ApiFactory.getFormService().fillEntityByTask(data, taskId);
	 }
