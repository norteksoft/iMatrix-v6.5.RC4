...
public class ButtonDao extends HibernateDao<Button, Long> {
	...
	public List<Button> findRebackButtonByModulePage(Long pageId) {
		return this.find("from Button btn where (btn.companyId=? and btn.modulePage.id=? and btn.toPage.listView.deleted=?) or (btn.toPage.id=?)", ContextUtils.getCompanyId(),pageId,false,pageId);
	}
