...
public class ModulePageDao extends HibernateDao<ModulePage, Long> {
	...
	public List<ModulePage> getModulePagesByFormViewId(Long formViewId) {
		...
		return this.find("from ModulePage v where v.formView.id=? and v.companyId=?", formViewId,ContextUtils.getCompanyId());
	}
	/**
	 * 根据页面视图id查询本公司本系统下的
	 */
	public List<ModulePage> getModulePagesByListViewId(Long listViewId) {
		...
		return this.find("from ModulePage v where v.listView.id=? and v.companyId=?", listViewId,ContextUtils.getCompanyId());
	}
