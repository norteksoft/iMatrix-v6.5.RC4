...
public class ModulePageManager {
	...
	public void saveModulePage(ModulePage modulePage){
		if(modulePage.getViewType().equals(ViewType.FORM_VIEW)){
			if(modulePage.getFormView()!=null){
				modulePage.setFormView(formViewDao.get(modulePage.getFormView().getId()));
			}
		}else{
			if(modulePage.getListView()!=null){
				modulePage.setListView(listViewDao.get(modulePage.getListView().getId()));
			}
		}
	...
	public String deleteModulePages(String pageIds){
		...
		if(pageIds!=null){
			...
			String successStr=canNum!=0?canNum+Struts2Utils.getText("pageManager.deleteSuccess"):"";
			String errorStr=(ids.length-canNum)!=0?(ids.length-canNum)+Struts2Utils.getText("pageManager.deleteFail"):"";
	...
	public String enableSet(String pageIds){
		...
		String enabelStr=enabelCount!=0?enabelCount+Struts2Utils.getText("pageManager.startToForbidden"):"";
		String disabelStr=disabelCount!=0?disabelCount+Struts2Utils.getText("pageManager.forbiddenToStart"):"";
		String draftStr=draftCount!=0?draftCount+Struts2Utils.getText("pageManager.draftToStart"):"";
		return enabelStr+disabelStr+draftStr;
	}
	
	public List<ModulePage> getModulePagesByListViewId(Long listViewId){
		return modulePageDao.getModulePagesByListViewId(listViewId);
	}
	public List<ModulePage> getModulePagesByFormViewId(Long formViewId){
		return modulePageDao.getModulePagesByFormViewId(formViewId);
	}
