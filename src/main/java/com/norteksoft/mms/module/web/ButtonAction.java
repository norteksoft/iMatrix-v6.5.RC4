...
public class ButtonAction extends CrudActionSupport<Button> {
	...
	public String delete() throws Exception {
		...
		ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("mms.buttonManagement"), 
				ApiFactory.getBussinessLogService().getI18nLogInfo("mms.deleteButton"), 
	...
	public String list() throws Exception {
		...
		ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("mms.buttonManagement"), 
				ApiFactory.getBussinessLogService().getI18nLogInfo("mms.buttionList"), 
				ContextUtils.getSystemId("mms"));
		return "button";
	}
	...
	public String saveButtons() throws Exception {
		...
		ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("mms.buttonManagement"), 
				ApiFactory.getBussinessLogService().getI18nLogInfo("mms.saveButton"), 
				ContextUtils.getSystemId("mms"));
		addSuccessMessage(Struts2Utils.getText("form.save.success"));
		return list();
	}
	@Action("button-selectPage")
	...
	private String defaultTree(Long menuId){
		...
		List<Menu> children=menuManager.getChildrenEnabledMenus(menu.getId(),systemIds);
		if(children==null||children.size()<=0){
			List<ModulePage> modulePages=modulePageManager.getEnableModulePagesByMenuId(menu.getId());
			if(modulePages.size()>0){
				node = new ZTreeNode(menu.getId().toString()+";"+getInternation(menu.getName())+";menu","0",getInternation(menu.getName())+Struts2Utils.getText("menuManager.menu"), "true", "false", "", "", "folder", "");
				treeNodes.add(node);
				childModulePage(modulePages,menu.getId().toString()+";"+getInternation(menu.getName())+";menu",treeNodes);
			}else{
				node = new ZTreeNode(menu.getId().toString()+";"+getInternation(menu.getName())+";menu","0",getInternation(menu.getName())+Struts2Utils.getText("menuManager.menu"), "false", "false", "", "", "folder", "");
				treeNodes.add(node);
			}
		}else{
			node = new ZTreeNode(menu.getId().toString()+";"+getInternation(menu.getName())+";menu","0",getInternation(menu.getName())+Struts2Utils.getText("menuManager.menu"), "true", "false", "", "", "folder", "");
			treeNodes.add(node);
			childModulePage(modulePageManager.getModulePagesByMenuId(menu.getId()),menu.getId().toString()+";"+getInternation(menu.getName())+";menu",treeNodes);
			childMenu(children,menu.getId().toString()+";"+getInternation(menu.getName())+";menu",treeNodes,systemIds);
		}
	 ...
	 * 递归菜单父子关系，形成tree
	 */
	private void childMenu(List<Menu> menus,String pid,List<ZTreeNode> treeNodes,List<Long> systemIds){
		...
		for(Menu menu :menus){
			...
			if(children==null||children.size()<=0){
				List<ModulePage> modulePages=modulePageManager.getEnableModulePagesByMenuId(menu.getId());
				if(modulePages.size()>0){
					node = new ZTreeNode(menu.getId().toString()+";"+getInternation(menu.getName())+";menu",pid,getInternation(menu.getName())+Struts2Utils.getText("menuManager.menu"), "true", "false", "", "", "folder", "");
					treeNodes.add(node);
					childModulePage(modulePages,menu.getId().toString()+";"+getInternation(menu.getName())+";menu",treeNodes);
				}else{
					node = new ZTreeNode(menu.getId().toString()+";"+getInternation(menu.getName())+";menu",pid,getInternation(menu.getName())+Struts2Utils.getText("menuManager.menu"), "false", "false", "", "", "folder", "");
					treeNodes.add(node);
				}
			}else{
				node = new ZTreeNode(menu.getId().toString()+";"+getInternation(menu.getName())+";menu",pid,getInternation(menu.getName())+Struts2Utils.getText("menuManager.menu"), "true", "false", "", "", "folder", "");
	...
	private void childModulePage(List<ModulePage> modulePages,String pid,List<ZTreeNode> treeNodes){
		...
		for(ModulePage modulePage :modulePages){
			node = new ZTreeNode(modulePage.getId().toString()+";"+modulePage.getName()+";view",pid,modulePage.getName()+Struts2Utils.getText("menuManager.page"), "false", "false", "", "", "folder", "");
			treeNodes.add(node);
		}
	}
