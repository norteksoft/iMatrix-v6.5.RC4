...
public class MmsUtil {
	...
	public ExportData getExportData(Page<?> page,String listCode){
		...
		if(columns!=null&&columns.size()>0){
			...
			exportData.setHead(setHeadNameI18n(head));
	...
	private static List<Object> setHeadNameI18n(List<Object> head){
		List<Object> result = new ArrayList<Object>();
		for(Object obj:head){
			if(obj!=null){
				result.add(getNameToi18n(obj.toString()));
			}else{
				result.add(obj);
			}
		}
	...
	public static String getNameToi18n(String name){
		if(name==null)return name;
		if(name.contains("${")&&name.contains("}")){
			name = name.substring(name.indexOf("${")+2,name.indexOf("}"));
			name = Struts2Utils.getText(name);
	...
	public Workbook createReport(Page<?> page, String listCode,String excelName, ExcelExportEnum excelEdition) {
		ListColumnManager listColumnManager = (ListColumnManager) ContextUtils.getBean("listColumnManager");
		if(listColumnManager==null){
			log.debug("ListColumnManager不能为null");
			throw new RuntimeException("ListColumnManager不能为null");
		...
		int colIndex = 0;
		//表头
		List<Object> headsData = null;
		List<ListColumn> columns=listColumnManager.getExportColumnsByCode(listCode);
		if(columns!=null&&columns.size()>0){
			String rowDatas=PageUtils.pageToRowData(page, columns);
			headsData=listColumnManager.getExportHeadnameByCode(listCode);
			if(headsData!=null){
			...
			rowDatas=rowDatas.replaceAll("\\\\", "\\\\\\\\").replaceAll("\t", "").replaceAll("\r\n", "").replaceAll("\n", "");
			getBodyData(rowDatas,columns,sheet);
		}
		return wb;
	}
	
	//excel设置样式
	private CellStyle setExcelStyle(Workbook wb){
		CellStyle style = wb.createCellStyle();
		Font font = wb.createFont(); 
		return produceExcelStyle(font,style);
	}
	
	private CellStyle produceExcelStyle(Font font, CellStyle style){
		// 字体
	 ...
	 * @return
	 */
	private void getBodyData(String rowDatas,List<ListColumn> columns,Sheet sheet){
		...
		getPropMaps(propMaps,columns,formatting,dataTypes);
		int index = 1;
		...
		for(Map<String, String> obj : objs){
			...
			for(ListColumn listColumn:columns){
				...
				CellFormatter cellFormatter=CellFormatterFactory.getCellFormatter(dataTypes[colIndex-1],cell,formatting[colIndex-1]);
				cellFormatter.formatValue(columnValue,formatting[colIndex-1]);
			}
			colIndex=0;
			index++;
	...
	public void getPropMaps(Map<String, Map<String, String>> propMaps,List<ListColumn> columns,String[] formatting,String[] dataTypes){
		Map<String, String> valueSets = new HashMap<String, String>();//保存解析第一行的值设置，避免重复解析值设置
		int i=0;
		for(ListColumn listColumn:columns){
			formatting[i]=listColumn.getFormat()==null?"":listColumn.getFormat();
			if(StringUtils.isNotEmpty(listColumn.getValueSet())){
				dataTypes[i]=DataType.TEXT.name();
			}else{
				dataTypes[i]=listColumn.getTableColumn().getDataType().name();
			}
			String columnName=listColumn.getTableColumn().getName();
			if(StringUtils.isNotEmpty(listColumn.getValueSet())){
