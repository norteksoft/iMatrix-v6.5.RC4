...
public class HistoryWorkflowTaskAction extends CrudActionSupport<HistoryWorkflowTask>{
	...
	public String list() throws Exception {
		...
		if(TaskType.WORKFLOW_NAME.toString().equalsIgnoreCase(taskType)){
			if(page.getPageSize()>1){
				...
				ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("wf.historyTask"), 
						ApiFactory.getBussinessLogService().getI18nLogInfo("wf.finishedTasklist"), 
	...
	public String canceledTasks() throws Exception{
		...
		if(TaskType.WORKFLOW_NAME.toString().equalsIgnoreCase(taskType)){
			if(page.getPageSize()>1){
				...
				ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("wf.historyTask"), 
						ApiFactory.getBussinessLogService().getI18nLogInfo("wf.cnaceledTaskList"), 
						ContextUtils.getSystemId("task"));
				this.renderText(PageUtils.pageToJson(page));
				return null;
			}
	...
	private void typeTree(List<Object[]> typeInfos,List<ZTreeNode> treeNodes,String parentId){
		...
		tree.append("[");
		if(TaskCategory.COMPLETE.equals(taskCategory)){
			ZTreeNode root = new ZTreeNode("complete_task",parentId,Struts2Utils.getText("task.menu.defaultCategory"),Struts2Utils.getText("task.menu.allMatters"), "false", "false", "", "", "folder", "");
			treeNodes.add(root);
		}else if(TaskCategory.CANCEL.equals(taskCategory)){
			ZTreeNode root = new ZTreeNode("cancel_task",parentId,Struts2Utils.getText("task.menu.defaultCategory"),Struts2Utils.getText("task.menu.allMatters"), "false", "false", "", "", "folder", "");
			treeNodes.add(root);
		}else{
