...
public class TaskAction extends CrudActionSupport<Task>{
	...
	private String currentLanguage;//当前用户语言
...
//		renderText(bl.toString()+taskWebserviceImpl.personalTasks(loginName, companyId, rows));
		...
		renderText(taskWebserviceImpl.personalTasks(loginName,userId, companyId, rows,order,currentLanguage));
		return null;
	}
	/**
	 * 任务消息
	...
	public String list() throws Exception {
		...
		if(TaskType.WORKFLOW_NAME.toString().equalsIgnoreCase(taskType)){
			if(tasks.getPageSize()>1){
				...
				ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("task.waitTask"), ApiFactory.getBussinessLogService().getI18nLogInfo("task.waitTasklist"), ContextUtils.getSystemId("task"));
	...
	public String mark() throws Exception{
		...
		ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("task.waitTask"), 
				ApiFactory.getBussinessLogService().getI18nLogInfo("wf.signTask"), 
	...
	public String completedTasks() throws Exception{
		...
		if(TaskType.WORKFLOW_NAME.toString().equalsIgnoreCase(taskType)){
			if(tasks.getPageSize()>1){
				...
				ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("task.waitTask"), 
						ApiFactory.getBussinessLogService().getI18nLogInfo("task.finishedTasklist"), 
						ContextUtils.getSystemId("task"));
				this.renderText(PageUtils.pageToJson(tasks));
				return null;
			}
	...
	public String canceledTasks() throws Exception{
		...
		if(TaskType.WORKFLOW_NAME.toString().equalsIgnoreCase(taskType)){
			if(tasks.getPageSize()>1){
				...
				ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("task.waitTask"), ApiFactory.getBussinessLogService().getI18nLogInfo("task.canceledTasklist"), ContextUtils.getSystemId("task"));
				this.renderText(PageUtils.pageToJson(tasks));
				return null;
			}
		}else if(TaskType.CUSTOM_TYPE.toString().equalsIgnoreCase(taskType)){
	...
	public String typeTree() throws Exception{
		...
		if(TaskType.WORKFLOW_NAME.toString().equalsIgnoreCase(taskType)){
			...
			ZTreeNode root = new ZTreeNode("workflow_name","0",Struts2Utils.getText("task.menu.processName"), "true", "false", "", "", "folder", "");
			treeNodes.add(root);
			typeTree(typeInfos,treeNodes,"workflow_name");
		}else if(TaskType.CUSTOM_TYPE.toString().equalsIgnoreCase(taskType)){
			typeInfos=workflowTaskManager.getCustomTypes(taskCategory);
			ZTreeNode root = new ZTreeNode("custom_type","0",Struts2Utils.getText("task.menu.customCategories"), "true", "false", "", "", "folder", "");
			treeNodes.add(root);
			typeTree(typeInfos,treeNodes,"custom_type");
		}else{
			typeInfos=taskManager.getTypeInfos(taskCategory);
			ZTreeNode root = new ZTreeNode("default_type","0",Struts2Utils.getText("task.menu.defaultCategory"), "true", "false", "", "", "folder", "");
			treeNodes.add(root);
	...
	private void typeTree(List<Object[]> typeInfos,List<ZTreeNode> treeNodes,String parentId){
		...
		if(TaskCategory.COMPLETE.equals(taskCategory)){
			ZTreeNode root = new ZTreeNode("complete_task",parentId,Struts2Utils.getText("task.menu.allMatters"), "false", "false", "", "", "folder", "");
			treeNodes.add(root);
		}else if(TaskCategory.CANCEL.equals(taskCategory)){
			ZTreeNode root = new ZTreeNode("cancel_task",parentId,Struts2Utils.getText("task.menu.allMatters"), "false", "false", "", "", "folder", "");
			treeNodes.add(root);
		}else{
			ZTreeNode root = new ZTreeNode("active_task",parentId,Struts2Utils.getText("task.menu.allMatters") + "("+taskNum+")", "false", "false", "", "", "folder", "");
			treeNodes.add(root);
		}
	...
	public String receive() throws Exception{
		...
		ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("task.waitTask"), 
				ApiFactory.getBussinessLogService().getI18nLogInfo("task.receiveTask"), 
				ContextUtils.getSystemId("task"));
...
//		String url="http://192.168.1.98:8088/imatrix/"+task.getUrl()+task.getId();
	...
	public String messageTask() throws Exception{
		...
		if(id==null){
			response.sendRedirect(SystemUrls.getSystemPageUrl("imatrix")+"/portal/my-message-error.action?errorInfo="+URLEncoder.encode(Struts2Utils.getText("task.text.theTaskDoes"),"UTF-8") );
		}else{
			...
			if(mytask==null){
				...
				if(mytask.getPaused()){
					response.sendRedirect(SystemUrls.getSystemPageUrl("imatrix")+"/portal/my-message-error.action?errorInfo="+URLEncoder.encode(Struts2Utils.getText("task.text.instanceOfThis"),"UTF-8") );
				}else{
					if(mytask.getActive().equals(2)){
						response.sendRedirect(SystemUrls.getSystemPageUrl("imatrix")+"/portal/my-message-error.action?errorInfo="+URLEncoder.encode(Struts2Utils.getText("task.text.theTaskHas"),"UTF-8") );
						return null;
					}if(mytask.getActive().equals(3)){
						response.sendRedirect(SystemUrls.getSystemPageUrl("imatrix")+"/portal/my-message-error.action?errorInfo="+URLEncoder.encode(Struts2Utils.getText("task.text.theTaskCanceled"),"UTF-8") );
						return null;
					}else if(mytask.getActive().equals(5)){
						response.sendRedirect(SystemUrls.getSystemPageUrl("imatrix")+"/portal/my-message-error.action?errorInfo="+URLEncoder.encode(Struts2Utils.getText("task.text.theTaskAssigned"),"UTF-8") );
						return null;
					}else if(mytask.getActive().equals(7)){
						response.sendRedirect(SystemUrls.getSystemPageUrl("imatrix")+"/portal/my-message-error.action?errorInfo="+URLEncoder.encode(Struts2Utils.getText("task.text.theTaskOthers"),"UTF-8") );
						return null;
					}else{
	...
	public String workflowNotification() throws Exception{
		...
		}else if("remind".equals(notificationType)){
			Message message = ApiFactory.getPortalService().getMessageById(messageId);
			notificationTheme=message.getCategory();
	...
	public String taskDetailWindow()throws Exception {
		...
		dataModel.put("noTaskkk", Struts2Utils.getText("portal.noTask", currentLanguage));
		dataModel.put("taskTypes", taskTypes);
		List<String> taskContents=new ArrayList<String>();
		String order = Struts2Utils.getParameter("order");
		String taskContent="";
		for(Object[] taskType:taskTypes){
			taskContent=taskWebserviceImpl.detailTasks(loginName,Long.parseLong(userId), Long.parseLong(companyId), rows,order,String.valueOf(taskType[0]),currentLanguage);
	...
	private String portalTypeTree(){
		...
		if(TaskType.WORKFLOW_NAME.toString().equalsIgnoreCase(taskType)){
			...
			ZTreeNode root = new ZTreeNode("workflow_name","0",Struts2Utils.getText("task.name"), "true", "false", "", "", "folder", "");
			treeNodes.add(root);
			...
			ZTreeNode root = new ZTreeNode("default_type","0",Struts2Utils.getText("task.type"), "true", "false", "", "", "folder", "");
			treeNodes.add(root);
			...
			portalTypeTree(typeInfos,treeNodes,"default_type");
			typeInfos=workflowTaskManager.getGroupNames(TaskCategory.ACTIVE);
			root = new ZTreeNode("workflow_name","0",Struts2Utils.getText("task.name"), "false", "false", "", "", "folder", "");
			treeNodes.add(root);
			portalTypeTree(typeInfos,treeNodes,"workflow_name");
		}
	...
	private void portalTypeTree(List<Object[]> typeInfos,List<ZTreeNode> treeNodes,String parentId){
		...
		ZTreeNode root = new ZTreeNode("active_task",parentId,Struts2Utils.getText("task.all.matter")+"("+taskNum+")", "false", "false", "", "", "folder", "");
		treeNodes.add(root);
		for(Object[] objs:typeInfos){
			String typeName=(String)objs[0];
			Long countTask=(Long)objs[1];
	...
	public String assignTo() throws Exception{
		...
		ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("task.waitTask"), 
				ApiFactory.getBussinessLogService().getI18nLogInfo("wf.assignTask"), 
				ContextUtils.getSystemId("task"));
		log.debug("*** receive 方法结束");
		return null;
	}
