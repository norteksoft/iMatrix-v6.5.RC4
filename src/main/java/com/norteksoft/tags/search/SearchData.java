...
//		dealWithFieldString(fieldString, fieldList);
		...
		root.put("currentLanguage", getCurrentLanguage());
	...
	public String getFixedFields(List<ObjectField> fixedFields,List<ObjectField> fieldList){
		...
		for (ObjectField field : fieldList) {
			if(field.getFixedField()){
				...
				col.append("keyValue:'").append(packagingKeyValue(field)).append("',");
	...
	private String packagingKeyValue(ObjectField field){
		String result = "";
		if(StringUtils.isNotEmpty(field.getKeyValue())){
			result = field.getKeyValue().replace("'", "");
			//false:${userInfo.woman},true:${userInfo.male}
			if(result.contains("${")&&result.contains("}")){
				String[] arr = result.split(",");
				StringBuilder temp = new StringBuilder();
				for(String a:arr){
					if(StringUtils.isNotEmpty(temp.toString()))temp.append(",");
					String[] b = a.split(":");
					temp.append(b[0]);
					temp.append(":");
					temp.append(Struts2Utils.getText(b[1].replace("${", "").replace("}", "")));
				}
	...
	public String getSearchPropertyByCode(ListView listView,String searchType){
		...
		if(searchType=="isAdvancedQuery"){
			...
			if(listView.getPopUp()==null || !listView.getPopUp()){//数据库中没值或者false时为嵌入式查询
				propertyBoolean = true;
			}else{//数据库中值为true时为弹出式查询
			...
			if(StringUtils.isNotEmpty(p)){
				...
				if(part[1].contains("'")||part[1].contains("\"")){
					...
					if(StringUtils.isNotEmpty(name)&&name.contains("${")&&name.contains("}")){
						option.setName(Struts2Utils.getText(name.replace("${", "").replace("}", "")));
					}else{
						option.setName(name);
					}
				}
...
//		 							${userInfo.woman}
	...
	private String getBeanName(String valueSet){
	 ...
	 private String getCurrentLanguage() {
		 String language = "";
		 try {
			UserCurrentLanguageManager manager = 
					   (UserCurrentLanguageManager)ContextUtils.getBean("userCurrentLanguageManager");
			 language = manager.getUserLanguageByUserId(ContextUtils.getUserId(), ContextUtils.getCompanyId());
		 } catch (Exception e) {
			return "zh_CN";//默认中文
		 }
