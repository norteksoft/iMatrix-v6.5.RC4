...
public class ztreeTag extends TagSupport{
    ...
    private String leafs;
	 public int doStartTag() {
		 try{
			 ...
			 if(StringUtils.isNotEmpty(leafs)){
				 leafs=leafs.replaceAll("\\t", "");
				 leafs=leafs.replaceAll("\\r", "");
				 leafs=leafs.replaceAll("\\n", "");
				 leafs=leafs.trim();
				 leafs=leafs.replaceAll("'", "\\\"");
				 leafs=leafs.replaceAll("\\\"", "\\\\\"");
				 out.print(readLeafZtreeTemplet());
			 }else{
    ...
    private String readLeafZtreeTemplet() {
		...
		//得到树的数据请求地址
		String actionUrl = webapp+"/portal/ztree.action";
		String searchUrl = webapp+"/portal/search-ztree.action";
		...
		root.put("language", ContextUtils.getCurrentLanguage());
		root.put("leafsMsg", leafs);
		String result="";
		try {
			result = TagUtil.getContent(root, "tree/ztree-tag-leaf.ftl");
		} catch (Exception e) {
			e.printStackTrace();
		}
		return result;
