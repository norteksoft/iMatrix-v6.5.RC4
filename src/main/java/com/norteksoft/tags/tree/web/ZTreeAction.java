...
public class ZTreeAction extends CRUDActionSupport<Company> {
	...
	private String leafsMsg;
	private String searchValue;//搜索
	//标签tree的参数
	private TreeType treeType=TreeType.COMPANY;//树的类型
   ...
   public String searchZtree(){
	...
	public String popZtree() throws IOException{
		...
		//得到树的数据请求地址
		if(StringUtils.isNotEmpty(leafsMsg)){
			actionUrl = webapp+"/portal/ztree.action";
			searchUrl = webapp+"/portal/search-ztree.action";
			root.put("ctx", webapp);
			...
			root.put("leafsMsg",leafsMsg);
			root.put("showBranch",showBranch?"true":"false");
		}else{
			root.put("ctx", webapp);
			root.put("treeId", (treeId==null||treeId.equals("undefined"))?UUID.randomUUID():treeId);
			root.put("actionUrl", actionUrl);
			root.put("searchUrl", searchUrl);
			...
			root.put("resourcesCtx",StringUtils.isEmpty(resourceCtx)?webapp:resourceCtx);
			root.put("showBranch",showBranch?"true":"false");
			root.put("confirm",ApiFactory.getSettingService().getInternationOptionValue("common.confirm",ContextUtils.getCurrentLanguage(),InternationType.PUBLIC_RESOURCE.toString()));
		}
		try {
			writer.print(TagUtil.getContent(root, StringUtils.isNotEmpty(leafsMsg)?"tree/ztree-pop-leaf.ftl":"tree/ztree-pop.ftl"));
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	...
	public String getLeafsMsg() {
		return leafsMsg;
	}
	public void setLeafsMsg(String leafsMsg) {
		this.leafsMsg = leafsMsg;
	}
