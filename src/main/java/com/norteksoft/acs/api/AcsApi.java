...
public class AcsApi {
	 ...
	 * @return List       [部门名称列表, 是否有子部门(true,false),是否有人员(true,false),是否是分支（true，false），部门编码]
	 */
	public static List<AllDeptsModelViewFlex> getAllDeptsToJson(Long companyId){
		log.debug("*** getAllDepts 方法开始");
		log.debug(new StringBuilder("*** Received parameter:[")
		.append("companyId:").append(companyId)
		.append("]").toString());
		...
		//Map<Department, String> depts = getAcsApiManager().getAllDepts(companyId);
		List<Department> depts = getAcsApiManager().getAllDeptsInOrder(companyId);
		List<AllDeptsModelViewFlex> results = new ArrayList<AllDeptsModelViewFlex>();
		boolean hasbranch = getAcsApiManager().hasBranch(companyId);
		for(Department dept : depts){
			String isHasUsersInDept=getAcsApiManager().getUsersByDeptCode(companyId, dept.getCode()).size()>0?"true":"false";
			String subCompanyName = dept.getSubCompanyName();
			...
			AllDeptsModelViewFlex modelView = new AllDeptsModelViewFlex();
			modelView.setName(dept.getName());
			modelView.setHasSubDepartment(getAcsApiManager().hasSubDepartment(dept));
			modelView.setIsHasUsersInDept(isHasUsersInDept);
			modelView.setBranch(dept.getBranch()==null?"false":dept.getBranch()+"");
			modelView.setCode(dept.getCode());
			modelView.setHasbranch(hasbranch+"");
			modelView.setSubCompanyName(subCompanyName);
			results.add(modelView);
		}
	...
	/**
	 * 根据部门名称查询该部门的所有子部门-flex
	 * @param companyId
	 * @param parentDeptName
	 * @return List  [部门名称列表, 是否有子部门(true,false),是否有人员(true,false),是否是分支（true，false），部门编码]
	 */
	public static List<SubDeptsModelViewFlex> getSubDeptsByParentDeptToJson(Long companyId, String parentDeptCode){
		log.debug("*** getSubDeptsByParentDept 方法开始");
		log.debug(new StringBuilder("*** Received parameter:[")
		.append("companyId:").append(companyId)
		.append(", parentDeptCode:").append(parentDeptCode)
		...
		List<Department> depts = getAcsApiManager().getSubDeptsByParentDept(companyId, parentDeptCode);
		List<SubDeptsModelViewFlex> results = new ArrayList<SubDeptsModelViewFlex>();
		boolean hasbranch = getAcsApiManager().hasBranch(companyId);
		for(Department dept : depts){
			String isHasUsersInDept=getAcsApiManager().getUsersByDeptCode(companyId, dept.getCode()).size()>0?"true":"false";
			String subCompanyName = dept.getSubCompanyName();
			...
			SubDeptsModelViewFlex modelView = new SubDeptsModelViewFlex();
			modelView.setName(dept.getName());
			modelView.setHasSubDepartment(getAcsApiManager().hasSubDepartment(dept));
			modelView.setIsHasUsersInDept(isHasUsersInDept);
			modelView.setDeptHasBranch(dept.getBranch()==null?"false":dept.getBranch()+"");
			modelView.setCode(dept.getCode());
			modelView.setHasbranch(hasbranch+"");
			modelView.setSubCompanyName(subCompanyName);
			results.add(modelView);
	 ...
	 * @return List       角色名称列表
	 */
	public static List<AllRolesModelViewFlex> getAllRolesBySystemIdToJson(Long systemId,Long companyId){
		log.debug("*** getAllRolesBySystemId 方法开始");
		List<Role> rs = getAcsApiManager().getAllRoles(systemId,companyId);
		BusinessSystem system = getAcsApiManager().getSystemById(systemId);
		List<AllRolesModelViewFlex> results = new ArrayList<AllRolesModelViewFlex>();
		boolean hasbranch = getAcsApiManager().hasBranch(companyId);
		for(Role r : rs){
			String subCompanyName = r.getSubCompanyName();
			if(subCompanyName==null){
			...
			AllRolesModelViewFlex modelView = new AllRolesModelViewFlex();
			modelView.setName(r.getName());
			modelView.setCode(r.getCode());
			modelView.setHasbranch(hasbranch+"");
			modelView.setBusinessSystemName(system.getName());
			modelView.setSubCompanyName(subCompanyName);
			results.add(modelView);
	 ...
	 * @return List       角色名称列表
	 */
	public static List<AllRolesModelViewFlex> getAllRolesToJson(String systemCode,Long companyId){
		log.debug("*** getAllRoles 方法开始");
		ThreadParameters parameters = new ThreadParameters(companyId);
		ParameterUtils.setParameters(parameters);
		BusinessSystem system=getAcsApiManager().getSystemBySystemCode(systemCode);
		if(system!=null)return getAllRolesBySystemIdToJson(system.getId(),companyId);
		log.debug("*** getAllRoles 方法结束");
		return new ArrayList<AllRolesModelViewFlex>();
	}
	 ...
	 * @return String[用户名称，用户登录名称,用户所属分支机构编码] 
	 */
	public static List<UsersNotInDeptModelViewFlex> getUsersByDeptToJson(Long companyId, String deptCode){
		log.debug("*** getUsersByDept 方法开始");
		log.debug(new StringBuilder("*** Received parameter:[")
		.append("companyId:").append(companyId)
		.append(", deptCode:").append(deptCode)
		...
		log.debug("*** getUsersByDept 方法结束");
		return getUserNamesFromListToJson(companyId,us);
	}
	 ...
	 * @return
	 */
	public static List<UsersNotInDeptModelViewFlex> getUsersNotInDeptToJson(Long companyId){
		log.debug("*** getUsersNotInDept 方法开始");
		log.debug(new StringBuilder("*** Received parameter:[")
		.append("companyId:").append(companyId)
		.append("]").toString());
	 ...
	 * @return
	 */
	public static List<UsersNotInDeptModelViewFlex> getBranchUsersNotInDeptToJson(Long companyId,String deptCode){
		log.debug("*** getUsersNotInDept 方法开始");
		log.debug(new StringBuilder("*** Received parameter:[")
		.append("companyId:").append(companyId)
		.append("]").toString());
	...
	private static List<UsersNotInDeptModelViewFlex> getUserNamesFromListToJson(Long companyId,Collection<User> users){
		log.debug("*** getUserNamesFromList 方法开始");
		
		boolean hasbranch = getAcsApiManager().hasBranch(companyId);
		List<UsersNotInDeptModelViewFlex> results = new ArrayList<UsersNotInDeptModelViewFlex>();
		UsersNotInDeptModelViewFlex modelView = null;
		if(users != null){
			for(User u : users){
				modelView = new UsersNotInDeptModelViewFlex();
				modelView.setName(u.getName());
				modelView.setLoginName(u.getLoginName());
				modelView.setSubCompanyCode(u.getSubCompanyCode());
				modelView.setSubCompanyName(u.getSubCompanyName());
				modelView.setHasbranch(hasbranch+"");
	 ...
	 * 查询所有业务系统信息-flex调用
	 */
	public static List<AllBusinessModelViewFlex> getAllBusinessToJsonFlex(Long companyId){
		MenuManager menuManager = (MenuManager)ContextUtils.getBean("menuManager");
		ThreadParameters parameters = new ThreadParameters(companyId);
		ParameterUtils.setParameters(parameters);
		List<Menu> list=menuManager.getAllEnabledStandardRootMenus();
		List<AllBusinessModelViewFlex> results = new ArrayList<AllBusinessModelViewFlex>();
		AllBusinessModelViewFlex modelView = null;
		if(list != null){
			for(Menu u : list){
				modelView = new AllBusinessModelViewFlex();
				modelView.setId(u.getSystemId().toString());
				modelView.setName(menuManager.getNameToi18n(u.getName()));
				modelView.setCode(u.getCode());
				results.add(modelView);
			}
		}
		return results;
	 ...
	 * 查询所有分支机构-flex调用
	 */
	public static List<AllBranchModelViewFlex> getAllBranchToJsonFlex(Long companyId){
		List<Department> list=getAcsApiManager().getAllBranchs(companyId);
		List<AllBranchModelViewFlex> results = new ArrayList<AllBranchModelViewFlex>();
		AllBranchModelViewFlex modelView = null;
		Company company = getAcsApiManager().getCompanyById(companyId);
		List<Workgroup> wgs = getAcsApiManager().getWorkgroups(companyId);
		modelView = new AllBranchModelViewFlex();//name,code,是否存在工作组
		modelView.setName(company.getName());
		modelView.setCode(company.getCode());
		modelView.setHasWorkgroup(wgs.size()>0?true+"":false+"");
		results.add(modelView);
		if(list != null){
			for(Department d : list){
				wgs = getAcsApiManager().getWorkgroupByBranchId(companyId, d.getId());
				modelView = new AllBranchModelViewFlex();
				modelView.setName(d.getName());
				modelView.setCode(d.getCode());
				modelView.setHasWorkgroup(wgs.size()>0?true+"":false+"");
				results.add(modelView);
			}
		}
	 ...
	 * 查询该分支机构内的工作组
	 */
	public static List<WorkgroupsModelViewFlex> getWorkgroupByBranchCodeToJson(Long companyId,String branchCode){
		ThreadParameters parameters = new ThreadParameters(companyId);
		ParameterUtils.setParameters(parameters);
		
		Company company = getAcsApiManager().getCompanyById(companyId);
		boolean hasbranch = getAcsApiManager().hasBranch(companyId);
		List<WorkgroupsModelViewFlex> results = new ArrayList<WorkgroupsModelViewFlex>();
		if(branchCode.equals(company.getCode())){
			return getWorkgroupsToJson(companyId);
		}else{
			Department branch = getAcsApiManager().getDepartmentByCode(companyId, branchCode);
			if(branch!=null){
				List<Workgroup> wgs = getAcsApiManager().getWorkgroupByBranchId(companyId, branch.getId());
				for(Workgroup wg : wgs){
					...
					WorkgroupsModelViewFlex modelView = new WorkgroupsModelViewFlex();
					modelView.setName(wg.getName());
					modelView.setCode(wg.getCode());
					modelView.setHasbranch(hasbranch+"");
					modelView.setSubCompanyName(subCompanyName);
					results.add(modelView);
				}
	 ...
	 * @return
	 */
	public static List<WorkgroupsModelViewFlex> getWorkgroupsToJson(Long companyId){
		boolean hasbranch = getAcsApiManager().hasBranch(companyId);
		List<Workgroup> wgs = getAcsApiManager().getWorkgroups(companyId);
		List<WorkgroupsModelViewFlex> results = new ArrayList<WorkgroupsModelViewFlex>();
		for(Workgroup wg : wgs){
