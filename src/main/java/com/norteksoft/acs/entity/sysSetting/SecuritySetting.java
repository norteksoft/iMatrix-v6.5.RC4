...
public class SecuritySetting extends IdEntity{
	...
	private String name;//设置名称（固定九个值login-security（登录安全设置）、loginTimeouts（系统登录超时设置）、password-over-notice（密码过期通知设置）、admin-password-overdue（管理员密码过期设置）、user-password-overdue（一般用户密码过期设置）、password-complexity（密码复杂设置）、log-set（审计信息设置）、reset-password（用户第一次登录系统是否重新设置密码，默认不用重新设置密码）、system-default-language（系统默认语言））
