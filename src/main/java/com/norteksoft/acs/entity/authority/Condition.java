...
public class Condition extends IdEntityNoExtendField implements Serializable{
...
//	private ConditionType relativeType;//条件为相对条件时，此字段有值，表示相对值
	...
	public String getConditionValue() {
		...
		List<String> items = permissionItemManager.getPermissionItemConditionValueByItem(getId(),ConditionValueType.DATA_RULE);
		StringBuilder tempConditionValue = new StringBuilder();
		for(String str:items){
			if(StringUtils.isNotEmpty(tempConditionValue.toString()))tempConditionValue.append(",");
			tempConditionValue.append(str);
		}
		return tempConditionValue.toString();
	}
	...
	public String getConditionName() {
		...
		PermissionItemManager permissionItemManager = (PermissionItemManager)ContextUtils.getBean("permissionItemManager");
		List<String> items = permissionItemManager.getPermissionItemConditionNameByItem(getId(),ConditionValueType.DATA_RULE);
		StringBuilder tempConditionName = new StringBuilder();
		for(String str:items){
			if(StringUtils.isNotEmpty(tempConditionName.toString()))tempConditionName.append(",");
			tempConditionName.append(str);
		}
		return tempConditionName.toString();
	}
