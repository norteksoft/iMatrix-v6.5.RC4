...
public class AcsApiManager {
	...
	public Set<User> getUsersByRoleName(Long systemId, Long companyId, String roleName){
		Role role=acsUtils.getRoleByName(systemId, roleName,companyId);
		return acsUtils.getUsersByRoleIncludeTrustedRole(systemId, companyId, role.getCode());
	}
	...
	public Set<User> getUsersByRoleCode(Long systemId, Long companyId, String roleCode){
		return acsUtils.getUsersByRole(systemId, companyId, roleCode);
	}
...
//		List<Long> idList = businessDao.find(hql, companyId, new Date());
		...
		List<Long> idList = businessDao.find(hql, companyId );
		if(idList.isEmpty()){
