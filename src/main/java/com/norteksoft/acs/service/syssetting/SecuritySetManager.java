...
public class SecuritySetManager {
	...
	private SecuritySetting resetPassword;
	...
	public void delete(List<SecuritySetting> entitys) {
		for (SecuritySetting entity : entitys) {
			securitySetDao.delete(entity);
		}
	}
	...
	public SecuritySetting getSystemValues(Long companyId, String name) {
	...
	/**
	 * 读取密码是否过期
	 * @return 返回值是长度为2的整形数组。
	 * “数组元素下标为0”的值等于“-1”表示“密码没有过期”，“数组元素下标为0”的值等于“整数几”表示“密码还有几天过期” ，若等于 0就是已经“密码过期”；
	 * “数组元素下标为1”的值等于“0”表示“用户第一次登录系统时不需要重新设置密码”，“数组元素下标为1”的值等于“1”表示“用户第一次登录系统时必须重新设置密码”。
	 */
	public Integer[] getPasswordIsOverdue(Long userId, Long companyId){
		User user = userDao.get(userId);
		Integer[] result=getPasswordIsOverdue(user.getUserInfo(), companyId);
	...
	public Integer[] getPasswordIsOverdue(UserInfo ui, Long companyId) {
		...
		result[0]=-1;
		if(resetPassword!=null&&"yes".equals(resetPassword.getValue())){//用户第一次登录系统时必须重新设置密码（表示在参数设置中设置了“用户第一次登录系统时必须重新设置密码”）
			result[1]=1;
		}else{//用户第一次登录系统时不用重新设置密码
			result[1]=0;
		}
			...
			if (adminPassOver != null
				...
				if (date.compareTo(validityDate) >= 0) {// 判断密码是否过期
					...
					if (date.compareTo(passNoticeDate) >= 0) {// 判断密码是否提前通知
						...
						result[0]=Integer.valueOf(String.valueOf(day));
					}
				}
			}
		} else {// 如果是普通用户
	...
	public void searchSystemValues(Long companyId) {
			...
			resetPassword = getSystemValues(companyId, "reset-password");
	}
	...
	public Set<Role> getRolesByUserNew(Long userId){
		User user=userDao.get(userId);
 		return acsUtils.getRolesByUserIncludeConsigner(user.getCompanyId(), userId);
	}
