...
public class DataRuleManager {
	...
	public ConditionResult getConditionResult(String hql,List<String> ruleCodes) {
		...
		return packagingConditionResult(hql,rules);
	}
	public ConditionResult getConditionResult(String hql,Object entity) {
		List<DataRuleResult> rules = new ArrayList<DataRuleResult>();
		String className = entity.getClass().getName();
		if(className.contains("_")){
			className =className.substring(0,className.indexOf("_"));
		}
		DataTable dataTable=dataTableDao.getDataTableByEntity(className);
		List<DataRule> dataRules=dataRuleDao.getDataRuleByDataTable(dataTable.getId());
		for (DataRule r : dataRules) {
			DataRuleResult ruleResult = new DataRuleResult(r, null);
			if(r!=null&&r.getSimplable()){
				rules.add(ruleResult);
			}else{
	...
	public ConditionResult packagingConditionResult(String hql,List<DataRuleResult> rules) {
		...
		permissionInfo.setSystemCodes(businessSystemManager.getAllSystemCodes());
		return PermissionUtils.getPermissionHqlPamateters(hql, permissionInfo,LogicOperator.OR);
	}
	...
	public void addConditionResult(String hql, List<String> dataRuleCodes) {
		ConditionResult cr = getConditionResult(hql, dataRuleCodes);
	...
	public void addConditionResult(String hql, Object entity) {
		ConditionResult cr = getConditionResult(hql, entity);
		PermissionThreadParameters parameters=new PermissionThreadParameters();
		parameters.setHql(cr.getHql());
		parameters.setParameters(cr.getPrameters());
		ParameterUtils.setPermissionParameters(parameters);
