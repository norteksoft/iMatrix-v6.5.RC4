...
public class UserDetailServiceImpl implements UserDetailsService {
	...
	private String defaultLanguage = "zh_CN";//默认语言
...
//		}
		...
		把上面注释代码修改成以下代码的原因：解决同一个用户用“登录名”或“邮箱”或“手机号码”在不同地方登录时跳转到“该用户已经在别处登录”页面*/
		String[] names = userManager.getUserInfo(loginName);
		String tempLoginName = user.getLoginName()+loginName.substring(names[0].length(),loginName.length());
		return createUserdetail(user, company, authsList,subCompany,tempLoginName);	
	}
...
//			throw new UsernameNotFoundException("company valid date");
	...
	private com.norteksoft.acs.entity.security.User createUserdetail(User user, Company company, List<GrantedAuthority> authsList,Department subCompany,String userinfo){
		...
		String language = userCurrentLanguageManager.getUserLanguageByUserId(user.getId(), company.getId());
