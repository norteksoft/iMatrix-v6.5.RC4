...
//		            sessionMappingStorage.addSessionById(mappingId, session);
	        ...
	        if(parameters!=null&&!parameters.isEmpty()){
		       if(parameters.contains("url")&&parameters.contains("ticket")){
		    	   ...
		    	   for (String parameter : parameters) {
		    		   //获得targetUrl的所有参数，并存在paramVals中值为:a=&b=&...&
		    		   getTargetUrlParam(parameter,map.get(parameter)[0],paramVals);
		    	   ...
		    	   if(StringUtils.isNotEmpty(paramVals.toString())){//targetUrl有参数则拼接参数
		    		   url = url+"?"+paramVals.substring(0,paramVals.lastIndexOf("&"));
		    	   }
		    	   targetUrl=url;
		       }
	       }
	 ...
	 private void getTargetUrlParam(String param,String paramVal,StringBuilder paramVals){
		 if(!isTicketParam(param)){//不是获得ticket所必须的参数，即targetUrl的参数
	 ...
	 private boolean isTicketParam(String param){
		 if(param.equals("ticket")||param.equals("url")||param.equals("tgt"))return true;
		 return false;
	 }
		...
		protected void sendRedirect(HttpServletRequest request,
			...
			if(request.getAttribute("overdue")!=null){
				...
				if(!(overdue[0]==-1&&overdue[1]==0)){
					url = "/portal/update-password.action?overdue="+(overdue[0]==-1?"":overdue[0])+"&resetPassword="+(overdue[1]==0?"":overdue[1])+"&name=" + URLEncoder.encode(ContextUtils.getUserName(),"utf-8") +"&url=" + url+ "&id=" + ContextUtils.getUserId()
