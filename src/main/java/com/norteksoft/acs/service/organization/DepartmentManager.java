...
public class DepartmentManager {
	...
	public List<Department> getSubBranchs(Long deptId){
		...
		List<Department> branches = getAllBranches();
		for(Department branch:branches){
			if(isSubBranch(branch,deptId)){
				result.add(branch);
			}
		}
		return result;
	...
	private boolean isSubBranch(Department dept,Long deptId){
		Department parentDept = dept.getParent();
		if(parentDept!=null){
			if(parentDept.getId().equals(deptId)){
				return true;
			}else{
				return isSubBranch(parentDept,deptId);
			}
		}
		return false;
	}
	...
	public List<Department> getAllBranches() {
		...
		hql.append("where d.company.id=? and d.deleted=? and d.branch=?   order by d.weight desc,d.id");
	...
	/**
	 * 通过分支机构id获得该分支下的所有部门id
	 * @param branchId
	 * @return
	 */
	public List<Long> getDepartmentIdsByBranchId(Long branchId) {
		List<Long> subDeptIds=new ArrayList<Long>();
		getDepartmentIdsByBranchId(branchId,subDeptIds);
		return subDeptIds;
	}
	
	private void getDepartmentIdsByBranchId(Long deptId,List<Long> subDeptIds) {
		List<Department> depts=getSubDeptments(deptId);
		for(Department dept:depts){
			if(dept.getBranch()){
				continue;
			}else{
				subDeptIds.add(dept.getId());
				getDepartmentIdsByBranchId(dept.getId(),subDeptIds);
			}
		}
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public Department getDepartmentByDepartmentName(Department parent,String name){
		List<Department> depts = new ArrayList<Department>();
		StringBuilder hql = new StringBuilder();
		hql.append("from Department d where d.company.id=? ");
		hql.append("and d.name=? and d.deleted=? ");
		hql.append("and d.branch=? and d.subCompanyId is null ");
		if(parent!=null){
			hql.append("and d.parent.id=?");
			depts = departmentDao.find(hql.toString(), ContextUtils.getCompanyId(), name, false,false,parent.getId());
		}else{
			hql.append("and d.parent is null");
			depts = departmentDao.find(hql.toString(), ContextUtils.getCompanyId(), name, false,false);
		}
		if(depts.size() == 1){
			return depts.get(0);
		}
	...
	public List<Department> getDepartments() {
		List<Department> list =  departmentDao.find(
				"FROM Department d WHERE d.company.id=? AND d.deleted=? and d.parent.id is null ORDER BY d.weight desc,d.id", 
				getCompanyId(), false);
		return list;
	}
	...
	public Long getSubDepartmentCount(Long paternDepartmentId) {
		...
		Long list =  departmentDao.findLong(
				"select count(id) FROM Department d WHERE d.parent.id=? AND d.deleted=?", 
	...
	public List<Department> getSubDepartmentList(Long paternDepartmentId) {
		if(paternDepartmentId == null) throw new RuntimeException("没有给定查询子部门集合的查询条件： 父部门ID");
		List<Department> list =  departmentDao.find(
				"FROM Department d WHERE d.parent.id=? AND d.deleted=?  ORDER BY d.weight desc,d.id", 
				paternDepartmentId, false);
		return list;
	}
}
