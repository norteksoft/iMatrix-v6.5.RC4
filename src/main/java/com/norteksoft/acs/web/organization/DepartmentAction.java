...
@Results( { @Result(name = CRUDActionSupport.RELOAD, location = "department?departmentId=${departmentId}&treeSelectedNode=${treeSelectedNode}", type="redirectAction") })
public class DepartmentAction extends CRUDActionSupport<Department>{
	...
	public String list() throws Exception {
		...
		if(userPage.getPageSize() <= 1){
			...
			ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("acs.departmentManagement"), 
					ApiFactory.getBussinessLogService().getI18nLogInfo("acs.viewDepartList"),ContextUtils.getSystemId("acs"));
			return null;
		}
	}
	...
	public String input() throws Exception {
		if(id == null){
			...
			ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("acs.departmentManagement"), 
					ApiFactory.getBussinessLogService().getI18nLogInfo("acs.updateDepartment"),ContextUtils.getSystemId("acs"));
		}
	 ...
	 * 保存部门信息
	 */
	@Action("department-saveDepartment")
	public String saveDepartment() throws Exception{
		...
		if(logSign){
			ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("acs.departmentManagement"), 
					ApiFactory.getBussinessLogService().getI18nLogInfo("acs.createDepart",messages),ContextUtils.getSystemId("acs"));
		}else{
			ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("acs.departmentManagement"), 
					ApiFactory.getBussinessLogService().getI18nLogInfo("acs.updateDepartment1",messages),ContextUtils.getSystemId("acs"));
		}
		if(roleManager.hasAdminRole(ContextUtils.getUserId())){
			return null;
		}else{
	...
	public String delete() throws Exception {
		...
		ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("acs.departmentManagement"), 
				ApiFactory.getBussinessLogService().getI18nLogInfo("acs.deleteDepartment",messages),ContextUtils.getSystemId("acs"));
	    return list();
	}
	 ...
	 * 验证删除分支机构
	 */
	@Action("department-validateDepartmentDelete")
	 ...
	 * @throws Exception
	 */
	@Action("department-departmentAddUser")
    public String departmentAddUser()throws Exception{
		...
		messages.add(logSign);
    	ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("acs.departmentManagement"), 
    			ApiFactory.getBussinessLogService().getI18nLogInfo("acs.addPartimePerson",messages),ContextUtils.getSystemId("acs"));
    	this.renderText("ok");
    	return null;
    }
    
  //判断所选择的部门下是否已经存在相同登录名的用户并得到这些用户
	@Action("department-getRepeatUserNames")
     ...
     * @throws Exception
     */
	@Action("department-removeDepartmentToUsers")
    public String removeDepartmentToUsers() throws Exception{
		...
		messages.add(logSign);
    	ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("acs.departmentManagement"), 
    			ApiFactory.getBussinessLogService().getI18nLogInfo("acs.remorePartimePerson",messages),ContextUtils.getSystemId("acs"));
    	return RELOAD;
    }
	 ...
	 * 验证部门名称唯一性
	 */
    @Action("department-checkDeptName")
	public String checkDeptName() throws Exception{
		...
		if(id!=null){//修改
			if(branchFlag){//是分支机构
				...
				Company company=companyManager.getCompanyByCode(departmentCode);
				if(company!=null){
					return this.renderText("branchCodeFalseOfCompanyCode");
				}
	...
	public String save() throws Exception {
		...
		addActionMessage(getText("common.saved"));
		ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("acs.departmentManagement"), 
				ApiFactory.getBussinessLogService().getI18nLogInfo("acs.saveDepartInfo"),ContextUtils.getSystemId("acs"));
		return RELOAD;
	}
	/**
	 * 是否是当前部门的子部门
	 * @return
	 */
	@Action("department-isSubDepartment")
	 ...
	 * @return
	 */
	@Action("department-getDepartmentByCompany")
