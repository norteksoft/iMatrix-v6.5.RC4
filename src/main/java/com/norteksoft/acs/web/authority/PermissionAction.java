...
public class PermissionAction extends CrudActionSupport<Permission>{
	...
	public String delete() throws Exception {
		...
		addSuccessMessage(Struts2Utils.getText("common.deleted"));
		ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("acs.authorization"), ApiFactory.getBussinessLogService().getI18nLogInfo("acs.deleteAuthorization"),ContextUtils.getSystemId("acs"));
	...
	public String input() throws Exception {
		...
		ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("acs.authorization"), ApiFactory.getBussinessLogService().getI18nLogInfo("acs.authorizationForm"),ContextUtils.getSystemId("acs"));
	...
	public String permissionData() throws Exception {
		...
		if(page.getPageSize()>1){
			if(dataRuleId==null){
				ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("acs.authorization"), ApiFactory.getBussinessLogService().getI18nLogInfo("acs.authorizationList"),ContextUtils.getSystemId("acs"));
	...
	public String save() throws Exception {
		...
		ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("acs.authorization"), ApiFactory.getBussinessLogService().getI18nLogInfo("acs.saveAuthorization"),ContextUtils.getSystemId("acs"));
		return "permission-input";
	}
	...
	public String permissionItemDelete() throws Exception {
		...
		ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("acs.authorization"), ApiFactory.getBussinessLogService().getI18nLogInfo("acs.deleteaAuthorizationOption"),ContextUtils.getSystemId("acs"));
		this.renderText(callback+"({msg:'"+Struts2Utils.getText("common.deleted")+"'})");
		return null;
	}
	...
	public String validatePermission() throws Exception {
		ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("acs.authorization"), ApiFactory.getBussinessLogService().getI18nLogInfo("acs.validateAuthorization"),ContextUtils.getSystemId("acs"));
	...
	public String fastListData() throws Exception {
		...
		if(page.getPageSize()>1){
			ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("acs.authorization"), ApiFactory.getBussinessLogService().getI18nLogInfo("acs.quickAuthorizationList"),ContextUtils.getSystemId("acs"));
	...
	public String fastPermissionDelete() throws Exception {
		...
		addSuccessMessage(Struts2Utils.getText("common.deleted"));
		ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("acs.authorization"), ApiFactory.getBussinessLogService().getI18nLogInfo("acs.deleteQuickAuthorization"),ContextUtils.getSystemId("acs"));
		return "fast-permission-data";
	}
	...
	public String fastPermissionInput() throws Exception {
		...
		ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("acs.authorization"),ApiFactory.getBussinessLogService().getI18nLogInfo("acs.quickAuthorizationForm"),ContextUtils.getSystemId("acs"));
	...
	private void dealWithPermissionUsers(){
		...
		if(allUserItems.size()>0){//所有人
			...
			if(pointUserItems.size()>0){
				...
				if(pointUsernames.size()>0){
					pointUserNames = pointUsernames.toString().replace("[", "").replace("]","").replace(" ", "");
					if("${common.allPersonnel}".equals(pointUserNames)){
						pointUserNames = Struts2Utils.getText("common.allPersonnel");
					}
				}
			...
			if(pointDeptItems.size()>0){
				...
				if(pointdeptnames.size()>0){
					pointDeptNames = pointdeptnames.toString().replace("[", "").replace("]","").replace(" ", "");
					if("${common.allDepartment}".equals(pointDeptNames)){
						pointDeptNames = Struts2Utils.getText("common.allDepartment");
					}
				}
			...
			if(pointWgItems.size()>0){
				...
				if(pointWgnames.size()>0){
					pointWorkgroupNames = pointWgnames.toString().replace("[", "").replace("]","").replace(" ", "");
					if("${common.allWorkgroup}".equals(pointWorkgroupNames)){
						pointWorkgroupNames = Struts2Utils.getText("common.allWorkgroup");
					}
				}
	...
	public String fastPermissionSave() throws Exception {
		...
		permissionManager.saveFastPermission(permission,docAuthes,dataTableId,dataTableName,dataRange,deparmentInheritable,permissionUsers,configurableField);
		...
		addSuccessMessage(Struts2Utils.getText("common.saved"));
		ApiFactory.getBussinessLogService().log(ApiFactory.getBussinessLogService().getI18nLogInfo("acs.authorization"), ApiFactory.getBussinessLogService().getI18nLogInfo("acs.saveQuickAuthorization"),ContextUtils.getSystemId("acs"));
		return "fast-permission-input";
	}
	...
	public String systemDataRuleTree(){
		...
		for(Menu menu:menus){
			...
			ZTreeNode root = new ZTreeNode("menuId_"+menu.getId(),"0",getInternation(menu.getName()), "false", "false", "", "", "folder", "");
			treeNodes.add(root);
