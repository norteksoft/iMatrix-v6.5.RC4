...
public class DataRuleAction extends CrudActionSupport<DataRule>{
				...
				for (Menu menu : menus) {
					...
					if(dataRules.size()>0){
						tree.append(JsTreeUtils.generateJsTreeNodeDefault("menu", "open", 
								getInternation(menu.getName()),getDataRuleByMenu(dataRules))).append(",");
					}else{
						tree.append(JsTreeUtils.generateJsTreeNodeDefault("menu", null, 
								getInternation(menu.getName()))).append(",");
					}
				}
			}
	...
	public String dataRuleTypeTree() throws Exception {
		...
		String result ="";
		for(Menu menu :menus){
			ZTreeNode root = new ZTreeNode("menuId_"+menu.getId().toString(),"0",getInternation(menu.getName()), "false", "false", "", "", "folder", "");
			treeNodes.add(root);
		}
	...
	public String relativeConditionTree(){
		...
		if(PermissionUtils.STANDARD_FIELD_CREATOR.equals(standardField)){
			...
			return null;
		}else if(PermissionUtils.STANDARD_FIELD_SUBCOMPANY.equals(standardField)){
			this.renderText(PermissionItemTreeUtil.getSubCompanyTree());
			return null;
		}else{
			ZTreeNode root = new ZTreeNode("root~~root~~root","0",Struts2Utils.getText("authorization.standardValue"), "true", "false", "", "", "folder", "");
			treeNodes.add(root);
	 ...
	 * @return
	 */
	@Action("data-rule-selectDefaultConditionValue")
	public String selectDefaultConditionValue(){
		...
		return "data-rule-selectDefaultConditionValue";
	}
