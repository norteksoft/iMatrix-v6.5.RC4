...
public class WidgetDao extends HibernateDao<Widget, Long>{
	...
	public Set<Widget> getAuthWidgets(){
		...
		.append("(select si.product.systemId from SubscriberItem si  join si.subsciber s  where  s.tenantId=? ) and bs.deleted=false)")
		.append(" and ru.user.deleted=false and ru.user.id=? and w.companyId=?");
		List<Widget> userWidgets = this.find(hql.toString(),ContextUtils.getCompanyId() ,ContextUtils.getUserId(),ContextUtils.getCompanyId());
		...
		List<Widget> deptRoleWidgets = this.find(hql.toString(),ContextUtils.getCompanyId() ,ContextUtils.getUserId(),ContextUtils.getCompanyId());
		...
		.append("(select bs.code from BusinessSystem bs where bs.id in ")
		.append("(select si.product.systemId from SubscriberItem si  join si.subsciber s  where  s.tenantId=?  ))")
		.append(" and u.id=? and w.companyId=?");
		List<Widget> subCompanyRoleWidgets = this.find(hql.toString(),ContextUtils.getCompanyId(),ContextUtils.getCompanyId() ,ContextUtils.getUserId(),ContextUtils.getCompanyId());
		...
		List<Widget> wgRoleWidgets = this.find(hql.toString(),ContextUtils.getCompanyId() ,ContextUtils.getUserId(),ContextUtils.getCompanyId());
		widgetList.addAll(wgRoleWidgets);
		Set<Role> roles=new HashSet<Role>();
		roles=acsUtils.getRolesByUserIncludeConsigner(ContextUtils.getCompanyId(), ContextUtils.getUserId());
		List<Widget> ws=new ArrayList<Widget>();
		for(Role role:roles){
			ws=this.find("select w from WidgetRole wr,Role r,Widget w where wr.roleId=r.id and wr.widgetId=w.id  and r.deleted=?  and w.companyId=? and r.id=?",false,ContextUtils.getCompanyId(),role.getId());
			widgetList.addAll(ws);
		}
		return widgetList;
	}
	//获得不受权限控制的小窗体集合
	public List<Widget> getNoAuthWidgets(){
		...
		return this.find(hql.toString(),ContextUtils.getCompanyId() ,ContextUtils.getCompanyId());
	}
	...
	public Page<Widget> getWidgetsBySystemCode(Page<Widget> widgetPage,String systemCode){
		return this.searchPageByHql(widgetPage,"from Widget w where w.systemCode=? and  w.companyId=?",systemCode,ContextUtils.getCompanyId());
	}
