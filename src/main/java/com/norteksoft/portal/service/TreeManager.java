...
//	}
	private List<User> getUserByDepartment(List<User> tempList,List<Department> depts){
		...
		for(Department dept:depts){
			...
			long totalPages = 0;
        	do {
        		//修改为分页查询的原因：解决数据很大时长时间占用资源导致的各种错误
        		Page<User> pageTemp = new Page<User>(500, true);
        		pageTemp.setPageNo(pageNo);
        		userDao.findPage(pageTemp,"select distinct u from DepartmentUser du join du.user u join du.department d  where du.companyId=? and d.deleted=? and du.deleted=? and u.deleted=? and d.id=?",ContextUtils.getCompanyId(),false,false,false,dept.getId());
        		tempList.addAll(pageTemp.getResult());
        		totalPages = pageTemp.getTotalPages();
        		pageNo++;
		...
		if(StringUtils.isNotEmpty(formId)){
			...
			if(treeType.equals("MAN_DEPARTMENT_TREE")||treeType.equals("COMPANY")||treeType.equals("MAN_GROUP_TREE")){
				...
				for(int i=0;i<usersSize;i++){
					userIds=userIds+users.get(i).getId()+((i==usersSize-1)?"":",");
					String tempUserName = users.get(i).getName();
					String tempEmail = users.get(i).getEmail();
					String tempTelephone = users.get(i).getUserInfo().getTelephone();
					Long tempSubCompanyId = users.get(i).getSubCompanyId();
					if(showBranch&&hasBranch){
						userNames=userNames+tempUserName+"("+users.get(i).getSubCompanyName()+")"+((i==usersSize-1)?"":",");
					}else{
						userNames=userNames+tempUserName+((i==usersSize-1)?"":",");
					}
					mails=mails+(StringUtils.isEmpty(tempEmail)?"null":tempEmail)+((i==usersSize-1)?"":",");
					telephones=telephones+(StringUtils.isEmpty(tempTelephone)?"null":tempTelephone)+((i==usersSize-1)?"":",");
					userLoginNames=userLoginNames+users.get(i).getLoginName()+((i==usersSize-1)?"":",");
					userSubCompanyIds=userSubCompanyIds+(tempSubCompanyId==null?"null":tempSubCompanyId)+((i==usersSize-1)?"":",");
				}
