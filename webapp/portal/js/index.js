...
function doQuery(){
	...
	if(userName==""&&userDepart==""&&userSex==""){
		iMatrix.alert(iMatrixMessage["portal.pleaseEnterTheQueryCondition"]);
	}else{
...
function _widget_parameter_set(widget,webpageId){
	...
	$.ajax({
		...
		success:function(data, textStatus){
			if(data=="false"){
				iMatrix.alert(portal_lang.noWidgetParameterTip);
			}else{
				if(data==null){
...
function submitCheckboxValue(id){
	...
	if(showNum==''){
		$("#message_"+id).html("<font color=\"#FF0000\"><nobr>"+iMatrixMessage["portal.param.item.count.input"]+"</nobr></font>");
		showMsg("message_"+id);
		return false;
	}else if(showNum>parseInt(500)){
		$("#message_"+id).html("<font color=\"#FF0000\"><nobr>"+iMatrixMessage["portal.param.item.count.max"]+"</nobr></font>");
		showMsg("message_"+id);
		return false;
	}else if(showNum.match( intege1 )==null){
		$("#message_"+id).html("<font color=\"#FF0000\"><nobr>"+iMatrixMessage["portal.param.item.count.number"]+"</nobr></font>");
		showMsg("message_"+id);
		return false;
	}else{
		return true;
...
function loadWidgetContents(){
	...
	for(var i=0;i<__widgetIdInputs.length;i++){
		...
		if(iframeable!="true"){
			...
			if(j/5.0==1||n==__widgetIdInputs.length){//每5个小窗体请求一次
...
function registerWidget(){
	var title = portal_lang.registerWidgetTitle;
	$.colorbox({href:webRoot+"/index/show-register-widget.htm",iframe:true, innerWidth:$(window).width()-40, innerHeight:$(window).height()-60,overlayClose:false,title:title});
}
//个人设置
function baseSetting(){
	var title = portal_lang.baseSettingTitle;
	$.colorbox({href:webRoot+"/index/base-setting-input.htm",iframe:true, innerWidth:400, innerHeight:400,overlayClose:false,onClosed:function(){baseSettingCallBack();},title:title});
}
...
function addTheme(){
	var title = portal_lang.addThemeTitle;
	$.colorbox({href:webRoot+"/index/add-theme.htm",iframe:true, innerWidth:$(window).width()-500, innerHeight:$(window).height()-160,overlayClose:false,onClosed:function(){baseSettingCallBack();},title:title});
}
