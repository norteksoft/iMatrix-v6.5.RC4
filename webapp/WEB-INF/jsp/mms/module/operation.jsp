			...
			for(var i=0;i<rows.length;i++){
				if(rows[i].id!=operationId){
					if(rows[i].code==code){
						...
						iMatrix.alert(iMatrixMessage.thisCode+code+iMatrixMessage.exist);
						return;
					}
				}
			}
...
<div class="ui-layout-center">
	<div class="opt-body">
		...
		<aa:zone name="pageTable">
			<div class="opt-btn">
				<button class="btn" onclick="createOperation();"><span><span><s:text name="menuManager.new"></s:text></span></span></button>
				<button class="btn" onclick="updateOperation();"><span><span ><s:text name="menuManager.update"></s:text></span></span></button>
				<button class="btn" onclick="deleteOperations();"><span><span ><s:text name="menuManager.delete"></s:text></span></span></button>
			</div>
			<div id="opt-content">
