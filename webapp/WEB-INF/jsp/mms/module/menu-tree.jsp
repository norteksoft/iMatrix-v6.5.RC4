	...
	<script type="text/javascript">
		...
		function importMenu(){
			$.colorbox({href:'${mmsCtx}/module/show-import-menu.htm',
				iframe:true, innerWidth:350, innerHeight:100,overlayClose:false,title:iMatrixMessage["menuManager.importMenuColorboxTitle"]});
		}
		...
		function updateUrlCacheCallback(){
			...
			$('#message').html("<font class=\"onSuccess\"><nobr>"+iMatrixMessage["menuManager.updateUrlCacheCallbackInfo"]+"</nobr></font>");
		...
		function exportCustomSystem(){
			if(!currentMenuId||typeof(currentMenuId)=="undefined"){
				iMatrix.alert(iMatrixMessage.selectCustomFirstMenuInfo);
				return;
			}
			$.ajax({
				...
				success:function(data, textStatus){
					if("ok"==data){
						...
						iMatrix.alert(iMatrixMessage.selectCustomFirstMenuStateInfo);
					}
				},
		...
		function importCustomSystem(){
			$.colorbox({href:'${mmsCtx}/module/show-import-custom-system.htm',
				iframe:true, innerWidth:350, innerHeight:100,overlayClose:false,title:iMatrixMessage["menuManager.importCustomSystemColorboxTitle"],onClosed:function(){
					refreshTree();
				}});
		}
		...
		<div class="opt-btn">
			<div class="btndiv" id="_newbtn" style="*top:-2px;">
				<button  class="ui-button ui-widget ui-state-default ui-button-text-only ui-corner-left" id="parentNewBtn">
					<span class="ui-button-text"><s:text name="menuManager.newSystem"></s:text></span>
				</button>
				<button  title="更多"  class="ui-button ui-widget ui-state-default ui-button-icon-only ui-corner-right" id="select">
					<span class="ui-button-icon-primary ui-icon ui-icon-triangle-1-s"></span>
					<span class="ui-button-text"><s:text name="menuManager.more"></s:text></span>
				</button>
			</div>
			<div id="newbtn" class="flag">
				<ul >
					<li><a href="#" onclick="createMenu();"><s:text name="menuManager.newMenu"></s:text></a></li>
					<li><a href="#"  onclick="updateMenu();"><s:text name="menuManager.update"></s:text></a></li>
					<li><a href="#"  onclick="deleteMenu();"><s:text name="menuManager.delete"></s:text></a></li>
				</ul>
			</div>
			<script type="text/javascript">
				$(document).ready(function() {
			...
			<button class="btn" onclick="updatUrlCache();"><span><span ><s:text name="menuManager.updatUrlCache"></s:text></span></span></button>
			<button class="btn" onclick="enableMenu();"><span><span ><s:text name="menuManager.start"></s:text></span></span></button>
			<s:if test='#versionType=="online"'>
				<button class="btn" onclick="demonstrateOperate();"><span><span ><s:text name="menuManager.forbidden"></s:text></span></span></button>
			</s:if><s:else>
				<button class="btn" onclick="disableMenu();"><span><span ><s:text name="menuManager.forbidden"></s:text></span></span></button>
			</s:else>
			<!-- 
			...
			<script type="text/javascript">
			 ...
			 <div class="btndiv" id="_importbtn" style="*top:-2px;">
				<button  class="ui-button ui-widget ui-state-default ui-button-text-only ui-corner-left" id="parentImportBtn">
					<span class="ui-button-text"><s:text name="menuManager.import"></s:text></span>
				</button>
				<button  title="更多"  class="ui-button ui-widget ui-state-default ui-button-icon-only ui-corner-right" id="select">
					<span class="ui-button-icon-primary ui-icon ui-icon-triangle-1-s"></span>
					<span class="ui-button-text"><s:text name="menuManager.more"></s:text></span>
			...
			<div id="importbtn" class="flag">
				<ul style="width: 120px;">
					<li><a href="#" onclick="exportMenu();"><s:text name="menuManager.export"></s:text></a></li>
					<s:if test='#versionType=="online"'>
						<li><a href="#" onclick="demonstrateOperate();"><s:text name="menuManager.importCustom"></s:text></a></li>
					</s:if><s:else>
						<li><a href="#" onclick="importCustomSystem();"><s:text name="menuManager.importCustom"></s:text></a></li>
					</s:else>
					<li><a href="#" onclick="exportCustomSystem();"><s:text name="menuManager.exportCustom"></s:text></a></li>
				</ul>
			</div>
			<script type="text/javascript">
				$(document).ready(function() {
