	...
	isUsingComonLayout=false;
		$(document).ready(function () {
			$("#pageForm").validate({
				...
				messages: {
					webpageName: iMatrixMessage["common.required"]//必填
				}
			});
		});
		function doAddWebpage(){
			if($.trim($('#webpageName').attr('value'))==''){
				_showMessage('msg','<s:text name="pageNameReqiure"/>');
				return;
			}
			$.ajax({
				type: "POST",
				...
				error:function(){
					_showMessage('msg','<s:text name="addpageError"/>');
				}
			}); 
		}
		function getData(){
...
<div class="opt-body" >
	<div class="opt-btn" >
		<button id='savebtn' class='btn' onclick="submitTag();"><span><span><s:text name="portal.save"/></span></span></button>
	</div>
	<div class="opt-content" >
	<div id='msg' style="color: red;"></div>
	
	<form action="${portalCtx}/index/index-savewebpage.htm" method="post" name="pageForm" id="pageForm">
	<p><s:text name="TabName" />:<input size="60" maxlength="10" style="width:300px" name="webpageName" id="webpageName" value="${webpage.name}"/><span class="required">*</span></p>
		<table>
			<tr>
			...
			<tr>
				<td> <input type="radio" <s:if test="webpageId==null||webpage.columns==3">checked="checked"</s:if> name="columnSize" value="3"/><s:text name="Threecolumns" /></td>
				<td> <input type="radio" <s:if test="webpageId!=null&&webpage.columns==2">checked="checked"</s:if> name="columnSize" value="2"/><s:text name="Twocolumns" /></td>
				<td> <input type="radio" <s:if test="webpageId!=null&&webpage.columns==1">checked="checked"</s:if> name="columnSize" value="1"/><s:text name="Column" /> </td>
			</tr>
		</table>
