...
<head>
	...
	function validateWidget(){
		$("#inputForm").validate({
			...
			messages: {
				...
				url:iMatrixMessage["common.required"]//必填
			}
		});
	}
	...
	function isWidgetExist(){
		$.ajax({
			   ...
			   success: function(msg){
				   if(msg=="true"){
					  iMatrix.alert("<s:text name='window_Numbers_already_exist'/>");
				   }else{
					   $("#inputForm").submit();
				   }
			   }
	...
	function updateWiget(){
		...
		if(ids.length<=0){
			iMatrix.alert("<s:text name='Please_choose_the_need_edit_record'/>！");
			return;
		}else if(ids.length>1){
			iMatrix.alert("<s:text name='doNot_select_multiple_records'/>！");
			return;
		}else{
	...
	function validateDeleteWidget(){
		var ids=jQuery("#widgetPage").getGridParam('selarrrow');
		if(ids.length<=0){
			iMatrix.alert("<s:text name='select_need_to_delete_record'/>！");
			return;
		}
			   ...
			   success: function(msg){
				   ...
				   if(msg==""){
					  deleteWiget();
					   ...
					   iMatrix.confirm({
							message:msg+"<s:text name='form_being_used_sure_delete'/>?",
							confirmCallback:validateDeleteWidgetOk
						});
				   }
			   }
		});
	...
	function validateDeleteWidgetOk(){
		 deleteWiget();
	}
	...
	function changeIframeable(obj){
		var checkedable = $(obj).attr("checked");
		$("#iframeable").attr("value",checkedable);
		if($(obj).attr("checked")&&$("#widgetHeight").val()==""){
			$("#widgetHeight").val(500);
		}
		if(checkedable){
			$("#autoLoginTr").removeAttr("style");
		}else{
			$("#autoLoginTr").css("display","none");
		}
	}
	
	function changeAutoLoginable(obj){
		$("#autoLoginable").attr("value",$(obj).attr("checked"));
	}
	function setParamValue(cellvalue, options, rowObject){
	 ...
	 function optionGroupNameClick(obj){
		 custom_ztree({url:webRoot+'/index/option-group-tree.htm',
				...
				width:300,
				height:400,
				title:iMatrixMessage["formManager.optionInfo.title"],
...
<form action="" name="defaultForm" id="defaultForm" method="post">
	...
	<div class="ui-layout-west">
		...
		<s:iterator value="menus">
			<div id="bs_${systemId }" class="west-notree" onclick="changeSystem('${systemId}');"><a>${name }</a></div>
		</s:iterator>
	</div>
	<div class="ui-layout-center">
		<div class="opt-body">
			<aa:zone name="widget-zones">
				<div class="opt-btn">
					<button class="btn" onclick='createWidget();' id="create"><span><span ><s:text name="portal.add"/></span></span></button>
					<s:if test='#versionType=="online"'>
						<button class="btn" onclick="demonstrateOperate();"><span><span ><s:text name="portal.update"/></span></span></button>
						<button class="btn" onclick="demonstrateOperate();"><span><span ><s:text name="portal.delete"/></span></span></button>
					</s:if><s:else>
						<button class="btn" onclick="updateWiget();"><span><span ><s:text name="portal.update"/></span></span></button>
						<button class="btn" onclick="validateDeleteWidget();"><span><span ><s:text name="portal.delete"/></span></span></button>
					</s:else>
				</div>
