...
<div class="opt-body">
	<div class="opt-btn">
		<a class="btn" href="#" onclick="saveBaseSetting('defaultForm','${portalCtx}/index/base-setting-save.htm');"><span><span ><s:text name="portal.save" /></span></span></a>
	</div>
	<div id="opt-content">
		<form action="" name="defaultForm" id="defaultForm" method="post">
			<input name="id" id="id" value="${id }" type="hidden">
			<FIELDSET><legend><s:text name="selfmessageSetting" /></legend>
				<table class="form-table-without-border">
					<tr>
						<td><s:text name="isShowWindow" /></td>
					...
					<tr>
						<td><s:text name="refrashTimer" /></td><td><input style="width: 50%;" name="refreshTime" id="refreshTime" value="${refreshTime}" onkeyup="value=value.replace(/[^\d]/g,'');value=value.length>3?value.substr(0,3):value"/><span style="color: green;"><s:text name="showSecond" /></span></td>
					</tr>
					<tr>
						<td><s:text name="showCount" /></td><td><input style="width: 50%;" name="showRows" id="showRows" value="${showRows}" onkeyup="value=value.replace(/[^\d]/g,'');value=value.length>3?value.substr(0,3):value"/><span style="color: green;"><s:text name="article" /></span></td>
					</tr>
				</table>
			</FIELDSET>	
		</form>
