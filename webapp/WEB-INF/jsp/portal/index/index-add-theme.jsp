...
<head>
	...
	<script type="text/javascript">
		function changeThemeState(url){
			...
			if(ids.length<=0){
				iMatrix.alert("<s:text name='selectRecord'/>");
			}else {
				$.ajax({
					data:{ids:ids.join(",")},
					type:"post",
	...
	<div class="ui-layout-center">
		<div class="opt-body">
			<aa:zone name="widget-zones">
				<div class="opt-btn">
					<button  class='btn' onclick="iMatrix.showSearchDIV(this);"><span><span><s:text name="portal.query"/></span></span></button>
					<s:if test='#versionType=="online"'>
						<button class='btn' onclick="demonstrateOperate();"><span><span><s:text name="portal.add"/></span></span></button>
						<button class='btn' onclick="demonstrateOperate();"><span><span><s:text name="portal.delete"/></span></span></button>
					</s:if><s:else>
						<button class='btn' onclick="iMatrix.addRow();"><span><span><s:text name="portal.add"/></span></span></button>
						<button class='btn' onclick="iMatrix.delRow();"><span><span><s:text name="portal.delete"/></span></span></button>
					</s:else>
					<!--  
