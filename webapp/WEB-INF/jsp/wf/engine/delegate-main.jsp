...
<head>
	...
	function getAssignedRoles(id){
		$.ajax({
	        ...
	        error:function(){
	        	iMatrix.alert(iMatrixMessage["wf.engine.delegate.error"]);
	        }
		});
	}
	//用户选的角色 动态赋值
...
//					callBack:function(){insertInputValue(jstree.getLoginName(),jstree.getName(),jstree.getId());}});
	...
	function selectTache(){
		...
		if(processId=="0"){
			iMatrix.alert(iMatrixMessage["wf.engine.selectProcess"]);
		}else{
			custom_ztree({url:webRoot+'/engine/delegate-main-getLink.htm',
				inputObj:'activityName',
				width:500,
				height:400,
				title:iMatrixMessage["wf.engine.selectLink"],
...
<div class="ui-layout-center">	
	<div class="opt-body">
		...
		<aa:zone name="delegatemainlist">
			<div class="opt-btn">
				<button class="btn" onclick="addForm('${wfCtx}/engine/delegate-main-input.htm');"><span><span><s:text name="menuManager.new"></s:text></span></span></button>
				<button class="btn" onclick="updateForm('${wfCtx}/engine/delegate-main-input.htm');"><span><span><s:text name="menuManager.update"></s:text></span></span></button>
				<button class="btn" onclick="deleteForm('${wfCtx}/engine/delegate-main-delete.htm');"><span><span><s:text name="menuManager.delete"></s:text></span></span></button>
				<button class="btn" onclick="enalbeDelegateMain('${wfCtx}/engine/delegate-main-start.htm');"><span><span><s:text name="menuManager.start"></s:text></span></span></button>
				<button class="btn" onclick="enalbeDelegateMain('${wfCtx}/engine/delegate-main-end.htm');"><span><span><s:text name="process.cancel"></s:text></span></span></button>
			</div>
