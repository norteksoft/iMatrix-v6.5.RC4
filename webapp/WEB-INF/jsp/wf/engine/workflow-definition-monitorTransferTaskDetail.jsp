				...
				<div id="opt-content" >
					<form id="searchSubmit" name="searchSubmit" action="" method="post">
						...
						<s:if test="position=='taskUserActive'">
							<view:jqGrid url="${wfCtx}/engine/workflow-definition-monitor-transfer-task-list-user-data.htm?position=${position }&transactorId=${transactorId }&transactorName=${transactorName }&lastTransactTimeStart=${lastTransactTimeStart }&lastTransactTimeEnd=${lastTransactTimeEnd }" pageName="tasks" code="WORKFLOW_MONITOR_TRANSFER_TASK_DETAIL" gridId="main_table"></view:jqGrid>
						</s:if><s:elseif test="position=='taskUserHistory'">
							<view:jqGrid url="${wfCtx}/engine/workflow-definition-monitor-transfer-task-list-user-data.htm?position=${position }&transactorId=${transactorId }&transactorName=${transactorName }&lastTransactTimeStart=${lastTransactTimeStart }&lastTransactTimeEnd=${lastTransactTimeEnd }" pageName="historyTasks" code="WORKFLOW_MONITOR_TRANSFER_HISTORY_TASK_DETAIL" gridId="main_table_history"></view:jqGrid>
						</s:elseif>
					</form>	
				</div>
			</aa:zone>
