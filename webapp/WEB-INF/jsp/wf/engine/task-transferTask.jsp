...
<head>
	...
	function back(){
		goBack("defaultForm", "${wfCtx}/engine/task-transferTask.htm", "myprocess","");
	}
	...
	function changeTransferList(flag){
		if(flag=='current'){
			ajaxSubmit('defaultForm', '${wfCtx}/engine/task-transferTask.htm?isEnd=true', 'endProcess');
		}else{
			ajaxSubmit('defaultForm', '${wfCtx}/engine/task-historyTransferTask.htm?isEnd=true', 'endProcess');
		}
	}
	function viewHistoryTask(ts1,cellval,opts){
...
<div class="opt-body">
	<aa:zone name="myprocess">
			...
			<div id="opt-content" >
				<div id="tabs">
						<ul>
							<li><a href="#tabs-1" onclick="changeTransferList('current');"><s:text name="wf.engine.current.task"></s:text></a></li>
							<li><a href="#tabs-1" onclick="changeTransferList('history');"><s:text name="wf.engine.history.task"></s:text></a></li>
						</ul>
					<div id="tabs-1">
						<aa:zone name="endProcess">
								<view:jqGrid url="${wfCtx}/engine/task-transferTask.htm?isEnd=${isEnd }" pageName="page" code="TRANSFER_TASK" gridId="main_table"></view:jqGrid>
						</aa:zone>
					</div>
			   </div>
			  </div>
