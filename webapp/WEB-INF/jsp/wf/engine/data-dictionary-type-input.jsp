		...
		<aa:zone name="dict_zone">
				<div class="opt-btn">
					<security:authorize ifAnyGranted="wf_engine_dict_type_save">
						<button  class='btn' onclick="save_type_form();" hidefocus="true"><span><span><s:text name="menuManager.save"></s:text></span></span></button>
					</security:authorize>	
					<button class='btn' onclick='setPageState();dataDicBack("defaultForm","${wfCtx}/engine/data-dictionary-type.htm","dict_zone","page");' hidefocus="true"><span><span><s:text name="menuManager.back"></s:text></span></span></button>
				</div>
				<div id="backTypeMsg" style="margin-left: 12px;color: green;"><s:actionmessage theme="mytheme"/></div>
			<form action="${wfCtx}/engine/data-dictionary-type-save.htm" name="dictTypeForm" id="dictTypeForm" method="post">
					<input name="id" id="id" type="hidden" value="${id}"/>
					<br/>
					<table>
						<tr>
							<td><s:text name="wf.dictionary.typeNumber"></s:text>：</td>
							<td>
								<s:if test="id==null">
						...
						<tr>
							<td><s:text name="wf.dictionary.typeName"></s:text>：</td>
							<td><input id="name" name="name"  type="text" value="${name }" maxlength="30"/><span class="required">*</span></td>
							<td></td>
						</tr>
						<tr>
							<td><s:text name="wf.dictionary.includeType"></s:text>：</td>
							<td>
