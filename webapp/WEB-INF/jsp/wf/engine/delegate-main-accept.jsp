...
<head>
	...
	function changeAcceptSet(inHistory){
		if(inHistory=="true"){//历史任务表
			ajaxSubmit('defaultForm',"${wfCtx}/engine/delegate-main-myAccept.htm?isDone=${isDone}&inHistory=true",'myprocess');
		}else{//当前任务表
			ajaxSubmit('defaultForm',"${wfCtx}/engine/delegate-main-myAccept.htm?isDone=${isDone}&inHistory=false",'myprocess');
		}
	}
...
<div class="opt-body">
	<s:if test="isDone">
		...
		--><div id="tabs">
			<ul>
				<li><a href="#tabs-1" onclick="changeAcceptSet('false');"><s:text name="wf.engine.current.task"></s:text></a></li>
				<li><a href="#tabs-1" onclick="changeAcceptSet('true');"><s:text name="wf.engine.history.task"></s:text></a></li>
			</ul>
			<div id="tabs-1">
				<aa:zone name="myprocess">
					<div id="opt-content" >
						<view:jqGrid url="${wfCtx}/engine/delegate-main-myAccept.htm?isDone=${isDone}&inHistory=false" pageName="tasks" code="WORKFLOW_TASK_ACCEPT" gridId="main_table"></view:jqGrid>
					</div>
				</aa:zone>
			</div>
		</div>
		...
		<aa:zone name="myprocess">
			...
			<div id="opt-content" >
				<view:jqGrid url="${wfCtx}/engine/delegate-main-myAccept.htm?isDone=${isDone}" pageName="tasks" code="WORKFLOW_TASK_ACCEPT" gridId="main_table"></view:jqGrid>
			</div>
		</aa:zone>
