...
<head>
	...
	<title><s:text name="basicSetting.rankManager"/></title>
	...
	function supValidate(){
		...
		$.ajax({
			   ...
			   success: function(data, textStatus) {
				   if( data == "cz" ){
		            	iMatrix.alert(iMatrixMessage["basicSetting.fatherValidate"]);
		            	return;
				   }else{
	...
	function validateRank(){
		$("#dictRankForm").validate({
			...
			messages: {
				title:iMatrixMessage["menuManager.required"],
				name: iMatrixMessage["menuManager.required"],
				userNames:iMatrixMessage["menuManager.required"]
			}
		});
	}
	...
	function chooseUser(btnId,id){
		...
		if(btnId=="selectUser"){
			$("#"+btnId).colorbox({href:webRoot+'/rank/user-tree.htm?id='+id,iframe:true, width:400, height:500,overlayClose:false,title:iMatrixMessage['rankSelectTitle'],onClosed:function(){$("#"+btnId).removeClass("cboxElement");}});
		}else if(btnId=="selectSuperiorUser"){
			$("#"+btnId).colorbox({href:webRoot+'/rank/superior-user-tree.htm?id='+id,iframe:true, width:400, height:500,overlayClose:false,title:iMatrixMessage["basicSetting.userTree"],onClosed:function(){$("#"+btnId).removeClass("cboxElement");}});
		}
	}
...
<div class="ui-layout-center">
	<div class="opt-body">
		...
		<aa:zone name="dict_zone">
			<div class="opt-btn">
				<button class="btn" onclick="createRank();"><span><span><s:text name="menuManager.new"></s:text></span></span></button>
				<s:if test='#versionType=="online"'>
				<button class="btn" onclick="alert('为确保系统的正常演示，屏蔽了该功能');"><span><span ><s:text name="menuManager.delete"></s:text></span></span></button>
				</s:if><s:else>
				<button class="btn" onclick="deleteRank();"><span><span ><s:text name="menuManager.delete"></s:text></span></span></button>
				</s:else>
			</div>
