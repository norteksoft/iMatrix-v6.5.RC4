...
function _signatureSave(){
	...
	$.ajax({
		   ...
		   success: function(data, textStatus){
			   if(data=='true'){
					$("#backMsg").html("<font class='onError'><nobr>"+iMatrixMessage["basicSetting.registerInfo"]+"</nobr></font>");
					showMsg("backMsg");
				}else{
	...
	function _upload_signature_picture(){
			swfu = new SWFUpload({
				...
				button_width: 250,
				button_height: 18,
				button_text : '<span class="button">'+iMatrixMessage["basicSetting.uploadMaxLength"]+'</span>',
	...
	function fileDialogBefore(){
		...
		if(id==""||id==null){
			...
			iMatrix.alert(iMatrixMessage["basicSetting.uploadValidateInfo"]);
			swfu.eventQueue=[];
		}else{
			validateSign=true;
	...
	function updateSignature(){
		...
		if(signature.length<=0){
			...
			$("#message").html("<font class=\"onError\"><nobr>"+iMatrixMessage.selectOnlyOneInfo+"</nobr></font>");
			showMsg("message");
		}else{
	...
	function deleteSignature(){
		...
		if(signatures.length<=0){
			...
			iMatrix.confirm({
				...
				confirmCallback:deleteSignatureOk,
				parameters:signatureDeleteIds
			});
		}
	}
	function deleteSignatureOk(signatureDeleteIds){
		setPageState();
	...
	function signatureDeleteCallBack(){
		$("#message").html("<font class='onSuccess'><nobr>"+iMatrixMessage["permission.deleteSuccess"]+"</nobr></font>");
		showMsg("message");
	}


	function _imageView(ts1,cellval,opts){
		var vsrc=webRoot+"/signature/signature-picture-show.htm?signatureId="+opts.id+"&___time="+new Date().getTime();
		if(opts.pictureSrc==undefined||opts.pictureSrc==""||opts.pictureSrc=="&nbsp;"){
			vsrc=imatrixRoot+"/images/photo.jpg?___time="+new Date().getTime();
		}
	...
	<form id="defaultForm" name="defaultForm"action="" method="post" ></form>
      <aa:zone name="signature_zone">
		<div class="opt-btn">
			...
			<button class='btn' onclick="createSignature()"><span><span><s:text name="menuManager.new"></s:text></span></span></button>
			<button class='btn' onclick="updateSignature()"><span><span><s:text name="menuManager.update"></s:text></span></span></button>
			<button class='btn' onclick="deleteSignature()"><span><span><s:text name="menuManager.delete"></s:text></span></span></button>
		</div>
