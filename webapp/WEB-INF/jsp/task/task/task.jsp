...
<head>
	...
	<title><s:text name="task.menu.toDoList"></s:text></title>
	...
	function assign(){
		if(validateReadioTaskIds()){
			...
			$.colorbox({href:webRoot+'/task/tree.htm',iframe:true, innerWidth:300, innerHeight:400,overlayClose:false,title:iMatrixMessage['wf.engine.choose']});
		}else{
			iMatrix.alert(iMatrixMessage["selectInfo"]);
		}
	}
	...
	function showErrorPageNo(){
		iMatrix.alert('<s:text name="page.errorPageNo"></s:text>');
	}
	...
	function taskListAssgin(){
		var msg = "<font class=\"onSuccess\"><nobr>"+iMatrixMessage['task.assign.success.info']+"</nobr></font>";
		$("#backMsg").html(msg);
		$("#backMsg").show();
	...
	<div class="opt-body">
		<aa:zone name="product_task_all">
			...
			<input type="hidden"" id="showInputId"  value="">
				<button class="btn" onclick="$('#flag').hide();iMatrix.showSearchDIV(this);" id="searchBtn"><span><span><s:text name="formManager.search"></s:text></span></span></button>
				<button class="btn" onclick="drawTask();"><span><span><s:text name="task.button.receive"></s:text></span></span></button>
				<button class="btn" onclick="assign();"><span><span><s:text name="task.button.assign"></s:text></span></span></button>
