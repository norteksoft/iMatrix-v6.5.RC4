...
<head>
	...
	var subCompanyFieldName="~~subCompanyId";//标准字段创建人的分支字段名
		...
		function create_tree(){
			...
			if(standardField==creatorFieldName){//创建人时选择的节点是否正确
				...
				treeId ="role_tree"; 
			}else if(standardField==subCompanyFieldName){//创建人分支
				treeId ="subcompany_tree"; 
			}
		...
		function select(){
			...
			if(standardField==creatorFieldName){//创建人时选择的节点是否正确
				...
				var values = getRoleTreeValue("role",roleFieldName);
				conditionNames = values[0];
				conditionValues = values[1];
			}else if(standardField==subCompanyFieldName){//创建人分支
				var values = getRoleTreeValue("branch",subCompanyFieldName);
				conditionNames = values[0];
				conditionValues = values[1];
			}
			if(standardValues==""&&conditionValues==""){
				//请选择正确的节点！
				iMatrix.alert(iMatrixMessage["acs.data.rule.pleaseChooseTheRightNode"]);
				return;
			}
		...
		function getRoleTreeValue(feildType,standardFieldName){
			var standardField=$("#standardField").val();
			...
			for(var i=0;i<checkedNodes.length;i++){
				...
				if(values[0]=="standard"){//标准值
					...
					if(standardField==standardFieldName){//创建人角色或创建人分支
						if(type==feildType){//选中的角色或分支节点
...
<div class="opt-body">
	...
	<aa:zone name="main_zone">
		...
		<div class="opt-btn">
			<button class="btn" onclick="<s:if test="standardField=='~~roleId'||standardField=='~~subCompanyId'">select();</s:if><s:else>selectMans();</s:else>"><span><span ><s:text name="user.sure"></s:text></span></span></button>
		</div>
		<div id="opt-content">
			<s:if test="standardField=='~~roleId'">
			...
			</s:if><s:elseif test="standardField=='~~subCompanyId'">
				<table>
				<tr>
				<td >
					<input id="searchroleInput" /></td><td ><a class="search-btn" href="#" onclick="tree.searchNodes($('#searchroleInput').val());" ><b class="ui-icon ui-icon-search"></b></a>
				...
				<div id="message" style="display: none;"></div>
				<ul class="ztree" id="subcompany_tree"></ul>
			</s:elseif><s:else>
				<script type="text/javascript">
					$(document).ready(function() {
						$( "#tabs" ).tabs();
				...
				<div id="tabs">
					<ul>
						<li><a href="#tabs-1" onclick="changeViewSet('org');"><s:text name="authorization.organization"></s:text></a></li>
						<li><a href="#tabs-2" onclick="changeViewSet('standard');"><s:text name="authorization.standardValue"></s:text></a></li>
					</ul>
					<div id="tabs-1">
