	...
	<script type="text/javascript">
			...
			if(id==null){
				...
				if(dataType=="ENUM"){
					...
					if(objectPath==""||typeof(objectPath)=='undefined'){
						iMatrix.alert(iMatrixMessage["authorization.enumConfig"]);
						return;
					}else{
						if(valueUrl==""||typeof(valueUrl)=='undefined'){
							iMatrix.alert(iMatrixMessage["authorization.enumConfigAddress"]);
							return;
						}
		...
		function submitStandardColumn(){
			...
			if(fieldNames.length<=0){
				//请选择一个字段！
				iMatrix.alert(iMatrixMessage["acs.data.rule.pleaseAField"]);
			}else{
				var currentInputId="${currentInputId }";
				var conditionId=currentInputId.substring(0,currentInputId.lastIndexOf("_"));
...
<div class="opt-body">
	...
	<aa:zone name="main_zone">
		...
		<aa:zone name="btnZone">
			<div class="opt-btn">
				<button class="btn" onclick="submitDataRule();"><span><span ><s:text name="menuManager.confirm"></s:text></span></span></button>
			</div>
		</aa:zone>
		<div id="opt-content">
			<div id="tabs">
				<ul>
					<li><a href="#tabs-1" onclick="changeViewSet('column',this);"><s:text name="authorization.formField"></s:text></a></li>
					<li><a href="#tabs-1" onclick="changeViewSet('standard',this);"><s:text name="authorization.standardField"></s:text></a></li>
				</ul>
				<div id="tabs-1">
