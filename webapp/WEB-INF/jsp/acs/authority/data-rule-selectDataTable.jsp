			...
			if(typeof(data)!='undefined'&& node!=""&&node.pId!=null){
				...
				if("${dataTableId}"!=""&&"${listViewId}"!=""){//快速授权中
					if("${dataTableId}"!=values[0]){
						//请修改该快速授权对应的列表
						iMatrix.alert(iMatrixMessage['permission.updateTables']);
				...
				//请选择数据表！
				iMatrix.alert(iMatrixMessage['permission.selectDataTables']);
			}
		}
	</script>
	<style type="text/css">
...
<div class="opt-body">
	...
	<aa:zone name="main_zone">
		<div class="opt-btn">
			<button class="btn" onclick="select();"><span><span ><s:text name="common.sure"/></span></span></button>
		</div>
		<div id="opt-content">
