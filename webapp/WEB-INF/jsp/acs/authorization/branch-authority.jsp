		...
		function addRole(){
			$.ajax({
				...
				success:function(data, textStatus){
					if("ok"==data){
						init_colorbox(webRoot+"/authorization/branch-authority-addRole.action?selectPageFlag="+true+"&branchesId="+$("#branchesId").val(),iMatrixMessage["authorization.addRole"],600,500);
					}else{
						//无权限为此分支机构添加角色！
						iMatrix.alert(iMatrixMessage["authorization.noAuthAddRole"]);
					}
				},
			...
			$.ajax({
				...
				success:function(data, textStatus){
					if("ok"==data){
						...
						//无权限为此分支机构添加管理员！
						iMatrix.alert(iMatrixMessage["authorization.noAuthAddManager"]);
					}
				},
				complete:function(XMLHttpRequest, textStatus){},
		        error:function(){
		...
		function saveBranchesManager(){
			...
			*/
			$.ajax({
				...
				success:function(data, textStatus){
					if("ALLCOMPANYID"==data){
						showMessage("message", "<font class=\"onError\"><nobr>"+iMatrixMessage['user.notAddAllUserManager']+"</nobr></font>");//不能把公司中的所有用户添加为分支机构管理员
					}else if("ok"!=data){
						showMessage("message", "<font class=\"onError\"><nobr>"+data+"</nobr></font>");
					}
					ajaxSubmit('defaultForm', webRoot+'/authorization/branch-authority.action', 'acs_content',getAddManagerContentHeight);
				...
				success:function(data, textStatus){
					if("ok"==data){
						...
						if(userIdsLength==0&&roleIdsLength==0){
							...
							//确定要移除所选项?
							iMatrix.confirm({
								message:iMatrixMessage["authorization.sureDeleteSeledted"],
								confirmCallback:clearAwayOk
							});
						}
					}else{
						//无权限移除此分支机构下的人员和角色！
						iMatrix.alert(iMatrixMessage["authorization.noAuthRemoveUserAndRole"]);
					}
				},
				complete:function(XMLHttpRequest, textStatus){},
		        error:function(){
...
<div class="opt-body">
	<s:if test="branchesId != null">	
		...
		<div class="opt-btn">
			<security:authorize ifAnyGranted="authorization-branch-authority-addManager"><button  class='btn' onclick="addBranchesManager();"><span><span><s:text name="authorization.addManager"/><!-- 添加管理员 --></span></span></button></security:authorize>
			<security:authorize ifAnyGranted="authorization-branch-authority-addRole"><button  class='btn' onclick="addRole();"><span><span><s:text name="authorization.addRole"/><!--  添加角色--></span></span></button></security:authorize>
			<security:authorize ifAnyGranted="authorization-branch-authority-clearAway"><button  class='btn' onclick="clearAway();"><span><span><s:text name="authorization.remove"/><!--  移除--></span></span></button></security:authorize>
		</div>
		...
		<div id="opt-content">
			<div style="padding: 5px;">
				<table border="1" cellpadding="0" cellspacing="0" class="leadTable">
					<thead>
						<tr>
							<th style="text-align:left; padding-left:4px;"><input type="checkbox" id="branchAuthority_users" onclick="checkAll(this);"/><s:text name="authorization.personName"/><!-- 人员名称--></th>
							<th style="text-align:left; padding-left:4px;"><input type="checkbox" id="branchAuthority_roles" onclick="checkAll(this);"/><s:text name="authorization.role"/><!-- 角色--></th>
						</tr>
					</thead>
					<tbody>
						<tr>
