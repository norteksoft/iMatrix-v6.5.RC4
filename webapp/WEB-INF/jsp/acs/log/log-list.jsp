	...
	function deleteSysLogin(){
		...
		if(sysId==""){
   		...
   		if(uIds == "" || uIds.length<=0){
             ...
             //确定删除吗？
             iMatrix.confirm({message:iMatrixMessage["log.sureDelete"],confirmCallback:deleteSysLoginConfirm,parameters:{uIds:uIds,sysId:sysId}});
		}
	}
   function deleteSysLoginConfirm(obj){
	   if($("#_loading_div_id").length==0){
		...
		$.post(url, {sysId:$("#sysId").val(),syIds:obj.uIds+"="+obj.sysId}, function(data) {
	...
	function deleteAllSysLogin(){
		...
		iMatrix.confirm({message:iMatrixMessage["log.sureDelete"],confirmCallback:deleteAllSysLoginConfirm,parameters:sysId});
	}
	function deleteAllSysLoginConfirm(sysId){
		if($("#_loading_div_id").length==0){
			$(".opt-btn").append('<div id="_loading_div_id" class="loading">'+iMatrixMessage['log.nowDelete']+'</div>');
		}
		$("#_loading_div_id").show();
...
<div class="opt-body">
	<div class="opt-btn">
		...
		<s:if test='#versionType=="online"'>
			<security:authorize ifAnyGranted="delete_sysLogs"><button  class='btn' onclick="demonstrateOperate();"><span><span<s:text name="common.delete"/></span></span></button></security:authorize>
			<security:authorize ifAnyGranted="delete_sysLogs"><button  class='btn' onclick="demonstrateOperate();"><span><span><s:text name="common.deleteAll"/></span></span></button></security:authorize>
		</s:if><s:else>
			<security:authorize ifAnyGranted="delete_sysLogs"><button  class='btn' onclick="deleteSysLogin();"><span><span><s:text name="common.delete"/></span></span></button></security:authorize>
			<security:authorize ifAnyGranted="delete_sysLogs"><button  class='btn' onclick="deleteAllSysLogin();"><span><span><s:text name="common.deleteAll"/></span></span></button></security:authorize>
		</s:else>
		<security:authorize ifAnyGranted="acs_export_log"><button  class='btn' onclick="iMatrix.export_Data('${acsCtx}/log/log-export.action?sysId=${sysId}','true');"><span><span><s:text name="user.export"/></span></span></button></security:authorize>
	</div>
	<div id="opt-content">
