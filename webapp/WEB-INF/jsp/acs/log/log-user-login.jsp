...
<html>
   ...
   <script  type="text/javascript" src="${imatrixCtx}/widgets/calendar/WdatePicker.js"></script>
	...
	function deleteUserLogin(){
		var uIds = getSelectedIds();
   		if(uIds == "" || uIds.length<=0){
			...
			iMatrix.confirm({message:iMatrixMessage["log.sureDelete"],confirmCallback:deleteUserLoginConfirm,parameters:uIds});
		}
	}
	function deleteUserLoginConfirm(uIds){
    ...
    function deleteAllUserLogin(){
  		iMatrix.confirm({message:iMatrixMessage["log.sureDeleteAll"],confirmCallback:deleteAllUserLoginConfirm});
	}
	function deleteAllUserLoginConfirm(){
		if($("#_loading_div_id").length==0){
			$(".opt-btn").append('<div id="_loading_div_id" class="loading">'+iMatrixMessage['log.nowDelete']+'</div>');
		}
		$("#_loading_div_id").show();
  ...
  //复选框至少有一个被选中
	...
	function _exportLoginLog(){
		if($('#login_log_table').children('tbody').children('tr').length<=1){
			iMatrix.alert(iMatrixMessage['log.noResultExport']);
			return;
		}
...
<div class="ui-layout-center">
	<div class="opt-body">
		<div class="opt-btn">
			<security:authorize ifAnyGranted="userLoginLog"><button  class='btn' onclick="iMatrix.showSearchDIV(this);"><span><span><s:text name="common.search"/></span></span></button></security:authorize>
			<s:if test='#versionType=="online"'>
				<security:authorize ifAnyGranted="delete_loginLog"><button  class='btn' onclick="demonstrateOperate();"><span><span><s:text name="common.delete"/></span></span></button></security:authorize>
				<security:authorize ifAnyGranted="delete_loginLog"><button  class='btn' onclick="demonstrateOperate();"><span><span><s:text name="common.deleteAll"/></span></span></button></security:authorize>
			</s:if><s:else>
				<security:authorize ifAnyGranted="delete_loginLog"><button  class='btn' onclick="deleteUserLogin();"><span><span><s:text name="common.delete"/></span></span></button></security:authorize>
				<security:authorize ifAnyGranted="delete_loginLog"><button  class='btn' onclick="deleteAllUserLogin();"><span><span><s:text name="common.deleteAll"/></span></span></button></security:authorize>
			</s:else>
			<security:authorize ifAnyGranted="acs_log_export_login_log"><button  class='btn' onclick="iMatrix.export_Data('${acsCtx}/log/log-exportLoginLog.action','true');"><span><span><s:text name="user.export"/></span></span></button></security:authorize>
		</div>
		<div id="opt-content">
