...
<head>
	...
	<title><s:text name="common.departmentManager"/></title>
	...
	<script type="text/javascript">
    		...
    		$("#inputForm").validate({
				...
				rules: {
		     	...
		     	,messages: {
			    	...
			    	'weight':iMatrixMessage["common.required"],//必填
			    	shortTitle:{
		     			maxlength: iMatrixMessage["department.longest30Characters"]//最长30字符
			     	},
			    	summary:{
		...
		function checkDeptName(){
			...
			if(code.indexOf(">")>=0||code.indexOf("<")>=0||code.indexOf("\"")>=0||code.indexOf("'")>=0
				...
				//编码不能包含符号>、<、\"、'、/、空格
				iMatrix.alert(iMatrixMessage['department.codeFail'] +" >、<、\"、'、/、 "+iMatrixMessage['department.Spaces']);
				return;
			}else  if(iMatrix.validateValueTrim(departmentName)){
				//部门名称前后不能包含空格
				iMatrix.alert(iMatrixMessage['department.departmentNameNotContainesSpaces']);
				return;
			}else  if(iMatrix.validateValueTrim(shortTitle)){
				//简称前后不能包含空格
				iMatrix.alert(iMatrixMessage['department.abbreviationNameNotContainesSpaces']);
				return;
			}
			$.ajax({
				type : "POST",
				...
				success : function(data){
					if(data!=''){
                       if(data=='departmentCodeFalse'){
                          //部门编码重复
                          iMatrix.alert(iMatrixMessage['department.departmentCodeRepeat']+"!");
                       }else if(data=='departmentNameFalse'){
                    	   //部门名称重复
                    	   iMatrix.alert(iMatrixMessage['department.departmentNameRepeat']+"!");
                       }else if(data=='branchCodeFalse'){
                    	   //分支机构编码重复
                    	   iMatrix.alert(iMatrixMessage['department.branchCodeRepeat']+"!");
                       }else if(data=='branchNameFalse'){
                    	   //分支机构名称重复
                    	   iMatrix.alert(iMatrixMessage['department.branchNameRepeat']+"!");
                       }else if(data=='branchNameFalseOfCompany'){
                    	   //分支机构名称不能与集团重复
                    	   iMatrix.alert(iMatrixMessage['department.branchNameAndGroupRepeat']+"!");
                       }else if(data=='branchShortNameFalse'){
                    	   //分支机构简称重复
                    	   iMatrix.alert(iMatrixMessage['department.branchAbbreviationNameRepeat']+"!");
                       }else if(data=="branchCodeFalseOfCompanyCode"){
                           //分支机构编码与集团编码重复
                           iMatrix.alert(iMatrixMessage['department.branchCodeAndGroupCodeRepeat']);
                       }
					}else{
						$('#inputForm').submit();
					}
				...
				onerror : "",
				onwait : iMatrixMessage['department.isValidateAndWait']+"..."//正在对部门名称进行合法性校验，请稍候
			});
		}
		...
		function updateDept(){
			...
			if(deptId==''){
				//请在左边树上选择部门
				iMatrix.alert(iMatrixMessage["department.selectDepLeft"]);
				return;
			}
		...
		function deleteDept(){
			...
			var zTree = parent.$.fn.zTree.getZTreeObj("company_department");
			var selectNodes = zTree.getSelectedNodes();
			var child=selectNodes.children;
			var childLength=0;
			if(child)childLength=child.length;
			var selectNode=selectNodes[0];
			var selectNodeType=selectNode.id.split("-")[0];
			var confirmContent="";
			if(childLength>0){
				if("USERSBYDEPARTMENT"==selectNodeType){
					//请先删除子部门或分支机构
					iMatrix.alert(iMatrixMessage["department.deleteChildDepFirst"]);
				}else{
					//请先删除分支机构下的部门、人员、工作组、分支机构、分支机构授权管理、角色
					iMatrix.alert(iMatrixMessage["department.deleteBranchDep"]);
				}
				return;
			}
			if("USERSBYDEPARTMENT"==selectNodeType){
				confirmContent=iMatrixMessage['department.sureDeleDep'];//确定要删除部门吗
				confirmDelete(confirmContent,deptId);
			}else{
				confirmContent=iMatrixMessage['department.sureDeleBranch'];//确定要删除分支机构吗
				$.ajax({
					data:{deptId:deptId},
					type:"post",
		...
		function confirmDelete(confirmContent,deptId){
			iMatrix.confirm({
				...
				confirmCallback:confirmDeleteOk,
				parameters:deptId
			});
		}
		function confirmDeleteOk(deptId){
		...
		function refreshAfterDeleteDept(){
			...
			parent.$("#myIFrame").attr("src",webRoot+"/organization/department.action");
		}
		...
		function deptAddUserSubmit(){
			...
			if(typeof(users)=='undefined'||users==''){
                //请选择用户
                iMatrix.alert(iMatrixMessage['user.seleteUser']+"!");
                return;
			}
			...
			if(users!=""){
				if(!isAllInSameBranch(departmentSubCompanyId,userSubCompanyIds)){
                   	//您已经跨分支机构选人，如果继续执行原来的分支机构将不存在这些人
                     iMatrix.confirm({
                    		message:iMatrixMessage['department.branchesAcrossCandidates']+"!",
                    		confirmCallback:deptAddUserSubmitOk,
                    		parameters:{users:users,deptId:deptId}
                    	});
				}else{
					 $.ajax( {
  	    				data : {
  	    				...
  	    				success : function(data, textStatus) {
     	    				if(data!=''){
                               	iMatrix.alert(iMatrixMessage['department.loginNameBelow']+":"+data+iMatrixMessage['department.baranchHasExist']+"!");
	     	    			}
		...
		function deptAddUserSubmitOk(obj){
			$.ajax( {
 				data : {
					  choseUserIds : obj.users,chooseDepartmentId:obj.deptId
 				},
 				type : "post",
 				url : "${acsCtx}/organization/department-getRepeatUserNames.action",
 				beforeSend : function(XMLHttpRequest) {
 				...
 				success : function(data, textStatus) {
	    				if(data!=''){
                       //如下用户登录名    当前分支机构已存在,已被自动过滤,不能被添加
                       iMatrix.alert(iMatrixMessage['department.loginNameBelow']+":"+data+iMatrixMessage['department.baranchHasExist']+"!");
 	    			}
 					
 					var treeSelectedNode = $('#tree_selected_node').val();
 					//ajaxSubmit('deptAddUserForm', '${acsCtx}/organization/department!departmentAddUser.action?treeSelectedNode='+treeSelectedNode, 'acs_content,',add_user_call_back);
 					departmentAddUser(obj.users);
 				},
 				error : function(XMLHttpRequest, textStatus) {
 					iMatrix.alert(textStatus);
 				}
		...
		function removeUsers(){
			...
			if(node!=''){
               ...
               if(type=='USERSBYBRANCH'){
            	   //请选择左边树上的部门节点
            	   iMatrix.alert(iMatrixMessage["department.selectDepLefNode"]);
   				   return;
               }
			}
			...
			if(uIds==''){
				//请选择要移除的用户
				iMatrix.alert(iMatrixMessage["department.selectRemoveUser"]);
				return;
			}else{
		...
		<aa:zone name="acs_content">
			<div class="opt-btn">
				...
				<security:authorize ifAnyGranted="deleteDepartment">
					<s:if test='#versionType=="online"'><!-- 为确保系统的正常演示，屏蔽了该功能 -->
			        	<button  class='btn' onclick="demonstrateOperate();"><span><span><s:text name="common.delete" /></span></span></button>
			        </s:if><s:else>
			        	<button  class='btn' onclick="deleteDept();"><span><span><s:text name="common.delete" /></span></span></button>
			        </s:else>
				</security:authorize>
