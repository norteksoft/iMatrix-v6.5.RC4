			...
			<input type="hidden" id="branchFlag" name="branchFlag" value="${branch}"/>
				<s:if test="canEditDepartmentInfor">
					<table class="form-table-without-border">
						<tr>
							<td class="content-title" ><s:text name='department.parentDepartment'/>:</td><!-- 父部门 -->
							<td>
								...
								<a href="#" onclick="$.colorbox({href:'${acsCtx}/organization/department-tree.action',iframe:true, innerWidth:600, innerHeight:500,overlayClose:false,title:'<s:text name="role.selectDepartment"/>'}); " class="small-btn" ><span ><span><s:text name="user.choose"/></span></span></a>
								</s:if> 
							</td>
						...
						<tr>
							...
							<input type="checkbox" id="branch" <s:if test="branch">checked="checked"</s:if>  onclick="checkClick('${code}','${id}');"/><s:text name="department.WhetherBbranch"/> <!-- 是否作为分支机构 -->
							</s:if>
							<s:else>
							<input disabled="disabled" type="checkbox" <s:if test="branch">checked="checked"</s:if>  /> <s:text name="department.WhetherBbranch"/> <!-- 是否作为分支机构 -->
							</s:else>
							</td>
							<td class="content-title"></td>
							<td ></td>
				    ...
				    <table class="form-table-without-border">
						<tr>
							<td class="content-title" ><s:text name='department.parentDepartment'/>:</td>
							<td>
						...
						<tr>
							<td class="content-title"></td>
							<td ><input disabled="disabled" type="checkbox" <s:if test="branch">checked="checked"</s:if>  /><s:text name="department.WhetherBbranch"/> <!-- 是否作为分支机构 --></td>
							<td class="content-title"></td>
							<td ></td>
						</tr>
					</table>
