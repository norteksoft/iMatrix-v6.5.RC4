function add()
	{
	  ...
	  var dataSrc = $('#dataSrc').attr("value");
		if(field=="")
		{
			iMatrix.alert(iMatrixMessage["formManager.databaseFieldValidate"]);
			return;
		}	
		var control=$('#itemTitle').attr("value");
		if(control=="")
		{
			iMatrix.alert(iMatrixMessage["formManager.mapControlNameValidate"]);
			return;
		}
		...
		for(var i=1;i<rows.length;i++)
	  	{
			  ...
			  if(dataField==field)
			  {
				  iMatrix.alert(iMatrixMessage["formManager.addValidate"]);
				  return;
			  }
		}
		  var queryFlag = $("#isQuery").attr("checked")==true ? "1" : "0"; 
		  var queryFlagDesc = queryFlag=="1" ? iMatrixMessage["basicSetting.yes"] : iMatrixMessage["basicSetting.no"];
		  
		 //alert($("#map_tbl>tbody>tr:last").attr("id"));//获得tr的id
		  var str = "";
		  if($("#isQuery").css("display")=="none"){
			  str="<tr><td >"+field+"</td><td >"+fieldName+"</td><td >"+control+"</td><td ><a href=\"#\" onclick=\"del(this)\">"+iMatrixMessage["basicSetting.delete"]+"</a></td></tr>";
		  }else{
			  str="<tr><td >"+field+"</td><td >"+fieldName+"</td><td >"+control+"</td><td >"+queryFlagDesc+"</td><td ><a href=\"#\" onclick=\"del(this)\">"+iMatrixMessage["basicSetting.delete"]+"</a></td></tr>";
		  }
		  $("#map_tbl").append(str);
		  $("#dataField ").val("");
