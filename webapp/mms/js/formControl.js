...
function initUploadControl(fileTypes,fileTypeDescription){
		new SWFUpload({
			...
			button_width: 250,
			button_height: 18,
			button_text : '<span class="button">'+iMatrixMessage["formManager.maxUpload"]+'</span>',
	...
	$.getJSON(
			...
			function(data){
				...
				for(var i=0;i<files.length;i++){
					...
					var col4 = "<td ><a href='#' onclick='downloadImage(\""+files[i].filePath+"\",\""+files[i].fileName+"\");'>"+iMatrixMessage["download"]+"</a>&nbsp;&nbsp;<a href='#' onclick='deleteFile(\""+files[i].filePath+"\");'>"+iMatrixMessage["delete"]+"</a></td>";
					var row1="</tr>";
...
function deleteFile(filePath){
	iMatrix.confirm({
		...
		confirmCallback:deleteFileOk,
		parameters:filePath
	});
}
function deleteFileOk(filePath){
	$.ajax({
		   type: "POST",
