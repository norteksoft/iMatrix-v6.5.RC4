...
function _view_opinion(obj,taskId){
	if(taskId==""){
		iMatrix.alert(iMatrixMessage["common.please.save.first"]);
	}else{
		if($(obj).attr("state") == "1"){
			$(obj).attr("state", "2");
...
function saveOpinion(){
	if($("#opinions").attr("value")==""){
		iMatrix.alert(iMatrixMessage['common.opinion.error.info']);
	}else{
...
function uploadDocument(){
	documentswfu = new SWFUpload({
		...
		button_width: 250,
		button_height: 18,
		button_text : '<span class="button">'+iMatrixMessage["common.PleaseUploadTheFileMaxHundredMB"]+'</span>',//请上传文件(最大100MB)
...
function fileDialogBefore(file,swfObject){
	...
	if(typeof(upload_target)!='undefined'&&upload_target=="divFileProgressContainer"){
		...
		if(id==""||id==null){
			...
			iMatrix.alert(iMatrixMessage['common.text.file.error.info']);
...
//第一环节提交后选择办理人的回调方法
function ___selectTransactorFirstCallback(){
	___selectTransactorCallback(true);
}
//非第一环节提交后选择办理人的回调方法
function ___selectTransactorTaskCallback(){
	___selectTransactorCallback(false);
}
function ___selectTransactorCallback(isFirst){
	var taskId = $("#taskId").val();
	if(typeof(taskId)!='undefined'&&taskId!=null){
		var userIds = ztree.getIds();
		$.ajax({
			data:{transactor:userIds,taskId:taskId},
			cache:false,
			type:"post",
			url:webRoot+"/common/assign.htm",
			success:function(data, textStatus){
				if(!isFirst){
					window.parent.close();
