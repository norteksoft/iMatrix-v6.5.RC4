...
function formValidate(){
	$("#inputForm").validate({
		...
		messages: {
			...
			entityName: iMatrixMessage["menuManager.required"],
			remark:{
...
function changeTableInfo(url,callback){
	...
	if(ids==""){
		...
		if(versionType=="online"){
			...
			if(state!= "DRAFT"){
				iMatrix.alert(iMatrixMessage["formManager.updateSheetValidate"]);
				return;
			}
		}
	...
	if(ids==''){
		...
		if(versionType=="online"){
			$.each(ids, function(i){
				...
				if(state!= "DRAFT"){
					iMatrix.alert(iMatrixMessage["formManager.delelteStartForbiddenInfo"]);
					canPost = false;
				}
			});
		}
		if(canPost){
			iMatrix.confirm({message:iMatrixMessage.deleteInfo,confirmCallback:deleteTableInfoOk,parameters:{ids:ids,url:url}});
		}
	}
}
function deleteTableInfoOk(obj){
	...
	setPageState();
	ajaxSubmit("contentFrom", obj.url, "dataTablelist",deleteCallBack);
	...
	if(ids==''){
		...
		if(versionType=="online"){
			$.each(ids, function(i){
				...
				var state=jQuery("#dataTables").jqGrid("getCell",id,"tableState");
				if(state!= "DRAFT"){
					iMatrix.alert(iMatrixMessage["formManager.chooseDraftValidate"]);
					canPost = false;
				}
			});
		}
...
function checkTableName(obj){
	...
	if(tableName.length>0 && tableName.length<=100){
		if(tableName.indexOf(" ")>=0 || tableName.indexOf("\'")>=0 || tableName.indexOf("\"")>=0){
			...
			showMessage("message", "<font color=\"red\">"+iMatrixMessage["formManager.notContainInfo"]+"</font>");
		}else{
			...
			$.ajax({
				...
				success : function(data) {
					if(data == true){
						...
						showMessage("message", "<font color=\"red\">"+iMatrixMessage["formManager.formExist"]+"</font>");
					}
				},
				error : function() {
					iMatrix.alert(iMatrixMessage["serverInfo"]);//服务器繁忙，请稍后再操作...
				}
			});
		}
	}else{
		...
		showMessage("message", "<font color=\"red\">"+iMatrixMessage["formManager.lengthError"]+"</font>");
	}
}
...
function pageUlChange(flag){
	...
	if(flag == 'b'){
		...
		if(id == ""){
			showMessage("message", "<font color=\"red\">"+iMatrixMessage["formManager.saveGenerateCodenfo"]+"</font>");
		}else{
			$("#contentFrom").append(createHiddenInput('tableId', id));
	...
	$("#inputForm").validate({
		submitHandler: function() {
			if('true'==$("#flowable").val()&&''==$("#workflowCode").val()){
				iMatrix.alert(iMatrixMessage["formManager.workFlowCodeValidate"]);
				return;
			}
...
function selectParent(){
	$.colorbox({href:webRoot+"/form/data-table-selectParent.htm?menuId="+$("#men_id").val()+"&tableId="+$("#tabelId").val(),iframe:true, innerWidth:500, innerHeight:400,overlayClose:false,title:iMatrixMessage["formManager.chooseFatherClass"]});
}
