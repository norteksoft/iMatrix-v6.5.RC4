...
function buttonExecute(btn, executer){
	...
	if(pageId==undefined){
		iMatrix.alert(iMatrixMessage["mms.buttonSettingsNotTurnPage"]);//alert("此页面的【按钮设置】中没有添加此按钮的转向页面！");
		return;
	}
...
function selectWorkflow(){
	...
	if(url == '/common/select-workflow.htm'){
		$.colorbox({href:webRoot+url+"?pageId="+$('#_pageId').val(),iframe:true, innerWidth:700, innerHeight:500,overlayClose:false,onClosed:function(){noWorkflowSelected();},title:iMatrixMessage["common.selectionProcess"]});
	}
}
//选择流程定义后进入表单页面
...
function toListPage(afterRefresh){
	...
	if($("#default_common_form").attr("id")=="default_common_form"){//是否存在该form，当没有默认列表页面时会有异常信息，所以要判断
		...
		iMatrix.alert(iMatrixMessage["mms.thereIsNoCorrespondingList"]);
	}
}
...
function deleteSuccessMsg(){
	__show_message('opt_message',iMatrixMessage["deleteSuccess"],'onSuccess');
}
...
function selectOne(){
	...
	if(selectCount == 1){
		...
		__show_message('opt_message',iMatrixMessage["common.pleaseChooseAndCanOnlyChooseOne"],'onError');
		return false;
	}
}
// 选择一项以上
function selectOneOrMore(){
	...
	if(selectCount >= 1){
		...
		__show_message('opt_message',iMatrixMessage["selectInfo"],'onError');
		return false;
	}
}
