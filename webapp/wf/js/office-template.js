...
function uploadTemplate(){
	swfu = new SWFUpload({
		...
		button_width: 250,
		button_height: 18,
		button_text : '<span class="button"><s:text name="wf.template.clickOnThe"></s:text></span>',
...
function fileQueueError(file, errorCode, message) {
	try {
		var errormessage = iMatrixMessage["wf.engine.goWrong"];
		var errorName = "";
		if (errorCode === SWFUpload.errorCode_QUEUE_LIMIT_EXCEEDED) {
			errorName = iMatrixMessage["wf.engine.youUploadThe"];
		}
		...
		case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
			errormessage = iMatrixMessage["wf.engine.uploadFileSize"];
			break;
		case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
			errormessage = iMatrixMessage["wf.engine.uploadFilesOut"];
			break;
		case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
			errormessage = iMatrixMessage["wf.engine.numberOfFiles"]+message;
			break;
		case SWFUpload.QUEUE_ERROR.INVALID_FILETYPE:
			errormessage = iMatrixMessage["wf.engine.invalidFileType"];
			break;
		default:
...
function uploadStart(file) {
	try {
		...
		progress.setStatus(iMatrixMessage["wf.engine.isUploading"]);
