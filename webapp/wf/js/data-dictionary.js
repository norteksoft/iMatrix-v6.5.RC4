...
function deleteDict(url,type){
	...
	if(ids==''){
		...
		iMatrix.confirm({
			...
			confirmCallback:deleteDictOk,
			parameters:{ids:ids,url:url,type:type}
		});
	}
}
function deleteDictOk(obj){
	$("#dictIds").attr("value",obj.ids);
	setPageState();
	ajaxSubmit("defaultForm", obj.url , "dict_zone","",obj.type);
}
function deleteDictType(url){
	...
	if(ids==''){
		...
		iMatrix.confirm({
			message:iMatrixMessage["wf.engine.referenceToThe"],
			confirmCallback:deleteDictTypeOk,
			parameters:{ids:ids,url:url}
		});
	}
}
function deleteDictTypeOk(obj){
	$("#dictIds").attr("value",obj.ids);	
	setPageState();
	ajaxSubmit("defaultForm", obj.url , "dict_zone","","page");
}
...
function addUsers(id){
	if($('#userNamesView').html()==iMatrixMessage["common.allPersonnel"]){
		iMatrix.alert(iMatrixMessage["wf.engine.hasBeenAll"]);
	}else{
		if(typeof(id)=='undefined' || id == "") id="0";
		$.colorbox({href:webRoot+'/engine/data-dictionary-addUsers.htm?id='+id+'&showPage="transactors"',iframe:true, innerWidth:700, innerHeight:400,overlayClose:false,title:iMatrixMessage["wf.engine.choosePeople"],onClosed:function(){$("#addBtn").removeClass("cboxElement");}});
...
function clearAway(){
	if($('#slcMan').children().length==0){
		iMatrix.alert(iMatrixMessage["wf.engine.noManager"]);
	}else{
		...
		init_colorbox(url,iMatrixMessage["wf.engine.removeManager"]);
	}
}
function idSlct(id){
	$("#dict_id").attr("value", id);
...
function addUser(user, view,type){
	if(view==iMatrixMessage["common.allPersonnel"]){
		if(type==0){
...
function addInfo(userInfos,deptInfos,groupInfos,allUsers,rankInfos){
	...
	if(allUsers==iMatrixMessage["common.allPersonnel"]){
		addUser("", iMatrixMessage["common.allPersonnel"],0);
	}
	if(userInfos!=""){
...
function validate(){
	$("#dictForm").validate({
		...
		messages: {
			info: iMatrixMessage["common.required"],
			displayIndex:{
				required:iMatrixMessage["common.required"],
				digits:iMatrixMessage["wf.engine.enterInteger"],
				min: iMatrixMessage["wf.engine.pleaseEnterANumber"]
			}
		}
	});
}
...
function validateType(){
	$("#dictTypeForm").validate({
		...
		messages: {
			no: iMatrixMessage["common.required"],
...
function validateTypeNo(){
	$.ajax({
		   ...
		   success: function(data){
				if(data=="true"){
					iMatrix.alert(iMatrixMessage["wf.engine.numberExists"]);
					$("#no").attr("value","");
				}
		   }
		}); 
...
function save_type_form(){
	if($("#id").val()==""){
		$.ajax({
			   type: "POST",
			   url: webRoot+"/engine/data-dictionary-type-validateTypeNo.htm",
			   data: "no="+$("#no").attr("value"),
			   success: function(data){
			   		if(data=="true"){
			   			iMatrix.alert(iMatrixMessage["wf.engine.numberExists"]);
...
function addProcesses(id,btnId){
	if(typeof(id)=='undefined' || id == "") id="0";
	$.colorbox({href:webRoot+'/engine/data-dictionary-addProcesses.htm?id='+id,iframe:true, width:700, height:500,overlayClose:false,title:iMatrixMessage["wf.engine.selectionProcess"],onClosed:function(){$("#"+btnId).removeClass("cboxElement");}});
...
function addProcessTb(){
	$("#processRange").html('');
	$('#processDiv').html('');
	$("#processDiv").append('<table class="leadTable" id="processTb" style="width:440px;"><thead><tr><th style="width:300px;">'+iMatrixMessage["wf.engine.process"]+'</th><th style="width:140px;">'+iMatrixMessage["wf.engine.operation"]+'</th></tr></thead><tbody id="processViewTb"></tbody></table>');
}
function addProcess(info,viewInfo){
	...
	$('#processViewTb').append('<tr><td>'+viewInfo+'</td><td><a href="#" onclick="deleteDictProcess(this,\''+processId+'\',\''+viewInfo+'\',\''+info+'\');">'+iMatrixMessage["basicSetting.delete"]+'</td></tr>');
}
