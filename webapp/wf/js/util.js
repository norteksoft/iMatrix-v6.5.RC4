...
function deleteText(documentId){
	iMatrix.confirm({
		...
		confirmCallback:deleteTextOk,
		parameters:documentId
	});
}
function deleteTextOk(documentId){
...
//	}
	...
	iMatrix.confirm({
		message:iMatrixMessage["common.attache.delete.confirm.info"],
		confirmCallback:deleteUploadOk,
		parameters:{url:url,id:id}
	});
}
function deleteUploadOk(obj){
	$("#workflow_attachments_form").attr("action",obj.url+"?documentId="+obj.id);
	ajaxAnywhere.formName = "workflow_attachments_form";
	ajaxAnywhere.getZonesToReload = function(){
		return "history_refresh_zone";
	};
	ajaxAnywhere.onAfterResponseProcessing = function () {
		$("#upload_tr_"+obj.id).remove();
	};
	ajaxAnywhere.submitAJAX();
}
...
function addSWf(type){
	...
	if(typeof($("#saveUrl").val()) != 'undefined'){
		so.addVariable("saveUrl", $("#saveUrl").val());
	}
	so.addVariable("localeLanguage", $("#localeLang").val());
	if(type=="add"){
