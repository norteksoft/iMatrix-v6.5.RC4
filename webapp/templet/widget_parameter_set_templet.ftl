	    ...
	    <div id="message_${widgetId?if_exists?string('###0')}" style="display:none;"></div>
			...
			<#list widgetParameterList as p>
				<#if p.controlType?if_exists=='parameter.type.checkbox'>
					 <tr>
						<td  style="width:30%;text-align:center;"> ${showContentTitle?if_exists}:</td>
						<td>
							<input type="checkbox" name="${p.code}"   value="只显示通知标题"     id="${p.code}"  />${showContentNoticeTile?if_exists}<br>
							<input type="checkbox" name="${p.code}"   value="只显示通知内容"     id="${p.code}"  />${showContentNoticeContent?if_exists}<br>
							<input type="checkbox" name="${p.code}"   value="只显示通知时间"     id="${p.code}"  />${showContentNoticeTime?if_exists}<br>
						</td>
					 </tr>
				<#elseif p.controlType?if_exists=='parameter.type.radio'>
					<tr>
						<td  style="width:30%;text-align:center;"> ${changeTabOrder?if_exists}：</td>
						<td>
							<#list p.values as v>
								<input <#if v.value=='yes'>checked="checked"</#if> type="radio" name="${p.code}" id="" value="yes"/>${changeTabOrderYes?if_exists}
								<input <#if v.value=='no'>checked="checked"</#if> type="radio" name="${p.code}" id="" value="no"/>${changeTabOrderNo?if_exists}
							</#list>
						</td>
					</tr>
				<#elseif p.controlType?if_exists=='parameter.type.text'>
					<tr>
						<td  style="width:30%;text-align:center;">${p.titleTemp}：</td>
						<td>
					...
					<tr>
						...
						<td >
							<select style="width:80%;" id="${p.code}" name="${p.code}" >
							    ...
							    <#if o.selected>
									<option  value="${o.value}" id="${o.value}" selected="selected">${o.viewName}</option>
								<#else>
								    <option  value="${o.value}" id="${o.value}" >${o.viewName}</option>
								</#if>	
								</#list> 
							</select>
							<#list p.parameterValues as v>
			...
			<a style="flort:right;" class="btn" href="#" onclick="doSubmit(${widgetId?if_exists?string('###0')});"><span><span >${doQuery?if_exists}</span></span></a>
			</div>
