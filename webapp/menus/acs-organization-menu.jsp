...
function accordionChange(event,ui){
	...
	if(url=="user.action"){//用户管理
       	initUserTree();
	}else if(url=="department.action"){//部门管理
		initDepartmentTree();
...
function initUserTree(){
	...
	$.fn.zTree.init($("#company_user"), setting);
}

function initSelectFirstNode(treeId){
	if(selectNodeId==""){//表示初始化时默认选中树的根节点
...
var selectNodeId="";
function getAsyncUserUrl(treeId, treeNode) {
	selectNodeId = "";
	var param = "";
	if (typeof (treeNode) != "undefined" && treeNode != null) {
		selectNodeId = treeNode.id;
		param = "treeNodeId=" + treeNode.id;
	} else {
		param = "treeNodeId=INITIALIZED_USERS";
	}
	return "${acsCtx}/organization/load-tree-loadDepartmentTree.action?currentId=INITIALIZED_USERS&"+param;
}

function settingOType(){
	$("#oType").attr("value","company");
	initSelectFirstNode("company_user");
}
// 用户树选择事件
function selectUserNode(event, treeId, treeNode){
	...
	var id = node.substring(node.lastIndexOf("-")+1);
	var type = node.substring(0,node.lastIndexOf("-"));
	var url="";
	if (type == 'USERSBYDEPARTMENT'||type=='USERSBYBRANCH'||type == 'USERSBYDEPARTMENT-BRANCH'||type=='USERSBYBRANCH-BRANCH') {//部门或者分支机构
...
function initDepartmentTree(){
	...
	var setting = {
		...
		callback : {
			...
			onAsyncSuccess: initSelectFirstNodeForDept
		}
	};
	$.fn.zTree.init($("#company_department"), setting);
}
...
function getAsyncDeptUrl(treeId, treeNode) {
	...
	if (typeof (treeNode) != "undefined" && treeNode != null) {
		...
		param = "treeNodeId=INITIALIZED";
	}
	return "${acsCtx}/organization/load-tree-loadDepartmentTree.action?currentId=INITIALIZED&"+param;
}

function initSelectFirstNodeForDept(event, treeId, treeNode){
	initSelectFirstNode(treeId);
}
function getdeptId(){
...
function selectDeptNode(event, treeId, treeNode){
	var node = treeNode.id;
	var zTree = $.fn.zTree.getZTreeObj(treeId);
	if(node=='undefined'||node==''){ return; }
...
<div id="accordion1" class="basic">
	<security:authorize ifAnyGranted="userList">
		<h3><a href="user.action" id="_user" ><s:text name="common.userManager"/></a></h3><!-- 用户管理 -->
		<div>
			<ul class="ztree" id="company_user"></ul>
		</div>
	</security:authorize>
	<security:authorize ifAnyGranted="departmentManager">
		<h3><a href="department.action" id="_department" ><s:text name="common.departmentManager"/></a></h3><!-- 部门管理 -->
		<div>
			<ul class="ztree" id="company_department"></ul>
		</div>
	</security:authorize>
	<security:authorize ifAnyGranted="listWorkGroup">
		<h3><a href="work-group.action" id="_group" ><s:text name="common.workGroupManager"/></a></h3><!-- 工作组管理 -->
		<div>
