...
<div id="authorization_accordion" class="basic">
	...
	<h3><a href="branch-authority.action" id="_branches_authorization"><s:text name="menu.branchManager"/></a></h3><!-- 分支机构授权管理 -->
	<div>
		<ul class="ztree" id="branches_tree"></ul>
	</div>
	</security:authorize>
	<security:authorize ifAnyGranted="standardTree">
	<h3><a href="standard-role-data.action" id="_authorization"><s:text name="common.authorizationManager"/></a></h3><!-- 授权管理 -->
	<div>
	...
	<h3><a href="role.action" id="_role_mgmt"><s:text name="common.standRoleManager"/></a></h3><!-- 角色管理 -->
	<div>
		<ul class="ztree" id="role_tree"></ul>
	</div>
	</security:authorize>
	<security:authorize ifAnyGranted="acs_role_query">
	<h3><a href="role-query.action" id="_authorization_query" onclick="leftMenu('role-query.action','ROLE_USER','<s:text name="menu.selectUser"/>')"><s:text name="queryfunction.title"/></a></h3><!-- 权限查询 -->
	<div>
		<div id="fourMenuUser" class="four-menu-selected">
			<a href="#"  onclick="leftMenu('role-query.action','ROLE_USER','<s:text name="menu.selectUser"/>')"><s:text name="menu.userAuth"/></a><!-- 用户权限 -->
		</div>
		<div id="fourMenuDep" class="four-menu">
			<a href="#" class="" onclick="leftMenu('role-query.action','ROLE_DEPARTMENT','<s:text name="user.selectDepartment"/>')"><s:text name="menu.departmentAuth"/></a><!-- 部门权限 -->
		</div>
		<div id="fourMenuWork" class="four-menu">
			<a href="#" class="" onclick="leftMenu('role-query.action','ROLE_WORKGROUP','<s:text name="user.selectWorkGroup"/>')"><s:text name="menu.workgroupAuth"/></a><!-- 工作组权限 -->
		</div>
	</div>
	</security:authorize>
</div>
