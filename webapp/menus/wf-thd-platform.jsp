...
<div id="accordion" class="basic">
	...
	<div>
	 ...
	 -->
	<h3><a href="delegate-main-myDelegate.htm"><s:text name="wf.menu.taskDelegationMonitoring"></s:text></a></h3>
	<div>
	...
	<security:authorize ifAnyGranted="wf_engine_process_definition_collection">
	 ...
	 -->
	<h3><a href="delegate-main-superviseAsTrustee.htm"><s:text name="wf.menu.missionEntrustedMonitoring"></s:text></a></h3>
	<div>
		<ul class="ztree" id="supervise-as-trustee"></ul>
	</div>
	
	<h3><a href="delegate-main.htm"><s:text name="wf.menu.mandatoryAdministration"></s:text></a></h3>
	<div>
		<ul class="ztree" id="delegate-tree"></ul>
	</div>
	<h3><a href="task-transferTask.htm"><s:text name="wf.menu.taskTransferMonitoring"></s:text></a></h3>
	<div>
		<ul class="ztree" id="transfer-task-tree"></ul>
	</div>
	<h3><a href="delegate-main-myAccept.htm"><s:text name="wf.menu.taskReceivingMonitoring"></s:text></a></h3>
	<div>
		<ul class="ztree" id="task-accept"></ul>
	</div>
...
function treeTwo(){
	...
	tree.initTree({treeId:"delegate-tree",
		data:[{ id:'_MY_DELEGATE', pId:0, name:iMatrixMessage["wf.menu.myCommission"], iconSkin:'folder'},
		      { id:'_RECEIVE_DELEGATE', pId:0, name:iMatrixMessage["wf.menu.myFiduciary"], iconSkin:'folder'}],
		type:"ztree",
...
function transferTaskTree(){
	$.ajaxSetup({cache:false});
	//treeId:,url:,data(静态树才需要该参数):,multiple:,callback:
	tree.initTree({treeId:"transfer-task-tree",
		url:"${wfCtx}/engine/transfer-task-tree.htm",
		type:"ztree",
		initiallySelect:"TRANSFER_ING",
		callback:{
				onClick:selectZNode
			}});
}
...
function treeFive(){
	$.ajaxSetup({cache:false});
	//treeId:,url:,data(静态树才需要该参数):,multiple:,callback:
	tree.initTree({treeId:"task-accept",
		url:"${wfCtx}/engine/tree-delegateAccept.htm",
		type:"ztree",
		initiallySelect:"ACC_ING",
		callback:{
				onClick:selectZNode
			}});
}
...
function treechange(id){
	...
	}else if(id=='ACC_ING'){//任务接收监控/办理中  
		url=webRoot+"/engine/delegate-main-myAccept.htm?isDone=false";
	}else if(id=='ACC_END'){//任务委托监控/已完成
		url=webRoot+"/engine/delegate-main-myAccept.htm?isDone=true";
	...
	}else if(id=='TRANSFER_ING'){//任务委托监控/办理中
		url=webRoot+"/engine/task-transferTask.htm?isEnd=false";
	}else if(id=='TRANSFER_END'){//任务委托监控/已完成
		url=webRoot+"/engine/task-transferTask.htm?isEnd=true";
	}
...
function accordionChange(event,ui){
	...
	if(url=="workflow.htm"){//我的流程
		...
		treeTwo();
	}else if(url=="delegate-main-myAccept.htm"){//任务接收监控
		treeFive();
	}else if(url=="task-transferTask.htm"){//移交任务
		transferTaskTree();
	}
